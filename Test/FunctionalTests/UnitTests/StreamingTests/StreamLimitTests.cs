...
namespace UnitTests.StreamingTests
{
    ...
    public class StreamLimitTests : UnitTestBase
    {
        ...
        public async Task SMS_Limits_FindMax_Consumers()
        {
            ...
            IStreamLifecycleProducerGrain producer = StreamLifecycleProducerGrainFactory.GetGrain(Guid.NewGuid());
            await producer.BecomeProducer(streamId, streamProviderName);

            int loopCount = 0;
            long grainIdStart = random.Next();
            try
            {
                // Loop until something breaks!
                for (loopCount = 1; loopCount <= MaxExpectedPerStream; loopCount++)
                {
                    long grainId = grainIdStart + loopCount;
                    IStreamLifecycleConsumerGrain consumer = StreamLifecycleConsumerGrainFactory.GetGrain(grainId);
        ...
        [TestMethod, TestCategory("BVT"), TestCategory("Nightly"), TestCategory("Streaming"), TestCategory("Limits")]
        public async Task SMS_Limits_FindMax_Producers()
        {
            ...
            IStreamLifecycleConsumerGrain consumer = StreamLifecycleConsumerGrainFactory.GetGrain(Guid.NewGuid());
            await consumer.BecomeConsumer(streamId, streamProviderName);
            ...
            try
            {
                ...
                {
                    long grainId = grainIdStart + loopCount;
                    IStreamLifecycleProducerGrain producer = StreamLifecycleProducerGrainFactory.GetGrain(grainId);
                    await producer.BecomeProducer(streamId, streamProviderName);
                }
            }
            catch (Exception exc)
            ...
            bool normalSubscribeCalls = true)
        {
            ...
            if (numProducers > 0 && warmUpProducers)
            {
                ...
                {
                    var grain = StreamLifecycleProducerGrainFactory.GetGrain(i);
            ...
            if (numProducers > 0)
            {
                ...
                for (int i = 0; i < numStreams; i++)
                {
                    ...
                    long idxProducer = i % numProducers;
                    var grain = StreamLifecycleProducerGrainFactory.GetGrain(idxProducer);
        ...
        private void WarmUpPubSub(string streamProviderName, Guid[] streamIds, AsyncPipeline pipeline)
        {
            ...
            for (int i = 0; i < numStreams; i++)
            {
                ...
                IPubSubRendezvousGrain pubsub = PubSubRendezvousGrainFactory.GetGrain(streamId, extKey);
        ...
        private static IList<Task> SetupProducers(Guid streamId, string streamProviderName, AsyncPipeline pipeline, int numProducers)
        {
            ...
            long producerIdStart = random.Next();
            for (int loopCount = 0; loopCount < numProducers; loopCount++)
            {
                long grainId = producerIdStart + loopCount;

                var grain = StreamLifecycleProducerGrainFactory.GetGrain(grainId);
                producers.Add(grain);
        ...
        private static IList<Task> SetupConsumers(Guid streamId, string streamProviderName, AsyncPipeline pipeline, int numConsumers, bool normalSubscribeCalls)
        {
            ...
            for (int loopCount = 0; loopCount < numConsumers; loopCount++)
            {
                ...
                var grain = StreamLifecycleConsumerGrainFactory.GetGrain(grainId);
                consumers.Add(grain);
            ...
            bool useFanOut = true)
        {
            ...
            var pubSub = ClientProviderRuntime.Instance.PubSub(StreamPubSubType.GrainBased);
