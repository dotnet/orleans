...
namespace UnitTests.Streaming.Reliability
{
    ...
    public class StreamReliabilityTests : UnitTestBase
    {
        ...
        public void TestCleanup()
        {
            ...
            {
                var queueMapper = new AzureQueueStreamQueueMapper(_streamProviderName);
                var queues = queueMapper.GetAllQueues().ToList();
                logger.Info("TestCleanup - DeleteAllAzureQueues Count={0}", queues.Count);
                DeleteAllAzureQueues(_streamProviderName, DeploymentId, TestConstants.DataConnectionString);
            }
#if DELETE_AFTER_TEST
        ...
        public async Task SMS_AllSilosRestart_UnsubscribeConsumer()
        {
            ...
            long consumerGrainId = random.Next();
            var consumerGrain = StreamUnsubscribeTestGrainFactory.GetGrain(consumerGrainId);
