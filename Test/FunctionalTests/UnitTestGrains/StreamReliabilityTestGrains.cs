...
namespace UnitTestGrains
{
    public interface IStreamReliabilityTestGrainState : IGrainState
    {
        ...
        // For consumer only.
#if USE_GENERICS
        HashSet<StreamSubscriptionHandle<T>> ConsumerSubscriptionHandles { get; set; }
#else
...
#else
    public class StreamReliabilityTestGrain : Grain<IStreamReliabilityTestGrainState>, IStreamReliabilityTestGrain
#endif
    {
        [NonSerialized]
...
#endif
        {
            ...
            await Stream.UnsubscribeAsync(subsHandle);
        ...
        public async Task RemoveAllConsumers()
        {
            ...
            {
                await consumer.UnsubscribeAsync(handle);
            }
            //Observers.Remove(subsHandle);
            State.ConsumerSubscriptionHandles.Clear();
            await State.WriteStateAsync();
...
#endif
        {
            logger.Info("ReconnectConsumerHandles SubscriptionHandles={0} Grain={1}", Utils.EnumerableToString(subscriptionHandles), this.AsReference());
            ...
            foreach (var subHandle in subscriptionHandles)
            {
                ...
                var observer = new MyStreamObserver<T>();
#else
                // var stream = GetStreamProvider(State.StreamProviderName).GetStream<int>(subHandle.Stream.StreamId.AsGuid);
                var stream = subHandle.Stream;
                var observer = new MyStreamObserver<int>(logger);
#endif
                var subsHandle = await stream.SubscribeAsync(observer);
    ...
    [Orleans.Providers.StorageProvider(ProviderName = "AzureStore")]
    public class StreamUnsubscribeTestGrain : Grain<IStreamReliabilityTestGrainState>, IStreamUnsubscribeTestGrain
    {
        [NonSerialized]
        private Logger logger;
        ...
        {
            logger.Info("Subscribe StreamId={0} StreamProvider={1} Grain={2}", streamId, providerName, this.AsReference());
