...
namespace PresenceConsoleTest
{
    class MyProgram
    {
        ...
        private const int REPORT_BLOCK_SIZE = 5000;
        ...
        private static void Main(string[] args)
        {
            ...
            var promises = new AsyncValue<List<Exception>>[NUM_WORKERS];
            ...
            for (int i = 0; i < NUM_WORKERS; i++)
            {
                ...
                workers[i].Initialize(NUM_USERS, _numRequests / NUM_WORKERS, REPORT_BLOCK_SIZE, PIPELINE_SIZE, callback, gw, (instanceIndex * NUM_WORKERS + i) % NUM_AZURE_SILOS, NUM_INITIAL_BLOCKS, NUM_MAX_BLOCKS, RAN_TIMER_INTERVAL, RandomBuckets, warmup && i == 0);
                Thread.Sleep(10);
            }
            ...
            Orleans.ActorClient.Logger.Info("{2} Starting sending {0} heartbeats for {1} sessions...", _numRequests, NUM_USERS, DateTime.Now);

            for (int i = 0; i < NUM_WORKERS; i++)
            {
                int index = i;
                promises[i] = AsyncValue<List<Exception>>.StartNew(() => workers[index].Run());
                Thread.Sleep(1 * 1000);
            }
            ...
            Orleans.ActorClient.Logger.Info(string.Format("Test completed successfully in {0} milliseconds", (end - start).TotalMilliseconds));
            Orleans.ActorClient.Logger.Info(string.Format("Average TPS : {0}", _numRequests / (end - start).TotalSeconds));
            Orleans.ActorClient.Logger.Info(totalAggregateLatency.ToString());        
        }
        ...
        private static bool ReportCallback(string workerName, int nFailures, int nPipelineSize, int nLate, int nBusy, Latency aggregateLatency)
        {
            lock (typeof(MyProgram))
            {
                ...
                if (nReports % NUM_REPORTS_IN_BLOCK == 0)
                {
                    ...
                    Orleans.ActorClient.Logger.Info(sb.ToString());
                    previousReport = now;
                    pipelineSize = 0;
                    totalFailures = 0;
                    totalLate = 0;
