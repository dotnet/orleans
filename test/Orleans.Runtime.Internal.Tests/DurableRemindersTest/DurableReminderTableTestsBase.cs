...
using Xunit;
using DurableReminderEntry = Orleans.DurableReminders.ReminderEntry;
using DurableReminderTableData = Orleans.DurableReminders.ReminderTableData;
using DurableReminderTable = Orleans.DurableReminders.IReminderTable;
...
public abstract class DurableReminderTableTestsBase : IAsyncLifetime, IClassFixture<ConnectionStringFixture>
{
    ...
    private readonly DurableReminderTable remindersTable;

    protected const string testDatabaseName = "OrleansDurableReminderTest";

    protected DurableReminderTableTestsBase(ConnectionStringFixture fixture, TestEnvironmentFixture clusterFixture, LoggerFilterOptions filters)
    {
    ...
    protected abstract DurableReminderTable CreateRemindersTable();
    ...
    private static async Task TestRemindersHashInterval(DurableReminderTable reminderTable, uint beginHash, uint endHash, uint[] reminderHashes)
    {
    ...
    private static DurableReminderEntry CreateReminder(GrainId grainId, string reminderName)
    {
        var now = DateTime.UtcNow;
        now = new DateTime(now.Year, now.Month, now.Day, now.Hour, now.Minute, now.Second, DateTimeKind.Utc);
        return new DurableReminderEntry
        {
            GrainId = grainId,
