...
public partial class DisseminationProtocolTests
{
    ...
    {
        var (_, peer, transport, ns) = CreatePeerFixture(39101, 39102);
    ...
    [Fact]
    public async Task PublishQueuesBroadcastToDeterministicTreeChildrenWithoutCapabilityProbing()
    {
    ...
    {
        var (_, _, transport, ns) = CreatePeerFixture();
        var protocol = CreateProtocol(transport, ns);
    ...
    public async Task NewPublicationWakesFailedPeerWithoutWaitingForBackoff()
    {
        ...
        var retry = schedules.WaitAsync(
            scheduled => scheduled.LocalSilo.Equals(local) && scheduled.Peer.Equals(peer)
                && scheduled.Reason == DisseminationBroadcastScheduleReason.Retry,
        ...
        try
        {
            ...
            var beforeNotification = timeProvider.GetTimestamp();
            ...
            Assert.Equal(TimeSpan.Zero, timeProvider.GetElapsedTime(beforeNotification));
    ...
    [InlineData(4000, 100)]
    [InlineData(50, 50)]
    public async Task SendFailureRetriesAutomaticallyWithBoundedBackoff(int antiEntropyMilliseconds, int retryMilliseconds)
    {
        var local = CreateSilo(11111);
        var peer = CreateSilo(11112);
        var transport = new FakeTransport(local, peer);
        ...
        var protocol = CreateProtocol(
            transport,
            ns,
            options => options.Overlay.AntiEntropyInterval = TimeSpan.FromMilliseconds(antiEntropyMilliseconds),
            timeProvider);
        using var schedule = new BroadcastScheduleObserver();
        var scheduledRetry = schedule.WaitAsync(
            e => e.LocalSilo.Equals(local) && e.Peer.Equals(peer) && e.Reason == DisseminationBroadcastScheduleReason.Retry,
            TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
        ...
        var retry = await scheduledRetry;
        Assert.Equal(TimeSpan.FromMilliseconds(retryMilliseconds), retry.DueTime);

        timeProvider.Advance(retry.DueTime - TimeSpan.FromMilliseconds(1));
    ...
    public async Task NewNotificationResetsRetryBackoff()
    {
        ...
        var protocol = CreateProtocol(
            ...
            timeProvider);
        using var schedule = new BroadcastScheduleObserver();
        var firstRetry = schedule.WaitAsync(
            e => e.LocalSilo.Equals(local) && e.Peer.Equals(peer) && e.Reason == DisseminationBroadcastScheduleReason.Retry && e.Attempt == 1,
            TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
        ...
        Assert.True(await PublishValue(
            ...
            TestContext.Current.CancellationToken));
        await firstAttempt.Task.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
        Assert.Equal(TimeSpan.FromMilliseconds(100), (await firstRetry).DueTime);
        var secondRetry = schedule.WaitAsync(
            e => e.LocalSilo.Equals(local) && e.Peer.Equals(peer) && e.Reason == DisseminationBroadcastScheduleReason.Retry && e.Attempt == 2,
            TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
        timeProvider.Advance(TimeSpan.FromMilliseconds(100));
        await secondAttempt.Task.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
        Assert.Equal(TimeSpan.FromMilliseconds(200), (await secondRetry).DueTime);
    ...
    [Fact]
    public async Task NamespaceNotificationFlushesImmediately()
    {
        var local = CreateSilo(11111);
        var peer = CreateSilo(11112);
        var transport = new FakeTransport(local, peer);
        ...
        var scheduled = await schedule.WaitAsync(
            e => e.LocalSilo.Equals(local) && e.Peer.Equals(peer) && e.Reason == DisseminationBroadcastScheduleReason.Immediate,
            TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
        ...
        Assert.Equal(TimeSpan.Zero, timeProvider.GetElapsedTime(start));
    ...
    public async Task RemovedPendingKeyIsDroppedWithoutRetry()
    {
        ...
        var publication = BeforeBroadcastPumpsRun(() =>
        {
            var pending = PublishValue(protocol, ns, ns.CreateValue("removed", sequence: 1), TestContext.Current.CancellationToken);
    ...
    {
        var (local, peer, transport, ns) = CreatePeerFixture();
        var timeProvider = new FakeTimeProvider();
        var sendCount = 0;
        var attempted = new List<DisseminationKey>();
        transport.SendBroadcastHandler = (target, batch, cancellationToken) =>
        {
            attempted.Add(Assert.Single(GetBroadcastValues(batch)).Value.Key);
            if (Interlocked.Increment(ref sendCount) == 1)
            {
                throw new InvalidOperationException("transient send failure");
            }
        ...
        ns.SetValue("first", 1);
        ns.SetValue("second", 1);
        ns.SetValue("third", 1);
        var queue = CreateBroadcastQueue(transport, [ns], options => options.MaxBatchItems = 1, timeProvider);
        using var schedules = new BroadcastScheduleObserver();
        var retry = schedules.WaitAsync(
            scheduled => scheduled.LocalSilo.Equals(local) && scheduled.Peer.Equals(peer)
                && scheduled.Reason == DisseminationBroadcastScheduleReason.Retry,
        ...
        try
        {
            Assert.True(queue.NotifyBatch(peer, ns, [new("first", 1, true), new("second", 1, true), new("third", 1, true)]));
            await retry;
            Assert.Equal(1, sendCount);
            Assert.Equal(new DisseminationKey[] { "first" }, attempted);
    ...
    public async Task BroadcastPeerFailureDoesNotBlockOtherPeerPumps()
    {
        ...
        using var schedules = new BroadcastScheduleObserver();
        var retry = schedules.WaitAsync(
            scheduled => scheduled.LocalSilo.Equals(local) && scheduled.Peer.Equals(failedPeer)
                && scheduled.Reason == DisseminationBroadcastScheduleReason.Retry,
            TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);

        Assert.True(await PublishValue(protocol, ns, ns.CreateValue("first", sequence: 1), TestContext.Current.CancellationToken));
        await Task.WhenAll(retry, healthySent.Task).WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
    ...
    {
        var (_, peer, transport, ns) = CreatePeerFixture();
        var protocol = CreateProtocol(transport, ns, options => options.Overlay.FanOutFactor = static _ => 1);

        var publications = BeforeBroadcastPumpsRun(() =>
        {
            var before = PublishValue(protocol, ns, ns.CreateValue("before-removal", sequence: 1), TestContext.Current.CancellationToken);
            transport.Peers.Remove(peer);
            var after = PublishValue(protocol, ns, ns.CreateValue("during-removal", sequence: 2), TestContext.Current.CancellationToken);
            return (before, after);
        });
        Assert.True(await publications.before);
        Assert.True(await publications.after);
        await protocol.FlushPendingBroadcast(TestContext.Current.CancellationToken);
    ...
    public async Task StopDrainsPendingBroadcastPumps()
    {
        ...
        var pending = BeforeBroadcastPumpsRun(() => (
            Publication: PublishValue(protocol, ns, ns.CreateValue("before-stop", sequence: 1), TestContext.Current.CancellationToken),
            Stop: protocol.StopAsync(TestContext.Current.CancellationToken)));
        Assert.True(await pending.Publication);
        await pending.Stop;
    ...
    [Fact]
    public async Task NotificationDuringInFlightSendRepairsLatestFullValueAfterAcknowledgment()
    {
        var local = CreateSilo(11111);
        var peer = CreateSilo(11112);
        var transport = new FakeTransport(local, peer);
        ...
        Assert.Equal(
            new[] { (From: 0L, To: 1L), (From: 0L, To: 2L) },
    ...
    [Theory]
    [InlineData(1, 0, 3, 3, 1)]
    [InlineData(2, 2, 3, 2, 0)]
    [InlineData(1, 2, 3, 1, 0)]
    [InlineData(1, -1, 3, 1, 0)]
    [InlineData(1, 0, 0, 1, 0)]
    [InlineData(1, 0, -1, 1, 0)]
    public async Task ReceiveBroadcastAppliesOnlyPositiveFullValues(
        long localVersion, long fromVersion, long toVersion, long expectedVersion, int expectedApplyCount)
    {
        var (_, root, transport, ns) = CreatePeerFixture(11112, 11111);
        var protocol = CreateProtocol(transport, ns);
        ns.SetValue(FakeNamespace.DefaultKey, localVersion);
        var item = ns.CreateItem(root, FakeNamespace.DefaultKey, toVersion, fromVersion);
        using var rejected = new Phase6ApplyObserver(ns.Name, FakeNamespace.DefaultKey, transport.LocalSilo, root);

        var response = await protocol.ReceiveBroadcast(
            CreateBroadcastBatch(root, item),
            TestContext.Current.CancellationToken);

        Assert.Equal(expectedVersion, ns.GetVersion(FakeNamespace.DefaultKey));
        Assert.Equal(expectedApplyCount, ns.ApplyCounts.GetValueOrDefault(FakeNamespace.DefaultKey));
        Assert.Equal(expectedApplyCount != 0, ns.ApplyCounts.ContainsKey(FakeNamespace.DefaultKey));
        Assert.Equal(expectedApplyCount == 0 ? 1 : 0, rejected.Events.Count);
        ...
        Assert.Equal(expectedVersion, acknowledgment.Version);
    }
    ...
    [Fact]
    public async Task BroadcastMaterializesLatestQueuedVersion()
    {
        var (_, peer, transport, ns) = CreatePeerFixture();
        var protocol = CreateProtocol(transport, ns);

        var publications = BeforeBroadcastPumpsRun(() => (
            First: PublishValue(protocol, ns, ns.CreateValue(FakeNamespace.DefaultKey, sequence: 1), TestContext.Current.CancellationToken),
            Second: PublishValue(protocol, ns, ns.CreateValue(FakeNamespace.DefaultKey, sequence: 2), TestContext.Current.CancellationToken)));
        var first = await publications.First;
        var second = await publications.Second;
        await protocol.FlushPendingBroadcast(TestContext.Current.CancellationToken);
    ...
    [Fact]
    public async Task BroadcastRepairsLatestFullValueAfterAcknowledgment()
    {
        var local = CreateSilo(11111);
        var peer = CreateSilo(11112);
        var transport = new FakeTransport(local, peer);
    ...
    [InlineData(3)]
    [InlineData(4)]
    public async Task BroadcastPublicationSupersedesMissingHistoryWithLatestFullValue(long latestVersion)
    {
        var local = CreateSilo(11111);
        var peer = CreateSilo(11112);
        var transport = new FakeTransport(local, peer);
        ...
        ns.SetValue(FakeNamespace.DefaultKey, latestVersion);
        Assert.True(await protocol.Publish(ns, FakeNamespace.DefaultKey, 1, TestContext.Current.CancellationToken));
        await protocol.FlushPendingBroadcast(TestContext.Current.CancellationToken);
        ...
        Assert.Equal(latestVersion, value.Value.ToVersion);
        Assert.Equal(latestVersion, BitConverter.ToInt64(value.Value.Payload.Span));
    }
    ...
    [InlineData(3, true)]
    [InlineData(5, false)]
    public async Task AntiEntropyResponseIncludesOnlyRequestedNewerValues(long peerVersion, bool hasUnrequestedKey)
    {
        var (_, peer, transport, ns) = CreatePeerFixture();
        var protocol = CreateProtocol(transport, ns);
        var key = hasUnrequestedKey ? new DisseminationKey("requested") : FakeNamespace.DefaultKey;
        ns.SetValue(key, version: 5);
        if (hasUnrequestedKey)
        {
        ...
        {
            Sender = peer,
            Digests = CreateAntiEntropyRequestDigest(ns.Name, (key, peerVersion)),
        }, TestContext.Current.CancellationToken);
        ...
        else
        {
            ...
            Assert.Equal(key, item.Value.Key);
    ...
    {
        var (_, peer, transport, ns) = CreatePeerFixture(11113, 11114);
    ...
    [Fact]
    public async Task AntiEntropyAppliesStaggeredFullResponsesAfterExchangesComplete()
    {
        var local = CreateSilo(11111);
        var fastPeer = CreateSilo(11112);
        ...
        transport.ExchangeAntiEntropyHandler = async (target, request, cancellationToken) =>
        {
            Assert.Equal(1, Assert.Single(request.Digests[ns.Name]).Version);
    ...
    [Fact]
    public async Task AntiEntropyRejectsNonFullRepairAndAppliesLaterFullValue()
    {
        var (local, peer, transport, _) = CreatePeerFixture();
        var ns = new ProtocolReviewNamespace(local);
        ns.Inner.SetValue(FakeNamespace.DefaultKey, version: 1);
        var protocol = CreateProtocol(transport, [ns]);
        var second = ns.Inner.CreateItem(
            peer,
            FakeNamespace.DefaultKey,
            sequence: 2,
            fromVersion: 1);
        var third = ns.Inner.CreateItem(
            peer,
            FakeNamespace.DefaultKey,
            sequence: 3);
        transport.ExchangeAntiEntropyHandler = (target, request, _) => ValueTask.FromResult(new DisseminationAntiEntropyResponse
        ...
        Assert.Equal(3, ns.GetVersion(FakeNamespace.DefaultKey));
        Assert.Equal(1, ns.Inner.ApplyCounts[FakeNamespace.DefaultKey]);
        Assert.Equal(new long[] { 3 }, ns.Attempts.Select(static value => value.ToVersion));
        Assert.Empty(transport.BroadcastBatches);
    }
    ...
    [InlineData(3, 2, 1)]
    [InlineData(2, 2, 1)]
    public async Task AntiEntropyAppliesFullResponsesWithoutRegressingState(
        long firstVersion, long secondVersion, int expectedApplyCount)
    {
        var local = CreateSilo(11111);
        ...
        {
            Assert.Equal(1, Assert.Single(request.Digests[ns.Name]).Version);
            var responseVersion = Interlocked.Increment(ref exchangeCount) == 1 ? firstVersion : secondVersion;
            var repair = ns.Inner.CreateItem(
                target,
                FakeNamespace.DefaultKey,
                sequence: responseVersion);
            return ValueTask.FromResult(new DisseminationAntiEntropyResponse
        ...
        Assert.Equal(Math.Max(firstVersion, secondVersion), ns.GetVersion(FakeNamespace.DefaultKey));
        Assert.Equal(peers.Length, exchangeCount);
        Assert.Equal(expectedApplyCount, ns.Inner.ApplyCounts[FakeNamespace.DefaultKey]);
        long[] expectedAttempts = secondVersion < firstVersion ? [firstVersion] : [firstVersion, secondVersion];
        Assert.Equal(expectedAttempts, ns.Attempts.Select(static value => value.ToVersion));
    }
    ...
    public async Task MonotonicDisseminationModelConformsToAccordantSpec()
    {
        ...
        spec.ExecuteWith<MonotonicDisseminationHarness>()
            .BindAsync<long, ModelApplyResponse>("Receive", static (harness, version) => harness.Receive(version, TestContext.Current.CancellationToken))
            .BindAsync<long, ModelRepairResponse>("RepairPeer", static (harness, peerVersion) => harness.RepairPeer(peerVersion, TestContext.Current.CancellationToken));
    ...
    [Fact]
    public async Task MembershipNamespaceRepairsCurrentFullSnapshotWithoutRetainingPeerBaseline()
    {
        var local = CreateSilo(11111);
        var peer = CreateSilo(11112);
        ...
        AssertMembershipState(updatedSnapshot, Assert.IsType<MembershipTableSnapshot>(update.Snapshot));
    ...
    public void MembershipNamespaceInvalidatesCachedPayloadForSameVersionUpdate()
    {
        ...
        Assert.Equal(DisseminationRepairStatus.Produced, firstRepair.Status);
        Assert.Equal(DisseminationRepairStatus.Produced, updatedRepair.Status);
        var firstValue = firstRepair.Value;
        var updatedValue = updatedRepair.Value;
    ...
    public async Task MembershipNamespaceRetainsPostApplySnapshotForRepair()
    {
        ...
        Assert.Equal(DisseminationApplyResult.Applied, result);
        Assert.Equal(DisseminationRepairStatus.Produced, repair.Status);
        var repairedValue = repair.Value;
    ...
    public async Task MembershipNamespacePublishesIndependentlyApplicableFullSnapshots()
    {
        ...
        Assert.Equal((0L, 2L), (second.FromVersion, second.ToVersion));
        Assert.Equal((0L, 3L), (third.FromVersion, third.ToVersion));
        ...
        Assert.NotNull(Assert.IsType<MembershipTableSnapshotUpdate>(
            serializer.Deserialize<MembershipTableSnapshotUpdate>(second.Payload)).Snapshot);
        Assert.NotNull(Assert.IsType<MembershipTableSnapshotUpdate>(
            serializer.Deserialize<MembershipTableSnapshotUpdate>(third.Payload)).Snapshot);
        ...
        AssertMembershipState(thirdSnapshot, receiverManager.CurrentSnapshot);

        var lateReceiver = new FakeMembershipManager(CreateMembershipSnapshot(MembershipVersion.MinValue.Value));
        var lateNamespace = CreateMembershipNamespace(lateReceiver, serializer);
        Assert.Equal(DisseminationApplyResult.Applied, await lateNamespace.ApplyValueAsync(third, TestContext.Current.CancellationToken));
        AssertMembershipState(thirdSnapshot, lateReceiver.CurrentSnapshot);
    }
    ...
    public async Task MembershipNamespacePublishesCurrentSnapshotWhenNotificationIsSuperseded(bool baselineAccepted, long nextVersion)
    {
        ...
        var sourceNamespace = CreateMembershipNamespace(sourceManager, serializer);
        var disseminationService = new FakeDisseminationService();
        disseminationService.Results.Enqueue(baselineAccepted);
        disseminationService.Results.Enqueue(true);

        Assert.Equal(baselineAccepted, await sourceNamespace.PublishAsync(disseminationService, firstSnapshot, TestContext.Current.CancellationToken));
        sourceManager.CurrentSnapshot = secondSnapshot;
        Assert.True(await sourceNamespace.PublishAsync(disseminationService, firstSnapshot, TestContext.Current.CancellationToken));

        var value = disseminationService.Values[1];
        Assert.Equal((0L, nextVersion), (value.FromVersion, value.ToVersion));
        var update = Assert.IsType<MembershipTableSnapshotUpdate>(
            serializer.Deserialize<MembershipTableSnapshotUpdate>(value.Payload));
        AssertMembershipState(secondSnapshot, Assert.IsType<MembershipTableSnapshot>(update.Snapshot));
    }
    ...
    public async Task MembershipNamespacePublishesFullInventoryWhenTopologyChanges()
    {
        ...
        var update = Assert.IsType<MembershipTableSnapshotUpdate>(
            serializer.Deserialize<MembershipTableSnapshotUpdate>(disseminationService.Values[1].Payload));
        AssertMembershipState(secondSnapshot, Assert.IsType<MembershipTableSnapshot>(update.Snapshot));
    }
    ...
    [Fact]
    public void NamespaceOptionsValidatorAllowsStaleTtlShorterThanPublicationPeriod()
    {
    ...
    [InlineData(1, true)]
    [InlineData(0, false)]
    public async Task ReceiveBroadcastUsesOnlyPositiveRelativeLifetime(int milliseconds, bool skewClock)
    {
        var (_, peer, transport, ns) = CreatePeerFixture();
        var timeProvider = skewClock
            ? new FakeTimeProvider(DateTimeOffset.MaxValue - TimeSpan.FromSeconds(1))
            : TimeProvider.System;
        var protocol = CreateProtocol(transport, ns, timeProvider: timeProvider);
        ...
        {
            Value = item.Value,
            TimeToLive = TimeSpan.FromMilliseconds(milliseconds),
        };

        await protocol.ReceiveBroadcast(CreateBroadcastBatch(peer, item), TestContext.Current.CancellationToken);

        Assert.Equal(milliseconds > 0 ? 1 : 0, ns.GetVersion(FakeNamespace.DefaultKey));
    }
    ...
    public async Task UnexpectedPeerPumpIterationFailure_IsRetriedAndCompletesFlushAndStopDrainWaiters()
    {
        ...
        Assert.Equal(TimeSpan.FromMilliseconds(200), retry.DueTime);
    ...
    private static T BeforeBroadcastPumpsRun<T>(Func<T> action)
    {
        var context = new DeferredBroadcastContext();
    ...
    private sealed class DeferredBroadcastContext : SynchronizationContext
    {
        ...
        public override void Post(SendOrPostCallback callback, object? state)
        {
            ...
            ThreadPool.QueueUserWorkItem(static item => item.Callback(item.State), (Callback: callback, State: state), preferLocal: false);
        }
        ...
        public void Resume()
        {
            lock (_pending)
            {
                ...
                {
                    ThreadPool.QueueUserWorkItem(static work => work.Callback(work.State), item, preferLocal: false);
                }
    ...
    private static (SiloAddress Local, SiloAddress Peer, FakeTransport Transport, FakeNamespace Namespace) CreatePeerFixture(
        int localPort = 11111, int peerPort = 11112)
    {
        var local = CreateSilo(localPort);
        var peer = CreateSilo(peerPort);
        return (local, peer, new FakeTransport(local, peer), new FakeNamespace(local));
    }
    ...
    private static DisseminationProtocol CreateProtocol(
        ...
        Microsoft.Extensions.Logging.ILogger<DisseminationBroadcastQueue>? broadcastLogger = null)
    {
        ...
        return new DisseminationProtocol(
            ...
            broadcastLogger ?? NullLogger<DisseminationBroadcastQueue>.Instance);
    }
    ...
    private static DisseminationProtocol CreateProtocolWithBroadcastLogger(
        ...
        CreateProtocol(transport, [ns], configure, timeProvider, broadcastLogger: broadcastLogger);
    ...
    private sealed class FakeNamespace : IDisseminationNamespace
    {
        ...
        private readonly Dictionary<DisseminationKey, DisseminationValue> _publishedValues = new();
        private readonly DisseminationNamespace _name;
        private int _repairRequestCount;

        public FakeNamespace(
            SiloAddress localSilo,
            DisseminationNamespace? name = null,
        ...
        public TimeSpan AggregationPeriod { get; set; } = TimeSpan.FromSeconds(1);
        ...
        public void PublishValue(DisseminationValue value)
        {
            lock (_lock)
            {
                ...
                {
                    _publishedValues[value.Key] = value;
        ...
        public (DisseminationKey Key, long Version, long Payload)[] GetState()
        {
            lock (_lock)
            {
                return _versions
                    ...
                    .Select(entry => (
                        entry.Key,
                        entry.Value,
                        BitConverter.ToInt64(_publishedValues[entry.Key].Payload.Span)))
                    .ToArray();
            }
        }
        ...
        public DisseminationRepairResult CreateRepair(in DisseminationRepairRequest request)
        {
            ...
            lock (_lock)
            {
                if (!_versions.TryGetValue(request.Key, out var version))
                {
                ...
                var value = _publishedValues[request.Key];
                return value.Payload.Length <= request.MaxPayloadBytes && value.Payload.Length <= request.MaxBatchBytes
            ...
            CancellationToken cancellationToken)
        {
            ...
            lock (_lock)
            {
                ...
                _publishedValues[value.Key] = CreateValue(value.Key, version);
    ...
    private sealed class FakeDisseminationService : IDisseminationService
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            Values.Add(repair.Value);
            var result = PublishHandler is null
                ? Results.Count == 0 || Results.Dequeue()
                : await PublishHandler(repair.Value, cancellationToken);
            if (result)
            {
    ...
    private sealed class MonotonicDisseminationHarness(SiloAddress localSilo)
    {
        ...
        public async Task<ModelApplyResponse> Receive(long version, CancellationToken cancellationToken)
        {
            var value = _topic.CreateValue(FakeNamespace.DefaultKey, version);
            var result = await _topic.ApplyValueAsync(value, cancellationToken);
            return new ModelApplyResponse(ToModelResult(result), _topic.GetVersion(FakeNamespace.DefaultKey));
        }

        public Task<ModelRepairResponse> RepairPeer(long peerVersion, CancellationToken cancellationToken)
        {
            cancellationToken.ThrowIfCancellationRequested();
            var localDigest = _topic.Digests.SingleOrDefault();
            if (localDigest.Version == 0)
            {
                return Task.FromResult(new ModelRepairResponse(false, 0));
            }
            ...
            return Task.FromResult(repair.Status is DisseminationRepairStatus.Produced
    ...
    [InlineData(true)]
    [InlineData(false)]
    public async Task PublishRoutesOnlyToNamespaceMembershipScope(bool activeOnly)
    {
        var scope = activeOnly ? DisseminationMembershipScope.ActiveMembers : DisseminationMembershipScope.AllMembers;
        var topology = CreateScopeRoutingTopology();
        var transport = topology.Transport;
        var ns = new FakeNamespace(topology.Local, "scoped-publish", scope);
        var protocol = CreateProtocol(
            transport,
            [ns],
        ...
        Assert.Equal(topology.GetPeers(scope), GetBroadcastPeersForNamespace(transport, ns.Name));
    ...
    [InlineData(true)]
    [InlineData(false)]
    public async Task ReceiveBroadcastForwardsOnlyToNamespaceMembershipScope(bool activeOnly)
    {
        var scope = activeOnly ? DisseminationMembershipScope.ActiveMembers : DisseminationMembershipScope.AllMembers;
        var topology = CreateForwardingScopeTopology();
        var ns = new FakeNamespace(topology.Local, "scoped-forward", scope);
        var protocol = CreateProtocol(
            topology.Transport,
            [ns],
        ...
        await protocol.ReceiveBroadcast(
            new DisseminationBroadcastBatch
            {
                ...
                Values = CreateValueGroups(ns.Name, ns.CreateItem(topology.Sender, "load", sequence: 1)),
            },
            TestContext.Current.CancellationToken);
        await protocol.FlushPendingBroadcast(TestContext.Current.CancellationToken);

        SiloAddress[] expected = scope == DisseminationMembershipScope.ActiveMembers
            ? [topology.LastActive]
            : [topology.LastActive, topology.Joining];
        Assert.Equal(expected, GetBroadcastPeersForNamespace(topology.Transport, ns.Name));
    ...
    [InlineData(true)]
    [InlineData(false)]
    public async Task AntiEntropyRoutesOnlyToNamespaceMembershipScope(bool activeOnly)
    {
        var scope = activeOnly ? DisseminationMembershipScope.ActiveMembers : DisseminationMembershipScope.AllMembers;
        var topology = CreateScopeRoutingTopology();
        var ns = new FakeNamespace(topology.Local, "scoped-anti-entropy", scope);
        ...
        Assert.Equal(topology.GetPeers(scope), GetAntiEntropyPeersForNamespace(topology.Transport, ns.Name));
    ...
    public async Task QueuePrunePreservesPeerStateRequiredByAnotherNamespaceScope()
    {
        ...
        var pending = BeforeBroadcastPumpsRun(() =>
        {
            var loadPublication = PublishValue(protocol, load, load.CreateValue("load", 1), TestContext.Current.CancellationToken);
            ...
            return (loadPublication, membershipPublication, receive);
        });
        Assert.True(await pending.loadPublication);
        Assert.True(await pending.membershipPublication);
    ...
    {
        var (_, peer, transport, ns) = CreatePeerFixture(36001, 36002);
    ...
    {
        var (_, peer, transport, ns) = CreatePeerFixture(36011, 36012);
    ...
    {
        var (_, peer, transport, ns) = CreatePeerFixture(36041, 36042);
        ns.Options.MaxPendingItemCount = 1;
        var queue = CreateBroadcastQueue(transport, [ns]);
        ns.SetValue("first", version: 1);
    ...
    {
        var (_, peer, transport, ns) = CreatePeerFixture(36071, 36072);
        var queue = CreateBroadcastQueue(transport, [ns]);
        ns.SetValue("latest", version: 1);
        Assert.True(BeforeBroadcastPumpsRun(() =>
        {
            var accepted = queue.Notify(peer, ns, "latest");
    ...
    public async Task PeerQueueAdvancesKnownVersionOnlyFromResponseAcknowledgment()
    {
        ...
        {
            baselines.Add(request.FromVersion);
        ...
        var retry = schedules.WaitAsync(
            scheduled => scheduled.LocalSilo.Equals(local) && scheduled.Peer.Equals(peer)
                && scheduled.Reason == DisseminationBroadcastScheduleReason.Retry,
            TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
        ns.Inner.SetValue("ack", 1);

        queue.Notify(peer, ns, "ack");
        await retry;
        Assert.Equal(1, sendCount);
        ns.Inner.SetValue("ack", 2);
        queue.Notify(peer, ns, "ack");
        await queue.FlushPendingBroadcast(TestContext.Current.CancellationToken);
        ns.Inner.SetValue("ack", 3);
        queue.Notify(peer, ns, "ack");
        await queue.FlushPendingBroadcast(TestContext.Current.CancellationToken);

        Assert.Equal(3, sendCount);
        Assert.Equal(new long[] { 1 }, sentVersions[0]);
        Assert.Equal(new long[] { 2 }, sentVersions[1]);
        Assert.Equal(new long[] { 3 }, sentVersions[2]);
        Assert.Equal(new long?[] { null, null, 2 }, baselines);
        await queue.StopAsync(TestContext.Current.CancellationToken);
    }
    ...
    private readonly record struct ScopeRoutingTopology(
        ...
        FakeTransport Transport)
    {
        public SiloAddress[] GetPeers(DisseminationMembershipScope scope) =>
            scope == DisseminationMembershipScope.ActiveMembers
                ? [ActiveOne, ActiveTwo]
                : [ActiveOne, ActiveTwo, Joining, ShuttingDown, Stopping];
    }
    ...
    public async Task QueueAcceptanceDoesNotConfirmPeerNamespaceSupport()
    {
        ...
        var publication = BeforeBroadcastPumpsRun(() =>
        {
            var pending = PublishValue(protocol, ns, ns.CreateValue(FakeNamespace.DefaultKey, 1), TestContext.Current.CancellationToken);
            Assert.Equal([peer], protocol.GetUnconfirmedPeers(ns));
            Assert.Empty(transport.BroadcastBatches);
            return pending;
        });
    ...
    {
        var (_, _, transport, ns) = CreatePeerFixture(31005, 31006);
        var protocol = CreateProtocol(transport, ns);
    ...
    {
        var (_, peer, transport, ns) = CreatePeerFixture(31007, 31008);
        var protocol = CreateProtocol(transport, ns);
    ...
    public async Task GetUnconfirmedPeersReturnsOnlyActiveUnconfirmedPeersForNamespace()
    {
        ...
        transport.PeerStatuses[joining] = SiloStatus.Joining;
        transport.PeerStatuses[dead] = SiloStatus.Dead;
        var ns = new FakeNamespace(local, "active-only", DisseminationMembershipScope.ActiveMembers);
        var protocol = CreateProtocol(transport, [ns]);
        await protocol.ReceiveAntiEntropy(
            new DisseminationAntiEntropyRequest
            {
    ...
    {
        var (_, peer, transport, ns) = CreatePeerFixture(31018, 31019);
        var protocol = CreateProtocol(transport, ns);
    ...
    [Theory]
    [InlineData("partially-confirmed")]
    [InlineData("confirmed")]
    [InlineData("inactive")]
    [InlineData("unconfirmed")]
    public async Task DeploymentLoadPublisherDirectFallbackRequiresUnconfirmedActivePeers(string support)
    {
        var harness = CreateDeploymentLoadPublisherHarness();
        SiloAddress[] unconfirmed = support switch
        {
            "partially-confirmed" => [harness.ActiveTwo],
            "confirmed" => [],
            "inactive" => [harness.Joining],
            _ => [harness.ActiveOne, harness.ActiveTwo],
        };
        harness.Dissemination.UnconfirmedPeers = unconfirmed;

        await harness.PublishStatistics(TestContext.Current.CancellationToken);

        SiloAddress[] expected = support is "confirmed" or "inactive" ? [] : [harness.ActiveOne, harness.ActiveTwo];
        AssertDirectRecipients(harness, expected);
        ...
        Assert.Equal(unconfirmed, harness.Dissemination.UnconfirmedPeers.OrderBy(static peer => peer));
    }
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task DeploymentLoadPublisherFallsBackToAllActivePeersWhenDisseminationFails(bool throws)
    {
        var harness = CreateDeploymentLoadPublisherHarness();
        harness.Dissemination.PublishHandler = (_, _, _, _) => throws
    ...
    public async Task DeploymentLoadPublisherFallsBackToAllActivePeersWhenDisseminationTimesOut()
    {
        ...
        var publish = harness.PublishStatistics(TestContext.Current.CancellationToken);
    ...
    public async Task DeploymentLoadPublisherCallerCancellationStopsDisseminationWithoutFallback()
    {
        ...
        var publish = harness.PublishStatistics(cancellation.Token);
        await harness.Dissemination.PublishStarted.Task.WaitAsync(
            TimeSpan.FromSeconds(5),
            TestContext.Current.CancellationToken);
    ...
    {
        public Task PublishStatistics(CancellationToken cancellationToken) =>
            Publisher.RunOrQueueTask(
                async token =>
                {
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task LoadNotificationAppliesQueuedUpdatesInPublicationOrder(bool interleaveSilos)
    {
        var harness = CreatePhase5DeploymentLoadPublisherHarness();
        (SiloAddress Silo, int Version)[] publications = interleaveSilos
            ? [(harness.ActiveOne, 10), (harness.ActiveTwo, 20), (harness.ActiveOne, 30), (harness.ActiveTwo, 40)]
            : [(harness.ActiveOne, 10), (harness.ActiveOne, 20), (harness.ActiveOne, 30)];
        ...
        var firstUpdate = harness.Publisher.ApplyDisseminatedRuntimeStatisticsAsync(
            harness.Local, CreatePhase5Statistics(1), TestContext.Current.CancellationToken);
        Task<DisseminationApplyResult>[] updates;
        try
        {
            ...
            updates = publications.Select(publication => harness.Publisher.ApplyDisseminatedRuntimeStatisticsAsync(
                publication.Silo, CreatePhase5Statistics(publication.Version), TestContext.Current.CancellationToken)).ToArray();
            Assert.All(updates, static update => Assert.False(update.IsCompleted));
        ...
        Assert.All(await Task.WhenAll(updates.Prepend(firstUpdate)),
            static result => Assert.Equal(DisseminationApplyResult.Applied, result));
        ...
        Assert.Equal(
            [
                ...
                .. publications.Select(static publication =>
                    Phase5Notification.Update(publication.Silo, CreatePhase5Statistics(publication.Version))),
            ],
            notifications);
        if (interleaveSilos)
        {
    ...
    [Fact]
    public async Task LoadNotificationTerminalRemovalRejectsQueuedUpdate()
    {
        var harness = CreatePhase5DeploymentLoadPublisherHarness();
        var notifications = new List<Phase5Notification>();
        var callbackEntered = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        ...
        var firstUpdate = harness.Publisher.ApplyDisseminatedRuntimeStatisticsAsync(
            harness.Local, CreatePhase5Statistics(1), TestContext.Current.CancellationToken);
        Task<DisseminationApplyResult> queuedUpdate;
        Task removal;
        try
        {
            await callbackEntered.Task.WaitAsync(TimeSpan.FromSeconds(10), TestContext.Current.CancellationToken);
            ...
            removal = SetPhase5SiloStatusAsync(
                harness, harness.ActiveOne, SiloStatus.Dead, TestContext.Current.CancellationToken);
        ...
        Assert.Equal(DisseminationApplyResult.Applied, await firstUpdate);
        Assert.Equal(DisseminationApplyResult.Rejected, await queuedUpdate);
        await removal;
        Assert.False(harness.Publisher.PeriodicStatistics.ContainsKey(harness.ActiveOne));
        Assert.Equal(
            [
    ...
    public async Task ConcurrentLoadUpdateCannotOverwriteTerminalRemoval()
    {
        ...
        var concurrentUpdate = harness.Publisher.ApplyDisseminatedRuntimeStatisticsAsync(
        ...
        try
        {
            ...
            termination = SetPhase5SiloStatusAsync(
                harness, harness.ActiveOne, SiloStatus.Dead, TestContext.Current.CancellationToken);
        ...
        Assert.Equal(DisseminationApplyResult.Rejected, await concurrentUpdate);
        await termination;
        Assert.False(harness.Publisher.PeriodicStatistics.ContainsKey(harness.ActiveOne));
        Assert.Equal(
    ...
    public async Task LoadNotificationDoesNotResurrectInactiveSilo(SiloStatus status)
    {
        var harness = CreatePhase5DeploymentLoadPublisherHarness();
        var notifications = new List<Phase5Notification>();
        harness.Publisher.SubscribeToStatisticsChangeEvents(new Phase5StatisticsListener(
        ...
        Assert.Equal(
            ...
            await harness.Publisher.ApplyDisseminatedRuntimeStatisticsAsync(
                harness.ActiveOne, CreatePhase5Statistics(10), TestContext.Current.CancellationToken));
        await SetPhase5SiloStatusAsync(
            harness, harness.ActiveOne, SiloStatus.Dead, TestContext.Current.CancellationToken);
        harness.StatusOracle.SetStatus(harness.ActiveOne, status);

        var futureUpdate = await harness.Publisher.ApplyDisseminatedRuntimeStatisticsAsync(
            harness.ActiveOne,
    ...
    [Fact]
    public async Task LoadNotificationAllowsActiveNewerIncarnation()
    {
        var harness = CreatePhase5DeploymentLoadPublisherHarness();
        var notifications = new List<Phase5Notification>();
        harness.Publisher.SubscribeToStatisticsChangeEvents(new Phase5StatisticsListener(
        ...
        notifications.Clear();
        await SetPhase5SiloStatusAsync(
            harness, terminated, SiloStatus.Dead, TestContext.Current.CancellationToken);
        harness.StatusOracle.SetStatus(restarted, SiloStatus.Active);

        var result = await harness.Publisher.ApplyDisseminatedRuntimeStatisticsAsync(
            restarted,
            CreatePhase5Statistics(20),
            TestContext.Current.CancellationToken);
    ...
    [Fact]
    public async Task LoadNotificationCleansOlderGenerationAfterNewerTermination()
    {
        var harness = CreatePhase5DeploymentLoadPublisherHarness();
        var notifications = new List<Phase5Notification>();
        harness.Publisher.SubscribeToStatisticsChangeEvents(new Phase5StatisticsListener(
        ...
        await SetPhase5SiloStatusAsync(
            harness, newer, SiloStatus.Dead, TestContext.Current.CancellationToken);
        await SetPhase5SiloStatusAsync(
            harness, older, SiloStatus.Dead, TestContext.Current.CancellationToken);
    ...
    [Fact]
    public async Task LoadNotificationCallbacksRunOutsideSubscriberLock()
    {
        var harness = CreatePhase5DeploymentLoadPublisherHarness();
        var notifications = new List<Phase5Notification>();
        var callbackEntered = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        ...
        var update = harness.Publisher.ApplyDisseminatedRuntimeStatisticsAsync(
            harness.Local, CreatePhase5Statistics(1), TestContext.Current.CancellationToken);
        var registrationCompleted = new TaskCompletionSource<(bool Added, bool Removed)>(
            TaskCreationOptions.RunContinuationsAsynchronously);
        Task registration;
        try
        {
            await callbackEntered.Task.WaitAsync(TimeSpan.FromSeconds(10), TestContext.Current.CancellationToken);
            var additionalSubscriber = new Phase5StatisticsListener();
            registration = Task.Run(
                () =>
                {
                    var added = harness.Publisher.SubscribeToStatisticsChangeEvents(additionalSubscriber);
                    var removed = harness.Publisher.UnsubscribeStatisticsChangeEvents(additionalSubscriber);
                    registrationCompleted.TrySetResult((added, removed));
                },
                TestContext.Current.CancellationToken);
            var result = await registrationCompleted.Task.WaitAsync(TimeSpan.FromSeconds(10), TestContext.Current.CancellationToken);
        ...
        await registration;
        Assert.Equal(DisseminationApplyResult.Applied, await update);
        Assert.Equal(
            DisseminationApplyResult.Applied,
            await harness.Publisher.ApplyDisseminatedRuntimeStatisticsAsync(
                harness.ActiveOne, CreatePhase5Statistics(10), TestContext.Current.CancellationToken));
    ...
    [Fact]
    public async Task LoadNotificationsAreSerializedOnPublisherScheduler()
    {
        var harness = CreatePhase5DeploymentLoadPublisherHarness();
        var notifications = new ConcurrentQueue<Phase5Notification>();
        var callbackContexts = new ConcurrentQueue<IGrainContext?>();
        var callbackEntered = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        var releaseCallback = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        ...
        harness.Publisher.SubscribeToStatisticsChangeEvents(new Phase5StatisticsListener(
            onUpdate: (silo, statistics) =>
            {
                ...
                try
                {
                    notifications.Enqueue(Phase5Notification.Update(silo, statistics));
                    callbackContexts.Enqueue(RuntimeContext.Current);
                    if (silo.Equals(harness.Local))
                    {
                        callbackEntered.TrySetResult();
                        releaseCallback.Task.GetAwaiter().GetResult();
        ...
        var firstUpdate = harness.Publisher.ApplyDisseminatedRuntimeStatisticsAsync(
            harness.Local, CreatePhase5Statistics(1), TestContext.Current.CancellationToken);
        Task<DisseminationApplyResult> firstProducer;
        Task<DisseminationApplyResult> secondProducer;
        try
        {
            await callbackEntered.Task.WaitAsync(TimeSpan.FromSeconds(10), TestContext.Current.CancellationToken);
            firstProducer = harness.Publisher.ApplyDisseminatedRuntimeStatisticsAsync(
                harness.ActiveOne, CreatePhase5Statistics(10), TestContext.Current.CancellationToken);
            secondProducer = harness.Publisher.ApplyDisseminatedRuntimeStatisticsAsync(
                harness.ActiveTwo, CreatePhase5Statistics(20), TestContext.Current.CancellationToken);
            Assert.False(firstProducer.IsCompleted);
            Assert.False(secondProducer.IsCompleted);
        ...
        Assert.All(await Task.WhenAll(firstUpdate, firstProducer, secondProducer),
            static result => Assert.Equal(DisseminationApplyResult.Applied, result));
        Assert.Equal(1, maximumCallbackCount);
        Assert.Equal(0, currentCallbackCount);
        Assert.Equal(3, notifications.Count);
        Assert.All(callbackContexts, context => Assert.Same(harness.Publisher, context));
        Assert.Equal(
    ...
    [Fact]
    public async Task LoadNotificationContinuesAfterListenerExceptions()
    {
        var harness = CreatePhase5DeploymentLoadPublisherHarness();
        ...
        var firstUpdate = harness.Publisher.ApplyDisseminatedRuntimeStatisticsAsync(
            harness.Local, CreatePhase5Statistics(1), TestContext.Current.CancellationToken);
        Task<DisseminationApplyResult> failingUpdate;
        Task<DisseminationApplyResult> laterUpdate;
        try
        {
            await callbackEntered.Task.WaitAsync(TimeSpan.FromSeconds(10), TestContext.Current.CancellationToken);
        ...
        Assert.Equal(DisseminationApplyResult.Applied, await firstUpdate);
        var exception = await Assert.ThrowsAsync<InvalidOperationException>(() => failingUpdate);
        Assert.Equal("phase 5 listener failure", exception.Message);
        Assert.Equal(DisseminationApplyResult.Applied, await laterUpdate);
        var expected = new[]
        {
            Phase5Notification.Update(harness.Local, CreatePhase5Statistics(1)),
            Phase5Notification.Update(harness.ActiveOne, CreatePhase5Statistics(10)),
    ...
    private static Task SetPhase5SiloStatusAsync(
        Phase5DeploymentLoadPublisherHarness harness,
        SiloAddress silo,
        SiloStatus status,
        CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        harness.StatusOracle.SetStatus(silo, status);
        harness.Publisher.SiloStatusChangeNotification(silo, status);
        return harness.Publisher.RunOrQueueTask(
            token =>
            {
    ...
    private sealed record Phase5DeploymentLoadPublisherHarness(
        DeploymentLoadPublisher Publisher,
        Phase4FakeSiloStatusOracle StatusOracle,
        SiloAddress Local,
        SiloAddress ActiveOne,
        SiloAddress ActiveTwo);
    ...
    [Theory]
    [InlineData(false, false, 1, 2)]
    [InlineData(false, true, 1, 2)]
    [InlineData(true, false, 1, 3)]
    [InlineData(false, false, 4, 5)]
    [InlineData(false, false, 6, 7)]
    public async Task ReceiveBroadcastRejectsAndDiagnosesApplyFailureWithoutBlockingLaterValues(
        bool unrelatedCancellation, bool forward, long failedVersion, long validVersion)
    {
        var sender = CreateSilo(33001);
        var local = CreateSilo(33002);
        var child = CreateSilo(33003);
        var transport = new FakeTransport(local, forward ? [sender, child] : [sender]);
        var failedKey = new DisseminationKey("throws");
        var validKey = new DisseminationKey("valid");
        using var cancellation = new CancellationTokenSource();
        Exception failure = unrelatedCancellation
            ? new OperationCanceledException("unrelated", cancellation.Token)
            : new InvalidOperationException("phase 6 apply failure");
        var ns = new Phase6ThrowingNamespace(local, failedKey)
        {
            Failure = (_, _) => throw failure,
        };
        ...
        try
        {
            var response = await protocol.ReceiveBroadcast(
                CreateBroadcastBatch(
                    sender,
                    ns.CreateItem(sender, failedKey, failedVersion),
                    ns.CreateItem(sender, validKey, validVersion)),
                TestContext.Current.CancellationToken);
            await protocol.FlushPendingBroadcast(TestContext.Current.CancellationToken);
            ...
            Assert.Equal(0, ns.GetVersion(failedKey));
            Assert.Equal(validVersion, ns.GetVersion(validKey));
            ...
            Assert.Equal(validVersion, acknowledgments[validKey].Version);
            ...
            Assert.Equal(failedVersion, rejection.ToVersion);
            ...
            Assert.Equal(failedVersion, entry.State["Version"]);
            ...
            if (forward)
            {
                ...
                Assert.Equal(validVersion, forwardedValue.Value.ToVersion);
            }
            else
            {
                Assert.Empty(transport.BroadcastBatches);
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task ReceivingStopsBeforeNextValueWhenCallerIsCanceledAfterApply(bool antiEntropy)
    {
        ...
        var laterKey = new DisseminationKey("must-not-apply");
        using var cancellation = new CancellationTokenSource();
        CancellationToken applyCancellation = default;
        var ns = new Phase6ThrowingNamespace(local, cancelingKey)
        {
            Failure = (_, cancellationToken) =>
            {
                applyCancellation = cancellationToken;
                cancellation.Cancel();
                return ValueTask.FromResult(DisseminationApplyResult.Applied);
            },
        };
        ...
        try
        {
            var exception = await Assert.ThrowsAnyAsync<OperationCanceledException>(
                () => antiEntropy
                    ...
                    : protocol.ReceiveBroadcast(new() { Sender = sender, Values = values }, cancellation.Token));
    ...
    {
        var (_, peer, transport, ns) = CreatePeerFixture(39003, 39004);
        ns.Options.MaxPendingItemCount = 1;
        ns.Options.MaxPayloadBytes = sizeof(long);
    ...
    {
        var (_, _, transport, ns) = CreatePeerFixture(39025, 39026);
        ...
        try
        {
            var publication = BeforeBroadcastPumpsRun(() =>
            {
                var pending = PublishValue(protocol, ns, ns.CreateValue("value", 1), TestContext.Current.CancellationToken);
    ...
    {
        var (_, _, transport, ns) = CreatePeerFixture(39005, 39006);
        ns.Options.MaxPendingItemCount = 1;
        var sendStarted = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        var releaseSend = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
    ...
    {
        var (_, peer, transport, ns) = CreatePeerFixture(39011, 39012);
        ns.Options.StaleItemTtl = TimeSpan.FromSeconds(1);
    ...
    [InlineData(2, true)]
    [InlineData(32, false)]
    public async Task TimedOutBroadcastReleasesLocalSlotBeforeRpcCompletes(int maxConcurrentSends, bool lateFault)
    {
        var (local, peer, transport, ns) = CreatePeerFixture(39013, 39014);
        ns.Options.StaleItemTtl = TimeSpan.FromSeconds(1);
        ns.SetValue("value", version: 1);
        var clock = new FakeTimeProvider();
        var response = new TaskCompletionSource<DisseminationBroadcastResponse>(TaskCreationOptions.RunContinuationsAsynchronously);
        var sendStarted = new TaskCompletionSource<CancellationToken>(TaskCreationOptions.RunContinuationsAsynchronously);
        var sendCount = 0;
        var sentVersions = new ConcurrentQueue<long>();
        transport.SendBroadcastResponseHandler = (_, batch, cancellationToken) =>
        {
            var attempt = Interlocked.Increment(ref sendCount);
            sentVersions.Enqueue(Assert.Single(GetBroadcastValues(batch)).Value.ToVersion);
            sendStarted.TrySetResult(cancellationToken);
            return attempt == 1 ? response.Task : Task.FromResult(FakeTransport.CreateAcknowledgment(batch));
        };
        var queue = CreateBroadcastQueue(transport, [ns], options => options.MaxConcurrentSends = maxConcurrentSends, clock);
        ...
        try
        {
            ...
            Assert.Equal(new long[] { 1, 2 }, sentVersions.ToArray());
            Assert.False(response.Task.IsCompleted);

            if (lateFault)
            {
                response.SetException(new InvalidOperationException("The previous attempt failed after its local deadline."));
            }
            else
            {
                response.SetResult(new DisseminationBroadcastResponse
                {
                    Acknowledgments = new() { [ns.Name] = [new("value", 99)] },
                });
            }
            ...
            Assert.Equal(new long[] { 1, 2, 3 }, sentVersions.ToArray());
        }
        finally
        {
            ns.Options.Enabled = false;
    ...
    {
        var (_, _, transport, ns) = CreatePeerFixture(39007, 39008);
        var protocol = CreateProtocol(transport, ns);
        await protocol.StopAsync(TestContext.Current.CancellationToken);
    ...
    {
        var (local, peer, transport, ns) = CreatePeerFixture(39009, 39010);
        ns.SetValue("value", version: 1);
        var queue = CreateBroadcastQueue(transport, [ns], timeProvider: new FakeTimeProvider());
        var callbackEntered = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        var releaseCallback = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        ...
        var notification = Task.Run(
            () => batch
                ? queue.NotifyBatch(peer, ns, [new("value", 1, true)])
                : queue.Notify(peer, ns, "value"),
            TestContext.Current.CancellationToken);
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task BroadcastLocalCompletionReleasesCapacityAcrossPeerRemovalAndRecreation(bool expireBeforePrune)
    {
        var local = CreateSilo(39301);
        var blocked = CreateSilo(39302);
        var healthy = CreateSilo(39303);
        var restarted = SiloAddress.New(blocked.Endpoint, blocked.Generation + 1);
        var transport = new FakeTransport(local, blocked, healthy);
        var ns = new FakeNamespace(local);
        ns.Options.StaleItemTtl = TimeSpan.FromSeconds(1);
        ns.SetValue("value", 1);
        var clock = new FakeTimeProvider();
        var blockedResponse = new TaskCompletionSource<DisseminationBroadcastResponse>(TaskCreationOptions.RunContinuationsAsynchronously);
        var blockedStarted = new TaskCompletionSource<CancellationToken>(TaskCreationOptions.RunContinuationsAsynchronously);
        var sends = new ConcurrentDictionary<SiloAddress, int>();
        var blockedVersions = new ConcurrentQueue<long>();
        transport.SendBroadcastResponseHandler = (peer, batch, cancellation) =>
        {
            var attempt = sends.AddOrUpdate(peer, 1, static (_, count) => count + 1);
            if (peer.Equals(blocked))
            {
                blockedVersions.Enqueue(Assert.Single(GetBroadcastValues(batch)).Value.ToVersion);
                if (attempt == 1)
                {
                    blockedStarted.TrySetResult(cancellation);
                    return blockedResponse.Task;
                }
            }

            return Task.FromResult(FakeTransport.CreateAcknowledgment(batch));
        };
        var queue = CreateBroadcastQueue(transport, [ns], options =>
        {
            options.MaxConcurrentSends = 1;
            options.Overlay.AntiEntropyInterval = TimeSpan.FromHours(1);
        }, clock);
        var membership = new DisseminationMembership(
            transport.MembershipManager, new FakeLocalSiloDetails(local), Options.Create(new DisseminationOptions()));
        using var admission = new SendGateObserver(local, "broadcast");

        try
        {
            Assert.True(queue.Notify(blocked, ns, "value"));
            var remoteCancellation = await blockedStarted.Task.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
            var initialFlush = queue.FlushPendingBroadcast(TestContext.Current.CancellationToken);
            var healthyQueued = admission.Wait(healthy, "queued", TestContext.Current.CancellationToken);
            Assert.True(queue.Notify(healthy, ns, "value"));
            await healthyQueued.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
            var healthyFlush = queue.FlushPendingBroadcast(TestContext.Current.CancellationToken);
            Assert.Equal(1, sends[blocked]);
            Assert.False(sends.ContainsKey(healthy));
            Assert.Equal(1, ns.RepairRequestCount);
            if (expireBeforePrune)
            {
                clock.Advance(ns.Options.StaleItemTtl);
                await Task.WhenAll(initialFlush, healthyFlush).WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
            }

            transport.Peers.Remove(blocked);
            await queue.Prune(membership.CurrentSnapshots, TestContext.Current.CancellationToken);
            await Task.WhenAll(initialFlush, healthyFlush).WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
            Assert.True(remoteCancellation.IsCancellationRequested);
            Assert.False(blockedResponse.Task.IsCompleted);
            Assert.Equal(1, sends[healthy]);

            transport.Peers.Add(blocked);
            Assert.True(queue.Notify(blocked, ns, "value"));
            transport.Peers.Add(restarted);
            Assert.True(queue.Notify(restarted, ns, "value"));
            await queue.FlushPendingBroadcast(TestContext.Current.CancellationToken)
                .WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
            Assert.Equal(2, sends[blocked]);
            Assert.Equal(1, sends[restarted]);
            Assert.Equal(2, transport.GetTargetResolutionCount(blocked));
            Assert.Equal(4, ns.RepairRequestCount);
            Assert.False(blockedResponse.Task.IsCompleted);

            blockedResponse.SetResult(new DisseminationBroadcastResponse
            {
                Acknowledgments = new() { [ns.Name] = [new("value", 99)] },
            });
            ns.SetValue("value", 2);
            ...
            await queue.FlushPendingBroadcast(TestContext.Current.CancellationToken)
                .WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
            Assert.Equal(3, sends[blocked]);
            Assert.Equal(5, sends.Values.Sum());
            Assert.Equal(5, ns.RepairRequestCount);
            Assert.Equal(new long[] { 1, 1, 2 }, blockedVersions.ToArray());
        }
        finally
        {
            ns.Options.Enabled = false;
            blockedResponse.TrySetResult(new DisseminationBroadcastResponse());
            await queue.StopAsync(TestContext.Current.CancellationToken).WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
        }
    }

    [Fact]
    public async Task CancelingFlushObserverPreservesQueuedBroadcastAttempt()
    {
        var local = CreateSilo(39331);
        var first = CreateSilo(39332);
        var waiting = CreateSilo(39333);
        var transport = new FakeTransport(local, first, waiting);
        var ns = new FakeNamespace(local);
        ns.SetValue("value", 1);
        var firstResponse = new TaskCompletionSource<DisseminationBroadcastResponse>(TaskCreationOptions.RunContinuationsAsynchronously);
        var firstStarted = new TaskCompletionSource<CancellationToken>(TaskCreationOptions.RunContinuationsAsynchronously);
        var sent = new ConcurrentQueue<SiloAddress>();
        transport.SendBroadcastResponseHandler = (peer, batch, cancellationToken) =>
        {
            sent.Enqueue(peer);
            if (peer.Equals(first))
            {
        ...
        var queue = CreateBroadcastQueue(transport, [ns], options => options.MaxConcurrentSends = 1, new FakeTimeProvider());
        using var admission = new SendGateObserver(local, "broadcast");
        using var flushCancellation = new CancellationTokenSource();

        try
        {
            Assert.True(queue.Notify(first, ns, "value"));
            var firstFlush = queue.FlushPendingBroadcast(TestContext.Current.CancellationToken);
            var sendCancellation = await firstStarted.Task.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
            var queued = admission.Wait(waiting, "queued", TestContext.Current.CancellationToken);
            Assert.True(queue.Notify(waiting, ns, "value"));
            var canceledFlush = queue.FlushPendingBroadcast(flushCancellation.Token);
            await queued.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
            await flushCancellation.CancelAsync();
            var exception = await Assert.ThrowsAnyAsync<OperationCanceledException>(
                () => canceledFlush.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken));
            Assert.Equal(flushCancellation.Token, exception.CancellationToken);
            Assert.False(sendCancellation.IsCancellationRequested);
            Assert.Equal(new[] { first }, sent.ToArray());
            Assert.Equal(1, ns.RepairRequestCount);

            firstResponse.SetResult(new DisseminationBroadcastResponse
            {
                Acknowledgments = new() { [ns.Name] = [new("value", 1)] },
            });
            await firstFlush.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
            await queue.FlushPendingBroadcast(TestContext.Current.CancellationToken)
                .WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
            Assert.Equal(new[] { first, waiting }, sent.ToArray());
            Assert.Equal(2, ns.RepairRequestCount);
        }
        finally
        {
            ns.Options.Enabled = false;
            firstResponse.TrySetResult(new DisseminationBroadcastResponse());
            await queue.StopAsync(TestContext.Current.CancellationToken).WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
        }
    }

    [Fact]
    public async Task WaitingBroadcastMaterializesLatestValueAfterAdmissionAndRotatesBehindReadyPeer()
    {
        var local = CreateSilo(39311);
        var large = CreateSilo(39312);
        var waiting = CreateSilo(39313);
        var transport = new FakeTransport(local, large, waiting);
        var ns = new FakeNamespace(local);
        ns.SetValue("large-first", 1);
        ns.SetValue("large-second", 2);
        ns.SetValue("large-third", 3);
        ns.SetValue("waiting", 1);
        ...
        var releaseFirst = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        var releaseWaiting = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        var sent = new ConcurrentQueue<(SiloAddress Peer, DisseminationKey Key, long Version)>();
        transport.SendBroadcastResponseHandler = async (peer, batch, cancellation) =>
        {
            var value = Assert.Single(GetBroadcastValues(batch)).Value;
            sent.Enqueue((peer, value.Key, value.ToVersion));
            if (value.Key == new DisseminationKey("large-first"))
            {
                firstStarted.TrySetResult();
                await releaseFirst.Task.WaitAsync(cancellation);
            }
            else if (peer.Equals(waiting))
            {
                waitingStarted.TrySetResult();
                await releaseWaiting.Task.WaitAsync(cancellation);
            }
        ...
        try
        {
            Assert.True(queue.Notify(large, ns, "large-first"));
            await firstStarted.Task.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
            Assert.True(queue.Notify(large, ns, "large-second"));
            Assert.True(queue.Notify(large, ns, "large-third"));
            var waitingQueued = admission.Wait(waiting, "queued", TestContext.Current.CancellationToken);
            Assert.True(queue.Notify(waiting, ns, "waiting"));
            var flush = queue.FlushPendingBroadcast(TestContext.Current.CancellationToken);
            await waitingQueued.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
            Assert.Equal(1, ns.RepairRequestCount);
            Assert.Equal(0, transport.GetTargetResolutionCount(waiting));

            ns.SetValue("waiting", 7);
            var largeQueued = admission.Wait(large, "queued", TestContext.Current.CancellationToken);
            releaseFirst.TrySetResult();
            await waitingStarted.Task.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
            await largeQueued.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
            Assert.Equal(2, ns.RepairRequestCount);
            Assert.Equal(
                new[] { (large, new DisseminationKey("large-first"), 1L), (waiting, new DisseminationKey("waiting"), 7L) },
                sent.ToArray());
            releaseWaiting.TrySetResult();
            await flush.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);

            Assert.Equal(
                new[]
                {
                    (large, new DisseminationKey("large-first"), 1L),
                    (waiting, new DisseminationKey("waiting"), 7L),
                    (large, new DisseminationKey("large-second"), 2L),
                    (large, new DisseminationKey("large-third"), 3L),
                },
                sent.ToArray());
            Assert.Equal(4, ns.RepairRequestCount);
            await queue.FlushPendingBroadcast(TestContext.Current.CancellationToken);
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task RepairAttemptsReleaseLocalCapacityBeforeLateCompletion(bool cancelRound)
    {
        var local = CreateSilo(39321);
        var blocked = Enumerable.Range(39322, 3).Select(CreateSilo).ToArray();
        var healthy = CreateSilo(39325);
        var transport = new FakeTransport(local, blocked);
        var ns = new FakeNamespace(local);
        ns.ExpectedKeys.Add(FakeNamespace.DefaultKey);
        ns.Options.StaleItemTtl = TimeSpan.FromSeconds(1);
        ns.Options.ExpectedUpdateCadence = TimeSpan.FromSeconds(1);
        var clock = new FakeTimeProvider();
        var responses = blocked.ToDictionary(static peer => peer,
            static _ => new TaskCompletionSource<DisseminationAntiEntropyResponse>(TaskCreationOptions.RunContinuationsAsynchronously));
        var remoteCancellations = new ConcurrentDictionary<SiloAddress, CancellationToken>();
        var attempts = new ConcurrentDictionary<SiloAddress, int>();
        var allStarted = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        transport.ExchangeAntiEntropyHandler = (peer, _, cancellation) =>
        {
            var attempt = attempts.AddOrUpdate(peer, 1, static (_, count) => count + 1);
            if (attempt == 1 && responses.TryGetValue(peer, out var response))
            {
                remoteCancellations[peer] = cancellation;
                if (remoteCancellations.Count == blocked.Length)
                {
            ...
            {
                Sender = peer,
                Values = CreateValueGroups(ns.CreateItem(peer, FakeNamespace.DefaultKey, peer.Equals(healthy) ? 3 : 2)),
            });
        };
        var protocol = CreateProtocol(transport, ns, options =>
        {
            options.MaxConcurrentSends = 1;
            options.Overlay.AntiEntropyPeerCount = 3;
            options.Overlay.FanOutFactor = static _ => 8;
        }, clock);
        using var admission = new SendGateObserver(local, "repair");
        using var roundCancellation = CancellationTokenSource.CreateLinkedTokenSource(TestContext.Current.CancellationToken);

        try
        {
            var released = blocked.Select(peer => admission.Wait(peer, "released", TestContext.Current.CancellationToken)).ToArray();
            var firstRound = protocol.RunAntiEntropyRound(roundCancellation.Token);
            await allStarted.Task.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
            ...
            Assert.False(firstRound.IsCompleted);
            Assert.Equal(3, transport.AntiEntropyRequests.Count);
            Assert.All(blocked, peer => Assert.Single(transport.AntiEntropyRequests, request => request.Peer.Equals(peer)));
            Assert.All(remoteCancellations.Values, static cancellation => Assert.False(cancellation.IsCancellationRequested));

            Assert.True(await PublishValue(
                protocol, ns, ns.CreateValue(FakeNamespace.DefaultKey, 1), TestContext.Current.CancellationToken));
            await protocol.FlushPendingBroadcast(TestContext.Current.CancellationToken)
                .WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
            Assert.Equal(blocked.Order(), transport.BroadcastBatches.Select(static batch => batch.Peer).Order());
            Assert.False(firstRound.IsCompleted);
            ...
            if (cancelRound)
            {
                ...
                await Assert.ThrowsAnyAsync<OperationCanceledException>(
                    () => firstRound.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken));
            }
            else
            {
                clock.Advance(ns.Options.StaleItemTtl);
                await firstRound.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
            }

            await Task.WhenAll(released).WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
            Assert.All(remoteCancellations.Values, static cancellation => Assert.True(cancellation.IsCancellationRequested));
            Assert.All(responses.Values, static response => Assert.False(response.Task.IsCompleted));
            AssertFakeState(ns, (FakeNamespace.DefaultKey, 1));

            clock.Advance(ns.Options.StaleItemTtl);
            await protocol.RunAntiEntropyRound(TestContext.Current.CancellationToken)
                .WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
            AssertFakeState(ns, (FakeNamespace.DefaultKey, 2));
            Assert.Equal(6, transport.AntiEntropyRequests.Count);
            Assert.All(blocked, peer => Assert.Equal(2, attempts[peer]));
            Assert.All(responses.Values, static response => Assert.False(response.Task.IsCompleted));

            transport.Peers.Clear();
            transport.Peers.Add(healthy);
            await protocol.RunAntiEntropyRound(TestContext.Current.CancellationToken)
                .WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
            AssertFakeState(ns, (FakeNamespace.DefaultKey, 3));
            Assert.Equal(7, transport.AntiEntropyRequests.Count);
            Assert.Equal(healthy, transport.AntiEntropyRequests[^1].Peer);

            foreach (var (peer, response) in responses)
            {
                response.SetResult(new DisseminationAntiEntropyResponse
                {
                    Sender = peer,
                    Values = CreateValueGroups(ns.CreateItem(peer, FakeNamespace.DefaultKey, 99)),
                });
            }
    ...
    [Fact]
    public async Task AntiEntropyConvergesAfterPartitionWithEveryTreeBroadcastDropped()
    {
        var members = Enumerable.Range(39201, 4).Select(CreateSilo).ToArray();
        DisseminationKey[] keys = ["first", "second", "third", "fourth"];
        await using var network = new RecoveryNetwork(members, silo =>
        {
            var ns = new FakeNamespace(silo);
        ...
        {
            var node = network[members[i]];
            var ns = Assert.IsType<FakeNamespace>(node.Namespace);
            Assert.True(await PublishValue(node.Protocol, ns, ns.CreateValue(keys[i], i + 1), network.Cancellation));
        }

        await network.WaitForBroadcastRetries(members, network.Cancellation);
        Assert.Equal(12, network.Broadcasts.Count);
        Assert.All(network.Broadcasts, static send => Assert.False(send.Delivered));
        await network.RunSweep(network.Cancellation);
        for (var i = 0; i < members.Length; i++)
        {
            AssertFakeState(Assert.IsType<FakeNamespace>(network[members[i]].Namespace), (keys[i], i + 1));
        }

        // Publications have ceased. Only the repair links heal; the tree remains completely disconnected.
        network.Partitioned = false;
        await network.RunSweep(network.Cancellation);
        await network.RunSweep(network.Cancellation);
        var expected = keys.Select((key, index) => (key, (long)index + 1)).ToArray();
        foreach (var member in members)
        {
            AssertFakeState(Assert.IsType<FakeNamespace>(network[member].Namespace), expected);
        }

        var repairCount = network.Replies.Sum(static reply => GetAntiEntropyResponseValues(reply).Count());
        Assert.Equal(24, repairCount);
        await network.RunSweep(network.Cancellation);
        await network.Flush(network.Cancellation);
        Assert.Equal(4, network.Sweeps);
        Assert.Equal(48, network.RequestCount);
        Assert.Equal(repairCount, network.Replies.Sum(static reply => GetAntiEntropyResponseValues(reply).Count()));
        Assert.Equal(12, network.Broadcasts.Count);
    }

    [Fact]
    public async Task LostBroadcastAcknowledgmentsAndReorderedRepairRepliesConvergeAndDrain()
    {
        var members = Enumerable.Range(39211, 3).Select(CreateSilo).ToArray();
        await using var network = new RecoveryNetwork(members, silo =>
        {
            var ns = new FakeNamespace(silo);
            ns.ExpectedKeys.Add(FakeNamespace.DefaultKey);
        ...
        network.DropTreeBroadcasts = false;
        network.LoseAcknowledgments = true;
        var source = network[members[0]];
        var older = network[members[1]];
        var receiver = network[members[2]];
        var sourceNamespace = Assert.IsType<FakeNamespace>(source.Namespace);
        var olderNamespace = Assert.IsType<FakeNamespace>(older.Namespace);
        var receiverNamespace = Assert.IsType<FakeNamespace>(receiver.Namespace);
        Assert.True(await PublishValue(
            source.Protocol, sourceNamespace, sourceNamespace.CreateValue(FakeNamespace.DefaultKey, 1), network.Cancellation));
        await network.WaitForBroadcastRetries([source.Address], network.Cancellation);
        Assert.Equal(2, network.Broadcasts.Count);
        Assert.All(network.Broadcasts, static send => Assert.True(send.Delivered));
        AssertFakeState(receiverNamespace, (FakeNamespace.DefaultKey, 1));
        network.DropTreeBroadcasts = true;

        sourceNamespace.SetValue(FakeNamespace.DefaultKey, 3);
        olderNamespace.SetValue(FakeNamespace.DefaultKey, 2);
        var bothRepliesCaptured = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        var newerReplyReturned = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        ...
        var replies = new ConcurrentDictionary<SiloAddress, DisseminationAntiEntropyResponse>();
        var exchange = receiver.Transport.ExchangeAntiEntropyHandler;
        receiver.Transport.ExchangeAntiEntropyHandler = async (peer, request, cancellation) =>
        {
            var reply = await exchange(peer, request, cancellation);
            replies[peer] = reply;
            if (replies.Count == 2)
            {
                bothRepliesCaptured.TrySetResult();
            }

            await (peer.Equals(source.Address) ? releaseNewer.Task : releaseOlder.Task).WaitAsync(cancellation);
            if (peer.Equals(source.Address))
            {
                newerReplyReturned.TrySetResult();
            }
        ...
        try
        {
            network.Clock.Advance(RecoveryNetwork.RepairCadence);
            var round = receiver.Protocol.RunAntiEntropyRound(network.Cancellation);
            await bothRepliesCaptured.Task.WaitAsync(TimeSpan.FromSeconds(5), network.Cancellation);
            Assert.Equal(3, Assert.Single(GetAntiEntropyResponseValues(replies[source.Address])).Value.ToVersion);
            Assert.Equal(2, Assert.Single(GetAntiEntropyResponseValues(replies[older.Address])).Value.ToVersion);
            releaseNewer.TrySetResult();
            await newerReplyReturned.Task.WaitAsync(TimeSpan.FromSeconds(5), network.Cancellation);
            Assert.False(round.IsCompleted);
            releaseOlder.TrySetResult();
            await round.WaitAsync(TimeSpan.FromSeconds(5), network.Cancellation);

            AssertFakeState(receiverNamespace, (FakeNamespace.DefaultKey, 3));
            Assert.Equal(2, receiverNamespace.ApplyCounts[FakeNamespace.DefaultKey]);
            receiver.Transport.ExchangeAntiEntropyHandler = (peer, _, _) => ValueTask.FromResult(replies[peer]);
            network.Clock.Advance(RecoveryNetwork.RepairCadence);
            await receiver.Protocol.RunAntiEntropyRound(network.Cancellation);
            AssertFakeState(receiverNamespace, (FakeNamespace.DefaultKey, 3));
            Assert.Equal(2, receiverNamespace.ApplyCounts[FakeNamespace.DefaultKey]);

            receiver.Transport.ExchangeAntiEntropyHandler = exchange;
            network.LoseAcknowledgments = false;
            await network.RunSweep(network.Cancellation);
            await network.RunSweep(network.Cancellation);
            await network.Flush(network.Cancellation);
            foreach (var member in members)
            {
                AssertFakeState(Assert.IsType<FakeNamespace>(network[member].Namespace), (FakeNamespace.DefaultKey, 3));
            }

            Assert.Equal(16, network.RequestCount);
            Assert.Equal(2, network.Broadcasts.Count);
            Assert.Equal(2, receiverNamespace.ApplyCounts[FakeNamespace.DefaultKey]);
        }
        finally
    ...
    [Fact]
    public async Task MembershipRepairConvergesAcrossJoinDepartureAndRestartWithoutRevivingDepartedGeneration()
    {
        var first = CreateSilo(39221);
        var second = CreateSilo(39222);
        var departed = CreateSilo(39223);
        var joined = CreateSilo(39224);
        var restarted = SiloAddress.New(departed.Endpoint, departed.Generation + 1);
        SiloAddress[] members = [first, second, departed, joined, restarted];
        var baseline = CreateMembershipSnapshot(1,
            CreateMembershipEntry(first, SiloStatus.Active, DateTime.UnixEpoch),
            CreateMembershipEntry(second, SiloStatus.Active, DateTime.UnixEpoch),
            CreateMembershipEntry(departed, SiloStatus.Active, DateTime.UnixEpoch));
        using var services = new ServiceCollection().AddSerializer().BuildServiceProvider();
        var serializer = services.GetRequiredService<Serializer>();
        var managers = members.ToDictionary(static silo => silo, _ => new FakeMembershipManager(baseline));
        await using var network = new RecoveryNetwork(
            members, silo => CreateMembershipNamespace(managers[silo], serializer));
        network.SetTopology((first, SiloStatus.Active), (second, SiloStatus.Active), (departed, SiloStatus.Active));
        var joiningSnapshot = CreateMembershipSnapshot(2,
            CreateMembershipEntry(first, SiloStatus.Active, DateTime.UnixEpoch),
            CreateMembershipEntry(second, SiloStatus.Active, DateTime.UnixEpoch),
            CreateMembershipEntry(departed, SiloStatus.Active, DateTime.UnixEpoch),
            CreateMembershipEntry(joined, SiloStatus.Joining, DateTime.UnixEpoch.AddSeconds(1)));
        managers[first].CurrentSnapshot = joiningSnapshot;
        network.SetTopology(
            (first, SiloStatus.Active), (second, SiloStatus.Active),
            (departed, SiloStatus.Active), (joined, SiloStatus.Joining));
        await network.RunSweep(network.Cancellation);
        foreach (var member in new[] { first, second, departed, joined })
        {
            AssertMembershipState(joiningSnapshot, managers[member].CurrentSnapshot);
        }

        var delayedDepartedValue = network[departed].Namespace.CreateRepair(new DisseminationRepairRequest(
            DisseminationKey.Default, null, 1024 * 1024, 1024 * 1024)).Value;
        var restartedSnapshot = CreateMembershipSnapshot(3,
            CreateMembershipEntry(first, SiloStatus.Active, DateTime.UnixEpoch),
            CreateMembershipEntry(second, SiloStatus.Active, DateTime.UnixEpoch),
            CreateMembershipEntry(joined, SiloStatus.Active, DateTime.UnixEpoch.AddSeconds(1)),
            CreateMembershipEntry(restarted, SiloStatus.Active, DateTime.UnixEpoch.AddSeconds(2)));
        managers[first].CurrentSnapshot = restartedSnapshot;
        network.SetTopology(
            (first, SiloStatus.Active), (second, SiloStatus.Active),
            (joined, SiloStatus.Active), (restarted, SiloStatus.Active));
        var requestsBeforeRestart = network.Nodes.ToDictionary(static node => node.Address, static node => node.Transport.AntiEntropyRequests.Count);
        await network.RunSweep(network.Cancellation);
        var acknowledgment = await network[second].Protocol.ReceiveBroadcast(new DisseminationBroadcastBatch
        {
            Sender = departed,
            Values = CreateValueGroups(network[second].Namespace.Name, CreateDisseminationValue(departed, delayedDepartedValue)),
        }, network.Cancellation);
        Assert.Equal(3, Assert.Single(acknowledgment.Acknowledgments[network[second].Namespace.Name]).Version);
        await network.RunSweep(network.Cancellation);
        await network.Flush(network.Cancellation);

        foreach (var member in new[] { first, second, joined, restarted })
        {
            AssertMembershipState(restartedSnapshot, managers[member].CurrentSnapshot);
            Assert.DoesNotContain(departed, managers[member].CurrentSnapshot.Entries.Keys);
            Assert.Contains(restarted, managers[member].CurrentSnapshot.Entries.Keys);
        }

        Assert.All(network.Nodes, node => Assert.DoesNotContain(
            node.Transport.AntiEntropyRequests.Skip(requestsBeforeRestart[node.Address]),
            request => request.Peer.Equals(departed)));
        Assert.Equal(3, network.Sweeps);
        Assert.Equal(36, network.RequestCount);
        Assert.Empty(network.Broadcasts);
    }

    [Fact]
    public async Task MembershipAntiEntropyConvergesSameVersionLivenessAndTerminatesDuplicates()
    {
        var members = Enumerable.Range(39231, 3).Select(CreateSilo).ToArray();
        var baseline = CreateMembershipSnapshot(7, members.Select(silo => CreateMembershipEntry(
            silo, SiloStatus.Active, DateTime.UnixEpoch, DateTime.UnixEpoch.AddSeconds(1))).ToArray());
        var advanced = CreateMembershipSnapshot(7, members.Select(silo => CreateMembershipEntry(
            silo, SiloStatus.Active, DateTime.UnixEpoch,
            DateTime.UnixEpoch.AddSeconds(silo.Equals(members[0]) ? 2 : 1))).ToArray());
        using var services = new ServiceCollection().AddSerializer().BuildServiceProvider();
        var serializer = services.GetRequiredService<Serializer>();
        var managers = members.ToDictionary(static silo => silo, _ => new FakeMembershipManager(baseline));
        var applied = members.ToDictionary(static silo => silo, static _ => 0);
        foreach (var member in members)
        {
            managers[member].ProcessGossipSnapshotHandler = (snapshot, _) =>
            {
                applied[member]++;
                managers[member].CurrentSnapshot = snapshot;
                return Task.CompletedTask;
            };
        }

        await using var network = new RecoveryNetwork(members, silo => CreateMembershipNamespace(managers[silo], serializer));
        await network.RunSweep(network.Cancellation);
        Assert.All(network.Replies, static response => Assert.Empty(response.Values));
        var source = network[members[0]];
        var oldFingerprint = source.Namespace.Digests.Single().Fingerprint;
        managers[source.Address].CurrentSnapshot = advanced;
        Assert.NotEqual(oldFingerprint, source.Namespace.Digests.Single().Fingerprint);
        Assert.True(await source.Protocol.Publish(source.Namespace, DisseminationKey.Default, 7, network.Cancellation));
        await network.WaitForBroadcastRetries([source.Address], network.Cancellation);
        await network.RunSweep(network.Cancellation);
        foreach (var member in members)
        {
            AssertMembershipState(advanced, managers[member].CurrentSnapshot);
        }

        var repairCount = network.Replies.Sum(static response => GetAntiEntropyResponseValues(response).Count());
        await network.RunSweep(network.Cancellation);
        var broadcastsBeforeDuplicates = network.Broadcasts.ToArray();
        var duplicate = source.Namespace.CreateRepair(new DisseminationRepairRequest(
            DisseminationKey.Default, null, 1024 * 1024, 1024 * 1024)).Value;
        foreach (var member in members.Skip(1))
        {
            var acknowledgment = await network[member].Protocol.ReceiveBroadcast(new DisseminationBroadcastBatch
            {
                Sender = source.Address,
                Values = CreateValueGroups(source.Namespace.Name, CreateDisseminationValue(source.Address, duplicate)),
            }, network.Cancellation);
            Assert.Equal(7, Assert.Single(acknowledgment.Acknowledgments[source.Namespace.Name]).Version);
            AssertMembershipState(advanced, managers[member].CurrentSnapshot);
            Assert.Equal(1, applied[member]);
        }

        // Flush recipients to isolate duplicate forwarding from the source's intentionally dirty retry queue.
        foreach (var member in members.Skip(1))
        {
            await network[member].Protocol.FlushPendingBroadcast(network.Cancellation);
        }

        Assert.Equal(0, applied[source.Address]);
        Assert.Equal(18, network.RequestCount);
        Assert.Equal(repairCount, network.Replies.Sum(static response => GetAntiEntropyResponseValues(response).Count()));
        Assert.Equal(broadcastsBeforeDuplicates, network.Broadcasts.ToArray());
        Assert.Equal(2, network.Broadcasts.Count);
        Assert.All(network.Broadcasts, static send => Assert.False(send.Delivered));
    }

    [Fact]
    public async Task RejectedQueueKeysRecoverThroughTruncatedDigestsDespiteAdvancingFirstKey()
    {
        var members = Enumerable.Range(39241, 2).Select(CreateSilo).ToArray();
        DisseminationKey[] keys = ["hot", "waiting-first", "waiting-last"];
        await using var network = new RecoveryNetwork(members, silo =>
        {
            var ns = new FakeNamespace(silo);
            ns.ExpectedKeys.UnionWith(keys);
        ...
        var source = network[members[0]];
        var receiver = network[members[1]];
        var sourceNamespace = Assert.IsType<FakeNamespace>(source.Namespace);
        var receiverNamespace = Assert.IsType<FakeNamespace>(receiver.Namespace);
        var firstSendStarted = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        var releaseFirstSend = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        var send = source.Transport.SendBroadcastResponseHandler!;
        source.Transport.SendBroadcastResponseHandler = async (peer, batch, cancellation) =>
        {
            firstSendStarted.TrySetResult();
            await releaseFirstSend.Task.WaitAsync(cancellation);
            return await send(peer, batch, cancellation);
        };

        try
        {
            Assert.True(await PublishValue(source.Protocol, sourceNamespace, sourceNamespace.CreateValue(keys[0], 1), network.Cancellation));
            await firstSendStarted.Task.WaitAsync(TimeSpan.FromSeconds(5), network.Cancellation);
            var flush = source.Protocol.FlushPendingBroadcast(network.Cancellation);
            Assert.False(await PublishValue(source.Protocol, sourceNamespace, sourceNamespace.CreateValue(keys[1], 1), network.Cancellation));
            Assert.False(await PublishValue(source.Protocol, sourceNamespace, sourceNamespace.CreateValue(keys[2], 1), network.Cancellation));
            releaseFirstSend.TrySetResult();
            await flush.WaitAsync(TimeSpan.FromSeconds(5), network.Cancellation);
            AssertFakeState(receiverNamespace);

            for (var round = 1; round <= 4; round++)
            {
                sourceNamespace.SetValue(keys[0], round);
                network.Clock.Advance(RecoveryNetwork.RepairCadence);
                await receiver.Protocol.RunAntiEntropyRound(network.Cancellation);
            }

            Assert.Equal(1, receiverNamespace.GetVersion(keys[1]));
            Assert.Equal(1, receiverNamespace.GetVersion(keys[2]));
            Assert.Equal(keys, receiver.Transport.AntiEntropyRequests[0].Request.Digests[sourceNamespace.Name]
                .Select(static digest => digest.Key));
            Assert.All(receiver.Transport.AntiEntropyRequests[0].Request.Digests[sourceNamespace.Name],
                static digest => Assert.Equal(0, digest.Version));
            Assert.Equal(new[] { keys[0], keys[1], keys[0], keys[2] },
                network.Replies.Select(static response => Assert.Single(GetAntiEntropyResponseValues(response)).Value.Key));
            Assert.All(network.Replies, static response => Assert.True(response.Truncated));
            ...
            await network.Flush(network.Cancellation);
            AssertFakeState(sourceNamespace, (keys[0], 4), (keys[1], 1), (keys[2], 1));
            Assert.Equal(sourceNamespace.GetState(), receiverNamespace.GetState());
            Assert.Equal(6, network.RequestCount);
            Assert.Equal(5, network.Replies.Sum(static response => GetAntiEntropyResponseValues(response).Count()));
            Assert.Empty(network.Replies.Last().Values);
            Assert.Single(network.Broadcasts);
        }
        finally
        {
            releaseFirstSend.TrySetResult();
    ...
    private static void AssertFakeState(FakeNamespace ns, params (DisseminationKey Key, long Version)[] expected) =>
        Assert.Equal(
            expected.OrderBy(static entry => entry.Key.ToString(), StringComparer.Ordinal)
                .Select(static entry => (entry.Key, entry.Version, entry.Version)),
            ns.GetState());

    private static void AssertMembershipState(MembershipTableSnapshot expected, MembershipTableSnapshot actual)
    {
        Assert.Equal(expected.Version, actual.Version);
        Assert.Equal(expected.Entries.Keys.Order(), actual.Entries.Keys.Order());
        foreach (var (silo, entry) in expected.Entries)
        {
            var observed = actual.Entries[silo];
            Assert.Equal(
                (entry.SiloAddress, entry.Status, entry.StartTime, entry.IAmAliveTime, entry.ProxyPort, entry.HostName, entry.SiloName, entry.RoleName),
                (observed.SiloAddress, observed.Status, observed.StartTime, observed.IAmAliveTime, observed.ProxyPort, observed.HostName, observed.SiloName, observed.RoleName));
            Assert.Equal((entry.UpdateZone, entry.FaultZone), (observed.UpdateZone, observed.FaultZone));
            Assert.Equal(entry.SuspectTimes, observed.SuspectTimes);
        }
    }

    // Every delivery enters a real protocol. There is deliberately no legacy transport or direct namespace-copy path.
    private sealed class RecoveryNetwork : IAsyncDisposable
    {
        public static readonly TimeSpan RepairCadence = TimeSpan.FromMilliseconds(1);
        private readonly Dictionary<SiloAddress, RecoveryNode> _nodes = [];
        ...
        private readonly BroadcastScheduleObserver _schedules = new();
        private SiloAddress[] _participants;

        public RecoveryNetwork(
            SiloAddress[] members,
            Func<SiloAddress, IDisseminationNamespace> createNamespace,
            Action<DisseminationOptions>? configure = null)
        {
            _participants = members;
            foreach (var member in members)
            {
                var ns = createNamespace(member);
                ns.Options.ExpectedUpdateCadence = RepairCadence;
                var transport = new FakeTransport(member, members.Where(peer => !peer.Equals(member)).ToArray());
                var protocol = CreateProtocol(transport, [ns], options =>
                {
                    options.MaxConcurrentSends = 1;
                    options.Overlay.FanOutFactor = static _ => 8;
                    options.Overlay.AntiEntropyPeerCount = members.Length;
                    options.Overlay.AntiEntropyInterval = TimeSpan.FromHours(1);
                    configure?.Invoke(options);
                }, Clock);
                var node = new RecoveryNode(member, ns, transport, protocol);
                _nodes.Add(member, node);
                transport.SendBroadcastResponseHandler = async (peer, batch, cancellation) =>
                {
                    var deliver = !DropTreeBroadcasts && !Partitioned;
                    Broadcasts.Enqueue((member, peer, deliver));
                    if (!deliver)
                    {
                        throw new InvalidOperationException("The tree link drops every delivery in this phase.");
                    }

                    var response = await _nodes[peer].Protocol.ReceiveBroadcast(batch, cancellation);
                    return LoseAcknowledgments ? new DisseminationBroadcastResponse() : response;
                };
                transport.ExchangeAntiEntropyHandler = async (peer, request, cancellation) =>
                {
                    if (Partitioned)
                    {
                        throw new InvalidOperationException("The repair link is partitioned.");
                    }

                    var response = await _nodes[peer].Protocol.ReceiveAntiEntropy(request, cancellation);
        ...
        public RecordingFakeTimeProvider Clock { get; } = new();
        public CancellationToken Cancellation => _cancellation.Token;
        public IEnumerable<RecoveryNode> Nodes => _nodes.Values;
        public RecoveryNode this[SiloAddress silo] => _nodes[silo];
        public ConcurrentQueue<(SiloAddress Sender, SiloAddress Peer, bool Delivered)> Broadcasts { get; } = new();
        public ConcurrentQueue<DisseminationAntiEntropyResponse> Replies { get; } = new();
        public bool DropTreeBroadcasts { get; set; } = true;
        public bool LoseAcknowledgments { get; set; }
        public bool Partitioned { get; set; }
        public int Sweeps { get; private set; }
        public int RequestCount => _nodes.Values.Sum(static node => node.Transport.AntiEntropyRequests.Count);

        public void SetTopology(params (SiloAddress Silo, SiloStatus Status)[] members)
        {
            _participants = members.Select(static member => member.Silo).ToArray();
            foreach (var node in _nodes.Values)
            {
                node.Transport.Peers.Clear();
                node.Transport.Peers.AddRange(_participants.Where(peer => !peer.Equals(node.Address)));
                node.Transport.PeerStatuses.Clear();
                node.Transport.PeerStatuses[node.Address] = SiloStatus.Dead;
                foreach (var (silo, status) in members)
                {
                    node.Transport.PeerStatuses[silo] = status;
                }
            }
        }

        public async Task RunSweep(CancellationToken cancellationToken)
        {
            cancellationToken.ThrowIfCancellationRequested();
            // Repair progresses while the fixed 100 ms broadcast retry timer remains dormant.
            Clock.Advance(RepairCadence);
            foreach (var member in _participants)
            {
                await _nodes[member].Protocol.RunAntiEntropyRound(cancellationToken)
        ...
        public async Task WaitForBroadcastRetries(SiloAddress[] publishers, CancellationToken cancellationToken)
        {
            var waits = publishers.SelectMany(publisher =>
                GetOriginatorTreeTargets(publisher, _nodes[publisher].Transport.Peers, fanout: 8)
                    .Select(peer => _schedules.WaitAsync(
                        scheduled => scheduled.LocalSilo.Equals(publisher) && scheduled.Peer.Equals(peer)
                            && scheduled.Reason == DisseminationBroadcastScheduleReason.Retry,
                        TimeSpan.FromSeconds(5), cancellationToken))).ToArray();
            try
            {
            ...
            catch (TimeoutException exception)
            {
                throw new TimeoutException(
                    $"Recovery broadcasts did not enter backoff for {string.Join(", ", publishers.Select(static silo => silo.ToString()))}; "
                    + $"broadcasts={Broadcasts.Count}, repair requests={RequestCount}, sweeps={Sweeps}.", exception);
            }
        }
        ...
        public async Task Flush(CancellationToken cancellationToken)
        {
            ...
            foreach (var node in _nodes.Values)
            {
                await node.Protocol.FlushPendingBroadcast(cancellationToken).WaitAsync(TimeSpan.FromSeconds(5), cancellationToken);
            }
        }
        ...
        public async ValueTask DisposeAsync()
        {
            ...
            var cleanupToken = _cancellation.Token;
            foreach (var node in _nodes.Values)
            {
            ...
            try
            {
                // Teardown aborts accepted retries; scenarios assert graceful drainage before disposal.
            ...
            async Task StopNode(RecoveryNode node)
            {
                try
                {
                    await node.Protocol.StopAsync(cleanupToken);
                }
                catch (OperationCanceledException exception) when (exception.CancellationToken == cleanupToken)
                {
                }
            }
        }
    ...
    private sealed record RecoveryNode(
        SiloAddress Address,
        IDisseminationNamespace Namespace,
        FakeTransport Transport,
        DisseminationProtocol Protocol);

    private sealed class SendGateObserver : IObserver<KeyValuePair<string, object?>>, IDisposable
    {
        private readonly SiloAddress _local;
        private readonly string _kind;
        private readonly object _lock = new();
        private readonly List<(SiloAddress Peer, string Stage)> _events = [];
        private readonly List<(SiloAddress Peer, string Stage, TaskCompletionSource Completion)> _waiters = [];
        private readonly IDisposable _subscription;

        public SendGateObserver(SiloAddress local, string kind)
        {
            _local = local;
            _kind = kind;
            _subscription = DisseminationEvents.Listener.Subscribe(
                this, static name => name == DisseminationEvents.SendGateEventName);
        }

        public Task Wait(SiloAddress peer, string stage, CancellationToken cancellationToken)
        {
            cancellationToken.ThrowIfCancellationRequested();
            lock (_lock)
            {
                var index = _events.FindIndex(entry => entry.Peer.Equals(peer) && entry.Stage == stage);
                if (index >= 0)
                {
                ...
                _waiters.Add((peer, stage, completion));
        ...
        public void OnNext(KeyValuePair<string, object?> value)
        {
            if (value.Value is DisseminationSendGateEvent transition && transition.Kind == _kind)
            {
                OnTransition(transition.LocalSilo, transition.Peer, transition.Stage);
            }
        }

        private void OnTransition(SiloAddress local, SiloAddress peer, string stage)
        {
            if (!local.Equals(_local))
            {
                return;
            }
            ...
            lock (_lock)
            {
                var index = _waiters.FindIndex(waiter => waiter.Peer.Equals(peer) && waiter.Stage == stage);
                if (index >= 0)
                {
                    completion = _waiters[index].Completion;
                    _waiters.RemoveAt(index);
                }
                else
                {
                    _events.Add((peer, stage));
                }
            }
    ...
    private static DisseminationProtocol CreatePhase6Protocol(
        ...
        Microsoft.Extensions.Logging.ILogger<DisseminationProtocol>? logger = null,
        Action<DisseminationOptions>? configure = null) =>
        CreateProtocol(transport, [disseminationNamespace], configure, logger: logger);
