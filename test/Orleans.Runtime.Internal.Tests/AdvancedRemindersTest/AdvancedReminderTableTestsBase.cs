...
public abstract class AdvancedReminderTableTestsBase : IAsyncLifetime, IClassFixture<ConnectionStringFixture>
{
    ...
    protected async Task ReminderSimple()
    {
        ...
        Assert.Equal(DurableJobPriority.Normal, readReminder.Priority);
    ...
    {
        foreach (var batch in Enumerable.Range(1, iterations).Chunk(32))
        {
            await Task.WhenAll(batch.Select(async i =>
            {
        ...
        foreach (var batch in Enumerable.Range(0, iterations).Chunk(32))
        {
            await Task.WhenAll(batch.Select(_ =>
