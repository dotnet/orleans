...
public class UnixSocketTransportListenerTests
{
    ...
    [Fact]
    public async Task BindDoesNotDeleteExistingRegularFile()
    {
        if (!Socket.OSSupportsUnixDomainSockets)
        {
            throw Xunit.Sdk.SkipException.ForSkip("Unix domain sockets are not supported.");
        ...
        var path = Path.Combine(Path.GetTempPath(), $"orleans-{Guid.NewGuid():N}.txt");
        const string contents = "preserve this file";
    ...
    [Fact]
    public async Task BindRemovesStaleUnixSocketFileOnLinux()
    {
        if (!OperatingSystem.IsLinux() || !Socket.OSSupportsUnixDomainSockets)
        {
            throw Xunit.Sdk.SkipException.ForSkip("Linux Unix domain sockets are required.");
        }

        var path = Path.Combine(Path.GetTempPath(), $"orleans-{Guid.NewGuid():N}.sock");
        using (var staleSocket = new Socket(AddressFamily.Unix, SocketType.Stream, ProtocolType.Unspecified))
        {
            staleSocket.Bind(new UnixDomainSocketEndPoint(path));
        }
    ...
    public async Task DisposeAsyncIsIdempotent()
    {
        ...
        var options = new StaticOptionsMonitor<UnixDomainSocketMessageTransportListenerOptions>(
            listenerName,
            new UnixDomainSocketMessageTransportListenerOptions());
        var listener = new UnixDomainSocketMessageTransportListener(listenerName, options, NullLoggerFactory.Instance);
