...
namespace UnitTests.StorageTests.Relational
{
    ...
    internal class CommonStorageTests
    {
        ...
        {
            return Tuple.Create((GrainReference)this.grainFactory.GetGrain(GrainId.GetGrainId(UniqueKey.NewKey(grainId, UniqueKey.Category.Grain))), new GrainState<TestState1> { State = new TestState1(), ETag = version });
        }
        ...
        {
            return Tuple.Create((GrainReference)this.grainFactory.GetGrain(GrainId.FromParsableString(GrainId.GetGrainId(RandomUtilities.NormalGrainTypeCode, grainId).ToParsableString())), new GrainState<TestState1> { State = new TestState1(), ETag = version });
        }
