...
public abstract class RebalancingTestBase<TFixture> : IAsyncLifetime where TFixture : BaseTestClusterFixture, new()
{
    private readonly TFixture _fixture;

    internal IInternalGrainFactory GrainFactory => _fixture.HostedCluster.InternalGrainFactory;
    internal IActivationRebalancerSystemTarget Silo1Rebalancer { get; }
    internal IActivationRebalancerSystemTarget Silo2Rebalancer { get; }
    protected SiloAddress Silo1 { get; }
    protected SiloAddress Silo2 { get; }

    public RebalancingTestBase(TFixture fixture)
    {
        _fixture = fixture;
        ...
        Silo1Rebalancer = IActivationRebalancerSystemTarget.GetReference(GrainFactory, Silo1);
        Silo2Rebalancer = IActivationRebalancerSystemTarget.GetReference(GrainFactory, Silo2);
    }
    ...
    {
        await Silo1Rebalancer.ResetCounters();
        await Silo2Rebalancer.ResetCounters();
    }
    ...
    public async Task AdjustActivationCountOffsets()
    {
        ...
        {
            var sysTarget = GrainFactory.GetSystemTarget<IActivationRebalancerSystemTarget>(Constants.ActivationRebalancerType, silo.SiloAddress);
