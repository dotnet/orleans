...
public class DefaultToleranceTests(DefaultToleranceTests.Fixture fixture)
    : TestBase(fixture), IClassFixture<DefaultToleranceTests.Fixture>
{
    [Fact]
    public async Task A_ShouldMoveToSilo2__B_And_C_ShouldStayOnSilo2()
    {
        ...
        var a = _grainFactory.GetGrain<IA>($"a{scenario}");
        await a.FirstPing(scenario, _silo1, _silo2);
        ...
        var b = _grainFactory.GetGrain<IB>($"b{scenario}");
        var c = _grainFactory.GetGrain<IC>($"c{scenario}");

        var a_host = await a.GetAddress();
        var b_host = await b.GetAddress();
        var c_host = await c.GetAddress();

        Assert.Equal(_silo1, a_host);
        Assert.Equal(_silo2, b_host);
        Assert.Equal(_silo2, c_host);
        ...
        Assert.Equal(_silo2, a_host);  // A is now in silo 2
        Assert.Equal(_silo2, b_host);
        Assert.Equal(_silo2, c_host);
    ...
    public async Task C_ShouldMoveToSilo1__A_And_B_ShouldStayOnSilo1()
    {
        ...
        Assert.Equal(_silo1, a_host);
        Assert.Equal(_silo1, b_host);
        Assert.Equal(_silo1, c_host);  // C is now in silo 1
    ...
    public async Task Immovable_C_ShouldStayOnSilo2__A_And_B_ShouldMoveToSilo2()
    {
        ...
        var c = _grainFactory.GetGrain<ICImmovable>($"c{scenario}");
        ...
        while (a_host == _silo1 || b_host == _silo1);
        ...
        Assert.Equal(_silo2, a_host);  // A is now in silo 2
        Assert.Equal(_silo2, b_host);  // B is now in silo 2
        Assert.Equal(_silo2, c_host);
    ...
    public async Task A_And_B_ShouldMoveToSilo2__C_And_D_ShouldStayOnSilo2_OrTheOtherwayAround()
    {
        ...
        var b = _grainFactory.GetGrain<IB>($"b{scenario}");
        var c = _grainFactory.GetGrain<IC>($"c{scenario}");
        var d = _grainFactory.GetGrain<ID>($"d{scenario}");
        ...
        if (a_host == _silo2 && c_host == _silo2)
        {
            Assert.Equal(_silo2, a_host);  // A is now in silo 2
            Assert.Equal(_silo1, b_host);  
            Assert.Equal(_silo2, c_host);  
        ...
        if (a_host == _silo1 && c_host == _silo1)
        {
            Assert.Equal(_silo1, a_host);  
            Assert.Equal(_silo1, b_host);  
            Assert.Equal(_silo1, c_host);  // C is now in silo 1
            ...
            Assert.Equal(_silo1, a_host);
            Assert.Equal(_silo1, b_host); 
            Assert.Equal(_silo1, c_host);
            Assert.Equal(_silo1, d_host);  // D is now in silo 1
    ...
    {
        var sp1 = _grainFactory.GetGrain<ISP>("s1");
        var sp2 = _grainFactory.GetGrain<ISP>("s2");
        var sp3 = _grainFactory.GetGrain<ISP>("s3");
        ...
        var sr1 = _grainFactory.GetGrain<ISR>("s1");
        var sr2 = _grainFactory.GetGrain<ISR>("s2");
        var sr3 = _grainFactory.GetGrain<ISR>("s3");
        ...
        Assert.Equal(_silo1, sr1_host);
        Assert.Equal(_silo1, sr2_host);
        Assert.Equal(_silo2, sr3_host);
        ...
        while (sr1_host == _silo1 || sr2_host == _silo1);
        ...
        Assert.Equal(_silo2, sr1_host);  // SR1 is now in silo 2, as there is 1 pulling agent (which is moved to silo 2 by the streaming runtime)
        Assert.Equal(_silo2, sr2_host);  // SR2 is now in silo 2, as there is 1 pulling agent (which is moved to silo 2 by the streaming runtime)
        Assert.Equal(_silo2, sr3_host);
