...
public class RedisReminderTableSerializationTests
{
    ...
    [Fact]
    public void ConvertFromEntry_WritesActionAsNumbers()
    {
        ...
        Assert.Equal((int)MissedReminderAction.Notify, segments[9]!.Value<int>());
    }
    ...
    public void ConvertFromEntry_WritesCronTimeZoneAtTailSegment()
    {
        ...
        Assert.Equal(entry.CronTimeZoneId, segments[10]!.Value<string>());
    }
    ...
    public void ConvertFromEntry_RoundTripsDurableJobIdentityFields()
    {
        ...
        Assert.Equal(entry.ScheduleId, segments[11]!.Value<string>());
        Assert.Equal(entry.JobId, segments[12]!.Value<string>());
        Assert.Equal(entry.JobShardId, segments[13]!.Value<string>());
    ...
    [Fact]
    public void ConvertToEntry_ParsesNumericAction()
    {
        var grainId = GrainId.Create("test", "redis-parse-numeric");
        var payload = BuildPayload(grainId, MissedReminderAction.FireImmediately, numericEnums: true);
    ...
    [Fact]
    public void ConvertToEntry_DefaultsActionWhenValuesAreInvalid()
    {
        var grainId = GrainId.Create("test", "redis-default-invalid");
        var payload = BuildPayloadWithCustomEnums(grainId, actionToken: "-3");

        var entry = InvokeConvertToEntry(payload);
        Assert.Equal(MissedReminderAction.Skip, entry.Action);
    }

    private static string BuildPayload(GrainId grainId, MissedReminderAction action, bool numericEnums)
    {
    ...
    private static string BuildPayloadWithCustomEnums(GrainId grainId, object actionToken)
    {
        var startAt = DateTime.UtcNow;
        var nextDueUtc = startAt.AddSeconds(1);
        var lastFireUtc = startAt;
    ...
    {
        var payload = BuildPayload(grainId, MissedReminderAction.Skip, numericEnums: true);
        var segments = ParseSegments(payload);
