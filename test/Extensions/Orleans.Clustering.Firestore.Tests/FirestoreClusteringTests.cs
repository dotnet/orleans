...
public class FirestoreClusteringTests : IAsyncLifetime
{
    ...
    public async Task CleanDeadSiloInstance()
    {
        ...
        await this._membershipTable.CleanupDefunctSiloEntries(TestStartTime.AddTicks(1), TestContext.Current.CancellationToken);

        var membership = await this._membershipTable.ReadAll(TestContext.Current.CancellationToken);
    ...
    public async Task UpdateIAmAliveDoesNotOverwriteNewerHeartbeat()
    {
        ...
        await this._membershipTable.UpdateIAmAlive(entry, TestContext.Current.CancellationToken);

        var row = await this._membershipTable.ReadRow(entry.SiloAddress, TestContext.Current.CancellationToken);
        Assert.Equal(current.UtcDateTime, Assert.Single(row.Members).Item1.IAmAliveTime);

        entry.IAmAliveTime = current.AddMinutes(1).UtcDateTime;
        await this._membershipTable.UpdateIAmAlive(entry, TestContext.Current.CancellationToken);

        row = await this._membershipTable.ReadRow(entry.SiloAddress, TestContext.Current.CancellationToken);
    ...
    {
        var expected = await this._membershipTable.ReadAll(TestContext.Current.CancellationToken);

        var actual = await this._membershipTable.ReadRow(SiloAddressUtils.NewLocalSiloAddress(this._generation + 1), TestContext.Current.CancellationToken);
    ...
    public async Task UpdateRowRefreshesEtagForIdenticalMembership()
    {
        ...
        var table = await this._membershipTable.ReadAll(TestContext.Current.CancellationToken);
        Assert.True(await this._membershipTable.InsertRow(entry, table.Version.Next(), TestContext.Current.CancellationToken));
        var inserted = await this._membershipTable.ReadRow(entry.SiloAddress, TestContext.Current.CancellationToken);
        var insertedEtag = Assert.Single(inserted.Members).Item2;

        Assert.True(await this._membershipTable.UpdateRow(entry, insertedEtag, inserted.Version.Next(), TestContext.Current.CancellationToken));

        var updated = await this._membershipTable.ReadRow(entry.SiloAddress, TestContext.Current.CancellationToken);
    ...
    public async ValueTask InitializeAsync()
    {
        ...
        this._membershipTable = new FirestoreMembershipTable(
            ...
            firestoreOptions,
            clusterOptions);
        await this._membershipTable.InitializeMembershipTable(tryInitTableVersion: true, TestContext.Current.CancellationToken);
