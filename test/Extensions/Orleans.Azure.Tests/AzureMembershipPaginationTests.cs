...
public class AzureMembershipPaginationTests
{
    ...
    [Theory]
    [InlineData(1, "ResourceNotFound")]
    [InlineData(2, "EntityNotFound")]
    [InlineData(3, "ResourceNotFound")]
    public async Task CleanupIndexedAbsentCandidates_PreservesRemainingConditionalDeletes(int disappearances, string code)
    {
        var entries = Enumerable.Range(0, 3).Select(index => DeadSilo($"silo-{index}", $"s{index}")).ToArray();
        var remaining = entries.Select(entry => entry.Entity.RowKey).ToHashSet(StringComparer.Ordinal);
        var storage = new ScriptedMembershipTableReadStorage();
        storage.AddQuery(FencedQuery(7, entries));
        var batches = new List<TableTransactionAction[]>();
        var client = Substitute.For<TableClient>();
        _ = client.SubmitTransactionAsync(Arg.Any<IEnumerable<TableTransactionAction>>(), Arg.Any<CancellationToken>())
            .Returns(call =>
            {
                var batch = call.Arg<IEnumerable<TableTransactionAction>>().ToArray();
                batches.Add(batch);
                Assert.All(batch, action =>
                {
                    Assert.Equal(TableTransactionActionType.Delete, action.ActionType);
                    Assert.Equal(entries.Single(entry => entry.Entity.RowKey == action.Entity.RowKey).ETag, action.ETag.ToString());
                });
                if (batches.Count <= disappearances)
                {
                    var index = batches.Count == 1 ? 1 : 0;
                    Assert.True(remaining.Remove(batch[index].Entity.RowKey));
                    var failure = new RequestFailedException(404, "Selected entity was removed concurrently.", code, null);
                    failure.Data["FailedEntity"] = index.ToString(System.Globalization.CultureInfo.InvariantCulture);
                    var indexed = new TableTransactionFailedException(failure);
                    Assert.Equal(index, indexed.FailedTransactionActionIndex);
                    return Task.FromException<Response<IReadOnlyList<Response>>>(indexed);
                }

                Assert.All(batch, action => Assert.True(remaining.Remove(action.Entity.RowKey)));
                return Task.FromResult(Response.FromValue<IReadOnlyList<Response>>([], Substitute.For<Response>()));
            });

        await CreateManager(storage, client).CleanupDefunctSiloEntries(DateTimeOffset.MaxValue, TestContext.Current.CancellationToken);

        Assert.Empty(remaining);
        Assert.Equal(Math.Min(disappearances + 1, entries.Length), batches.Count);
        Assert.True(batches.Count <= entries.Length);
        Assert.Equal(["silo-0", "silo-1", "silo-2"], batches[0].Select(action => action.Entity.RowKey));
        Assert.Equal(["silo-0", "silo-2"], batches[1].Select(action => action.Entity.RowKey));
        if (disappearances > 1) Assert.Equal(["silo-2"], batches[2].Select(action => action.Entity.RowKey));
        Assert.Equal(1, storage.QueryCount);
    }

    [Theory]
    [InlineData(false, 404, "ResourceNotFound", 0)]
    [InlineData(true, 404, "ResourceNotFound", null)]
    [InlineData(true, 404, "ResourceNotFound", -1)]
    [InlineData(true, 404, "EntityNotFound", 1)]
    [InlineData(true, 404, "TableNotFound", 0)]
    [InlineData(true, 412, "UpdateConditionNotSatisfied", 0)]
    [InlineData(true, 403, "AuthorizationFailure", 0)]
    [InlineData(true, 503, "ServerBusy", 0)]
    public async Task CleanupUnconfirmedAbsenceAndOtherFailures_PropagateWithoutRetry(bool transactionFailure, int status, string code, int? index)
    {
        var storage = new ScriptedMembershipTableReadStorage();
        storage.AddQuery(FencedQuery(7, DeadSilo("silo-1", "s1")));
        var failure = new RequestFailedException(status, "Native failure.", code, null);
        if (index is { } actionIndex)
        {
            failure.Data["FailedEntity"] = actionIndex.ToString(System.Globalization.CultureInfo.InvariantCulture);
        }

        RequestFailedException expected = transactionFailure ? new TableTransactionFailedException(failure) : failure;
        var client = Substitute.For<TableClient>();
        _ = client.SubmitTransactionAsync(Arg.Any<IEnumerable<TableTransactionAction>>(), Arg.Any<CancellationToken>())
            .Returns(Task.FromException<Response<IReadOnlyList<Response>>>(expected));

        var observed = await Record.ExceptionAsync(() => CreateManager(storage, client)
            .CleanupDefunctSiloEntries(DateTimeOffset.MaxValue, TestContext.Current.CancellationToken));
    ...
    [Fact]
    public async Task NativePagedReadAllRetriesUntilMarkersAgree()
    {
        var client = CreateHeartbeatClient();
        client.ReturnsForAll(AsyncPageable<SiloInstanceTableEntry>.FromPages([]));
