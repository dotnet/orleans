...
public class Base_PersistenceGrainTests_GoogleStorage : OrleansTestingBase
{
    ...
    private readonly double _timingFactor;
    ...
    public Base_PersistenceGrainTests_GoogleStorage(ITestOutputHelper output, BaseTestClusterFixture fixture)
    {
        this._output = output;
        this._fixture = fixture;
        HostedCluster = fixture.HostedCluster;
        _timingFactor = TestUtils.CalibrateTimings();
    }

    protected async Task Grain_GoogleStore_Delete()
    {
        Guid id = Guid.NewGuid();
        IGoogleStorageTestGrain grain = this._fixture.GrainFactory.GetGrain<IGoogleStorageTestGrain>(id);
    ...
    protected async Task Grain_GoogleStore_Read()
    {
        Guid id = Guid.NewGuid();
        IGoogleStorageTestGrain grain = this._fixture.GrainFactory.GetGrain<IGoogleStorageTestGrain>(id);
    ...
    protected async Task Grain_GuidKey_GoogleStore_Read_Write()
    {
        Guid id = Guid.NewGuid();
        IGoogleStorageTestGrain grain = this._fixture.GrainFactory.GetGrain<IGoogleStorageTestGrain>(id);
    ...
    protected async Task Grain_LongKey_GoogleStore_Read_Write()
    {
        long id = Random.Shared.Next();
        IGoogleStorageTestGrain_LongKey grain = this._fixture.GrainFactory.GetGrain<IGoogleStorageTestGrain_LongKey>(id);
    ...
    protected async Task Grain_LongKeyExtended_GoogleStore_Read_Write()
    {
        long id = Random.Shared.Next();
        string extKey = Random.Shared.Next().ToString(CultureInfo.InvariantCulture);

        IGoogleStorageTestGrain_LongExtendedKey
            grain = this._fixture.GrainFactory.GetGrain<IGoogleStorageTestGrain_LongExtendedKey>(id, extKey, null);
    ...
    protected async Task Grain_GuidKeyExtended_GoogleStore_Read_Write()
    {
        var id = Guid.NewGuid();
        string extKey = Random.Shared.Next().ToString(CultureInfo.InvariantCulture);

        IGoogleStorageTestGrain_GuidExtendedKey
            grain = this._fixture.GrainFactory.GetGrain<IGoogleStorageTestGrain_GuidExtendedKey>(id, extKey, null);
    ...
    protected async Task Grain_Generic_GoogleStore_Read_Write()
    {
        long id = Random.Shared.Next();

        IGoogleStorageGenericGrain<int> grain = this._fixture.GrainFactory.GetGrain<IGoogleStorageGenericGrain<int>>(id);
    ...
    protected async Task Grain_Generic_GoogleStore_DiffTypes()
    {
        long id1 = Random.Shared.Next();
        long id2 = id1;
        long id3 = id1;

        IGoogleStorageGenericGrain<int> grain1 = this._fixture.GrainFactory.GetGrain<IGoogleStorageGenericGrain<int>>(id1);

        IGoogleStorageGenericGrain<string> grain2 = this._fixture.GrainFactory.GetGrain<IGoogleStorageGenericGrain<string>>(id2);

        IGoogleStorageGenericGrain<double> grain3 = this._fixture.GrainFactory.GetGrain<IGoogleStorageGenericGrain<double>>(id3);
    ...
    protected async Task Grain_GoogleStore_SiloRestart()
    {
        ...
        _output.WriteLine("ClusterId={0} ServiceId={1}", this.HostedCluster.Options.ClusterId, serviceId);
        ...
        grain = this._fixture.GrainFactory.GetGrain<IGoogleStorageTestGrain>(id);
    ...
    protected void Persistence_Perf_Write_Reread()
    {
        ...
        RunPerfTest(n, testName + "--ReRead", target,
            ...
            grainGoogleStore => grainGoogleStore.DoRead());
    }
    ...
    protected void RunPerfTest(int n, string testName, TimeSpan target,
        ...
        Func<IGoogleStorageTestGrain, Task> actionGoogleTable)
    {
        IEchoTaskGrain[] noStateGrains = new IEchoTaskGrain[n];
        IPersistenceTestGrain[] memoryGrains = new IPersistenceTestGrain[n];
        IGoogleStorageTestGrain[] GoogleStoreGrains = new IGoogleStorageTestGrain[n];
        IMemoryStorageTestGrain[] memoryStoreGrains = new IMemoryStorageTestGrain[n];

        for (int i = 0; i < n; i++)
        {
            Guid id = Guid.NewGuid();
            noStateGrains[i] = this._fixture.GrainFactory.GetGrain<IEchoTaskGrain>(id);
            memoryGrains[i] = this._fixture.GrainFactory.GetGrain<IPersistenceTestGrain>(id);
            GoogleStoreGrains[i] = this._fixture.GrainFactory.GetGrain<IGoogleStorageTestGrain>(id);
            memoryStoreGrains[i] = this._fixture.GrainFactory.GetGrain<IMemoryStorageTestGrain>(id);
        }
