...
public sealed class DynamoDBStreamCheckpointStoreTests
{
    ...
    public async Task UpdatePersistsArbitrarySizeSequenceNumberAsString()
    {
        ...
        var state = await store.Update(checkpoint, string.Empty, CancellationToken.None);
    ...
    public async Task ConditionalConflictWithNewerCheckpointDoesNotOverwriteIt()
    {
        ...
        var result = await store.Update("10", string.Empty, CancellationToken.None);
    ...
    public async Task ConditionalConflictWithOlderCheckpointAllowsRetryUsingItsVersion()
    {
        ...
        var conflicted = await store.Update("30", string.Empty, CancellationToken.None);
        Assert.Equal("20", conflicted.Checkpoint);
        Assert.Equal("7", conflicted.Version);
        var updated = await store.Update("30", "7", CancellationToken.None);
    ...
    public async Task ExpectedVersionMismatchReturnsPersistedStateWithoutWriting()
    {
        ...
        var result = await store.Update("30", "6", CancellationToken.None);
