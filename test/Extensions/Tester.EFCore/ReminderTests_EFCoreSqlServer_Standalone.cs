...
public class ReminderTests_EFCoreSqlServer_Standalone
{
    ...
    public ReminderTests_EFCoreSqlServer_Standalone(ITestOutputHelper output, TestEnvironmentFixture fixture)
    {
        ...
        _log = _loggerFactory.CreateLogger<ReminderTests_EFCoreSqlServer_Standalone>();
    ...
    private EFReminderTable<SqlServerReminderDbContext, byte[]> GetReminderTable(string clusterId)
    {
        var cs = "Server=localhost,1433;Database=OrleansTests.Reminders;User Id=sa;Password=yourStrong(!)Password;TrustServerCertificate=True";
        var sp = new ServiceCollection()
            .AddPooledDbContextFactory<SqlServerReminderDbContext>(optionsBuilder =>
            {
        ...
        var factory = sp.GetRequiredService<IDbContextFactory<SqlServerReminderDbContext>>();
        ...
        return new EFReminderTable<SqlServerReminderDbContext, byte[]>(this._loggerFactory, clusterOptions, factory, new SqlServerReminderETagConverter());
    }
