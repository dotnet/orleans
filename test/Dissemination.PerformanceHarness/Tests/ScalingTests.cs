...
public sealed class ScalingTests
{
    ...
    {
        Assert.Equal("true", Environment.GetEnvironmentVariable("ORLEANS_DISSEMINATION_MANUAL_RUN"));
        var sizes = (Environment.GetEnvironmentVariable("ORLEANS_DISSEMINATION_SIZES") ?? "4,8")
            .Split(',').Select(value => int.Parse(value, System.Globalization.CultureInfo.InvariantCulture)).ToArray();
        var iterations = Setting("ITERATIONS", 3, 3, 200);
        var repetitions = Setting("REPETITIONS", 2, 1, 5);
        ...
        foreach (var size in sizes)
        {
            foreach (var scenario in scenarios)
            {
                ...
                {
                    // Reverse the whole sequence so every off/on pair changes order.
    ...
    private static async Task<object> Measure(int size, string scenario, RuntimeVariant variant, int iterations, int repetition, int order)
    {
        ...
        var record = new
        {
            ...
            Methodology = new
            {
                ...
                Observation = "identical state polling on selected paths; detailed counters only at window boundaries",
    ...
    internal static int ExecutionIndex(int repetition, int order, int count) =>
        repetition % 2 == 0 ? order : count - 1 - order;
