...
internal sealed class ProcessCluster : IAsyncDisposable
{
    ...
    public ProcessCluster(string scenario, int siloProcessorCount = 0, int gcConserveMemory = 0)
    {
        var artifactRoot = Environment.GetEnvironmentVariable("ORLEANS_DISSEMINATION_RESULTS")
            ?? throw new InvalidOperationException("Run .github/scripts/dissemination-performance.ps1 first.");
    ...
    public async Task<SiloProcess> Start(string runtime, bool enabled, int? port = null)
    {
        ...
        {
            Assert.Equal(Environment.GetEnvironmentVariable("ORLEANS_DISSEMINATION_CANDIDATE_SHA"), manifest.SourceRevision);
        ...
        var node = new SiloProcess(
            ...
            new(name, Path.GetFileName(Directory), _membershipDirectory, port ?? AvailablePort(), Environment.ProcessId,
                enabled, SiloProcessorCount, GCConserveMemory),
            Directory,
            manifest);
        _all.Add(node);
        await node.Start();
