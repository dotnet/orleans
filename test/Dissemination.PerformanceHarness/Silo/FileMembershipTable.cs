...
// A shared JSON schema and exclusive file lease coordinate the loopback silo processes.
internal sealed class FileMembershipTable(NodeConfiguration configuration) : IMembershipTable
{
    ...
    {
        cancellationToken.ThrowIfCancellationRequested();
        var state = await Access(static state => state, write: false, cancellationToken);
