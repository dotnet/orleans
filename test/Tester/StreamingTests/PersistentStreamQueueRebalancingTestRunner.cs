...
namespace Tester.StreamingTests
{
    ...
    public abstract class PersistentStreamQueueRebalancingTestRunner
    {
        ...
        public async Task RebalanceQueuesWhileStreamingTest()
        {
            ...
            await producer.BecomeProducer(streamGuid, streamGuid.ToString(), this.streamProviderName);

            var consumer = this.grainFactory.GetGrain<ISampleStreaming_ConsumerGrain>(Guid.NewGuid());
            await consumer.BecomeConsumer(streamGuid, streamGuid.ToString(), this.streamProviderName);
            ...
            await consumer2.BecomeConsumer(streamGuid, streamGuid.ToString(), this.streamProviderName);
