...
namespace UnitTests.StreamingTests
{
    public class PersistentStreamPullingAgentTests
    {
            ...
            bool isRewindable = false)
        {
            ...
            queueAdapter.IsRewindable.Returns(isRewindable);
        ...
        private sealed class RecordingSimpleQueueCache()
            : SimpleQueueCache(256, NullLogger.Instance), IQueueCache
        {
        ...
        public async Task Checkpoint_DrainedSubscriptionsDoNotPinOtherStreams(bool pooled, int batchSize, bool filtered)
        {
            await using var scenario = await CreateCheckpointScenario(pooled, batchSize, filtered);
            var batches = Enumerable.Range(1, 200)
                .Select(sequence => (IBatchContainer)new TestBatchContainer(
                    sequence == 1 ? scenario.Idle.StreamId.StreamId : scenario.Busy.StreamId.StreamId,
            ...
            if (filtered)
            {
                Assert.Empty(((ImmediateRecordingConsumer)scenario.Idle.StreamConsumer).DeliveredTokens);
                Assert.Empty(((ImmediateRecordingConsumer)scenario.Busy.StreamConsumer).DeliveredTokens);
            }
        ...
        [InlineData("unregistered-producer", 1L)]
        [InlineData("unregistered-consumer", null)]
        [InlineData("unknown-progress", null)]
        public async Task Checkpoint_UnprovenSubscriptionsUseOriginalProgress(string condition, long? expected)
        {
            ...
            switch (condition)
            {
                case "active":
                    ...
                    break;
                case "pending-batch":
                    scenario.Idle.PendingBatch = new TestBatchContainer(scenario.Idle.StreamId.StreamId, new EventSequenceTokenV2(1));
                    break;
                case "handshake":
                    scenario.Idle.PendingHandshakes = 1;
                    break;
                case "disposed":
                ...
                case "unregistered-producer":
                    streams[scenario.Idle.StreamId].StreamRegistered = false;
                    break;
                case "unregistered-consumer":
                    scenario.Idle.IsRegistered = false;
                    break;
                case "unknown-progress":
                    scenario.Idle.LastProcessedToken = null;
                    break;
                default:
            ...
            await scenario.Accessor.Shutdown();
            if (expected.HasValue)
            {
                Assert.Equal(expected.Value, Assert.Single(scenario.Checkpoints)?.SequenceNumber);
            }
            else
            {
            ...
            string deliveryTiming, long expectedCheckpoint)
        {
            ...
            var handshakeStarted = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
            var releaseHandshake = new TaskCompletionSource<StreamHandshakeToken?>(TaskCreationOptions.RunContinuationsAsynchronously);
            var releaseNewDelivery = new TaskCompletionSource<StreamHandshakeToken?>(TaskCreationOptions.RunContinuationsAsynchronously);
            ...
            var consumer = new RecordingConsumer
            {
                ...
                OnDelivery = batch =>
                {
                    if (batch.SequenceToken.SequenceNumber == 200)
                    {
                        return releaseNewDelivery.Task;
                    }

                    Assert.Equal(100, batch.SequenceToken.SequenceNumber);
            ...
            try
            {
                ...
                if (deliveryTiming != "none")
                {
                    if (deliveryTiming == "completed")
                    {
                    ...
                    await consumer.Delivered.Task.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
                    Assert.Equal(
                        deliveryTiming == "overlapping" ? StreamConsumerDataState.Active : StreamConsumerDataState.Inactive,
                        scenario.Busy.State);
                    Assert.Equal(1, scenario.Busy.PendingHandshakes);
                ...
                Assert.Equal(
                    ...
                    consumer.DeliveredTokens.Select(token => token.SequenceNumber));
                Assert.Empty(consumer.Errors);
        ...
        [InlineData(false, "drained")]
        [InlineData(true, "pending")]
        [InlineData(true, "replaying")]
        [InlineData(true, "drained")]
        public async Task Checkpoint_IdleCleanupPreservesHandshakeUntilCompletion(bool legacyMode, string outcome)
        {
            var timeProvider = new FakeTimeProvider(DateTimeOffset.UtcNow);
            await using var scenario = await CreateCheckpointScenario(timeProvider: timeProvider);
            await scenario.Read((scenario.Idle, 50), (scenario.Idle, 100), (scenario.Busy, 200));
            // Isolate subscription cleanup from the cache's independent eviction policy.
            using var replayPin = scenario.Cache.GetCacheCursor(scenario.Idle.StreamId, new EventSequenceTokenV2(50));
            if (legacyMode)
            {
                scenario.Idle.SafeDisposeCursor(NullLogger.Instance);
                scenario.Idle.Cursor = new InvalidMoveCursor();
                await Assert.ThrowsAnyAsync<InvalidOperationException>(
                    () => scenario.Accessor.RunConsumerCursor(scenario.Idle));
                scenario.Idle.Cursor = scenario.Cache.GetCacheCursor(scenario.Idle.StreamId, new EventSequenceTokenV2(100));
            }
            ...
            var handshakeToken = StreamHandshakeToken.CreateDeliveyToken(
                new EventSequenceTokenV2(outcome == "drained" ? 100 : 50));
            var attachment = scenario.Accessor.AddSubscriber(scenario.Idle);

            try
            {
                await handshakeStarted.Task.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
                Assert.Equal(StreamConsumerDataState.Inactive, scenario.Idle.State);
                Assert.Equal(1, scenario.Idle.PendingHandshakes);
                ...
                Assert.Equal(1, scenario.Idle.PendingHandshakes);
                Assert.False(attachment.IsCompleted);
                Assert.Equal(201, scenario.Busy.LastProcessedToken?.SequenceNumber);

                if (outcome != "pending")
                {
                    releaseHandshake.TrySetResult(handshakeToken);
                    await attachment.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
                    Assert.Equal(0, scenario.Idle.PendingHandshakes);
                    Assert.Empty(consumer.Errors);
                    if (outcome == "replaying")
                    {
                        await consumer.Delivered.Task.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
                        Assert.Equal(50, scenario.Idle.LastProcessedToken?.SequenceNumber);
                    }
                    else
                    {
                        Assert.Equal(100, scenario.Idle.LastProcessedToken?.SequenceNumber);
                        ...
                        streams = await scenario.Accessor.GetPubSubCache();
                        Assert.Equal(scenario.Busy.StreamId, Assert.Single(streams).Key);
                        Assert.Empty(idleStream.AllConsumers());
                        Assert.Null(scenario.Idle.Cursor);
                    }
                }
                ...
                await shutdown.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
                Assert.Equal(
                    outcome == "drained" ? 202 : outcome == "replaying" ? 100 : 50,
                    Assert.Single(scenario.Checkpoints)?.SequenceNumber);
            }
            finally
            {
                releaseHandshake.TrySetResult(handshakeToken);
        ...
        [InlineData(false)]
        [InlineData(true)]
        public async Task Checkpoint_NewReadsMustBeAcknowledgedAfterDraining(bool pooled)
        {
        ...
        [InlineData(false)]
        [InlineData(true)]
        public async Task Checkpoint_EmptyCursorCannotHideAnUnacknowledgedRead(bool pooled)
        {
            await using var scenario = await CreateCheckpointScenario(pooled);
            await scenario.Read((scenario.Idle, 1), (scenario.Busy, 2));
            scenario.Idle.State = StreamConsumerDataState.Active;
            ...
            scenario.Idle.SafeDisposeCursor(NullLogger.Instance);
            if (scenario.Cache is PurgeablePooledQueueCache pooledCache)
            {
                pooledCache.Purge();
                scenario.Idle.Cursor = pooledCache.GetCacheCursor(scenario.Idle.StreamId, scenario.Idle.LastProcessedToken);
            }
            else
            {
                scenario.Idle.Cursor = new EmptyQueueCacheCursor();
            }
        ...
        [InlineData(true, 0)]
        [InlineData(true, 1)]
        public async Task Checkpoint_InitialResumeCanEstablishCaughtUpProgress(bool pooled, long acknowledged)
        {
            await using var scenario = await CreateCheckpointScenario(pooled);
            scenario.Idle.IsRegistered = false;
            var consumer = new RecordingConsumer(StreamHandshakeToken.CreateDeliveyToken(new EventSequenceTokenV2(acknowledged)));
            consumer.ReleaseDelivery();
            scenario.Idle.StreamConsumer = consumer;
            await scenario.Read((scenario.Idle, 1), (scenario.Busy, 200));

            Assert.True(await scenario.Accessor.DoHandshakeWithConsumer(scenario.Idle, new EventSequenceTokenV2(1)));
            scenario.Idle.LastProcessedToken = PersistentStreamPullingAgent.GetInitialDeliveryProgress(scenario.Idle.LastToken);
            scenario.Idle.IsRegistered = true;
            scenario.Idle.PendingStartToken = null;
            await scenario.Accessor.RunConsumerCursor(scenario.Idle);

            Assert.Equal(1, scenario.Idle.LastProcessedToken?.SequenceNumber);
            Assert.Equal(acknowledged == 0 ? 1 : 0, consumer.DeliveredTokens.Count);
            await scenario.AssertCheckpoint(200);
        }
        ...
        [InlineData(false)]
        [InlineData(true)]
        public async Task Checkpoint_CursorFailureDoesNotFreezeLaterOriginalProgress(bool failCursor)
        {
            await using var scenario = await CreateCheckpointScenario();
            await scenario.Read((scenario.Idle, 1), (scenario.Busy, 2));
            scenario.Idle.SafeDisposeCursor(NullLogger.Instance);
            ...
            await scenario.Read((scenario.Idle, 3), (scenario.Busy, 200));
            scenario.Idle.Cursor = scenario.Cache.GetCacheCursor(scenario.Idle.StreamId, new EventSequenceTokenV2(3));
            await scenario.Accessor.RunConsumerCursor(scenario.Idle);

            Assert.Equal(3, scenario.Idle.LastProcessedToken?.SequenceNumber);
            Assert.True(scenario.Idle.IsCaughtUp);
            await scenario.AssertCheckpoint(failCursor ? 3 : 200);
        }
        ...
        public async Task Checkpoint_InFlightDeliveryConstrainsProgressUntilAcknowledged(bool acknowledge)
        {
            ...
            var consumer = new RecordingConsumer();
            scenario.Busy.StreamConsumer = consumer;
            scenario.Busy.State = StreamConsumerDataState.Active;
            await scenario.Read((scenario.Busy, 200));
            scenario.Busy.State = StreamConsumerDataState.Inactive;
            var delivery = scenario.Accessor.RunConsumerCursor(scenario.Busy);
        ...
        [InlineData("pending-registration")]
        public async Task Checkpoint_UncertainRemovalUsesRemainingOriginalProgress(string condition)
        {
            await using var scenario = await CreateCheckpointScenario();
            await scenario.Read((scenario.Idle, 1), (scenario.Busy, 2));
            scenario.Idle.State = StreamConsumerDataState.Active;
            await scenario.Read((scenario.Idle, 200));

            if (condition == "incompatible")
            {
                scenario.Idle.State = StreamConsumerDataState.Inactive;
                scenario.Idle.LastProcessedToken = new IsolatedProviderToken(200);
            }
            else if (condition == "pending-registration")
            {
                var streams = await scenario.Accessor.GetPubSubCache();
                streams[scenario.Idle.StreamId].RegistrationTask = new TaskCompletionSource().Task;
            }
        ...
        [InlineData(false)]
        [InlineData(true)]
        public async Task Checkpoint_NoSubscriptionsDoNotManufactureProgress(bool previouslySubscribed)
        {
            await using var scenario = await CreateCheckpointScenario();
            if (!previouslySubscribed)
            {
                var streams = await scenario.Accessor.GetPubSubCache();
                streams[scenario.Idle.StreamId].RemoveConsumer(scenario.Idle.SubscriptionId, NullLogger.Instance);
                streams[scenario.Busy.StreamId].RemoveConsumer(scenario.Busy.SubscriptionId, NullLogger.Instance);
            }
            ...
            {
                await scenario.Remove(scenario.Idle);
                await scenario.Remove(scenario.Busy);
            }

            scenario.Checkpoints.Clear();
            await scenario.Accessor.Shutdown();
            Assert.Null(Assert.Single(scenario.Checkpoints));
        }

        [Theory, TestCategory("BVT"), TestCategory("Streaming")]
        [InlineData("ordered", 300L)]
        [InlineData("unordered", 1L)]
        [InlineData("unordered-batch", 1L)]
        [InlineData("incompatible", 1L)]
        [InlineData("missing-token", 1L)]
        [InlineData("null-batch", 1L)]
        public async Task Checkpoint_UncertainReadUsesOriginalProgress(string condition, long expected)
        {
            await using var scenario = await CreateCheckpointScenario(emptyCache: new RecordingQueueCache());
            scenario.Idle.LastProcessedToken = new EventSequenceTokenV2(1);
            scenario.Busy.LastProcessedToken = new EventSequenceTokenV2(200);
            await scenario.Read((scenario.Idle, 1), (scenario.Busy, 200));

            var other = new QualifiedStreamId("provider", StreamId.Create("other", Guid.NewGuid()));
            await scenario.Accessor.RegisterStream(other, new EventSequenceTokenV2(201), DateTime.UtcNow);
            StreamSequenceToken? token = condition switch
            {
                "unordered" => new EventSequenceTokenV2(100),
                "incompatible" => new IsolatedProviderToken(300),
                "missing-token" => null,
                _ => new EventSequenceTokenV2(300),
            };
            IList<IBatchContainer> batches = condition == "null-batch"
                ? [null!]
                : [new TestBatchContainer(other.StreamId, token!)];
            if (condition == "unordered-batch")
            {
                batches.Add(new TestBatchContainer(other.StreamId, new EventSequenceTokenV2(250)));
            }

            await scenario.Read(batches);
            await scenario.AssertCheckpoint(expected);
        }

        [Fact, TestCategory("BVT"), TestCategory("Streaming")]
        public async Task Checkpoint_IncompatibleReadBoundaryStillAllowsOriginalSubscriberToken()
        {
            await using var scenario = await CreateCheckpointScenario();
            await scenario.Read((scenario.Idle, 1), (scenario.Busy, 200));
            await scenario.Remove(scenario.Busy);
            scenario.Idle.LastProcessedToken = new IsolatedProviderToken(10);

            await scenario.AssertCheckpoint(10);
        ...
        [InlineData(false)]
        [InlineData(true)]
        public async Task Checkpoint_ReinitializeClearsReadBoundaryAndOptimizationOptOut(bool readAgain)
        {
            await using var scenario = await CreateCheckpointScenario(emptyCache: new RecordingQueueCache());
            scenario.Idle.LastProcessedToken = new EventSequenceTokenV2(1);
            scenario.Busy.LastProcessedToken = new EventSequenceTokenV2(200);
            ...
            await Assert.ThrowsAnyAsync<InvalidOperationException>(
                () => scenario.Accessor.RunConsumerCursor(scenario.Idle));
            await scenario.Accessor.Shutdown();
            await InitializeAgent((PersistentStreamPullingAgent)scenario.Accessor);

            await scenario.Accessor.RegisterStream(scenario.Idle.StreamId, new EventSequenceTokenV2(1), DateTime.UtcNow);
            await scenario.Accessor.RegisterStream(scenario.Busy.StreamId, new EventSequenceTokenV2(200), DateTime.UtcNow);
            var streams = await scenario.Accessor.GetPubSubCache();
            var restarted = scenario with
            {
                Idle = AddCheckpointConsumer(streams[scenario.Idle.StreamId], scenario.Idle.StreamId, scenario.Cache),
                Busy = AddCheckpointConsumer(streams[scenario.Busy.StreamId], scenario.Busy.StreamId, scenario.Cache),
            };
            restarted.Idle.LastProcessedToken = streams[scenario.Idle.StreamId].LastReadToken = new EventSequenceTokenV2(1);
            restarted.Busy.LastProcessedToken = streams[scenario.Busy.StreamId].LastReadToken = new EventSequenceTokenV2(200);
            restarted.Idle.IsCaughtUp = restarted.Busy.IsCaughtUp = true;

            if (readAgain)
            {
                await restarted.Read((restarted.Idle, 1), (restarted.Busy, 200));
            }

            await restarted.AssertCheckpoint(readAgain ? 200 : 1);
        }
            ...
            TimeProvider? timeProvider = null)
        {
            ...
            if (emptyCache is not null)
            {
                cache = emptyCache;
                checkpoints = emptyCache.DeliveryProgressTokens;
            }
            else if (pooled)
            {
            ...
            {
                var simpleCache = new RecordingSimpleQueueCache();
                cache = simpleCache;
                checkpoints = simpleCache.DeliveryProgressTokens;
            }
            ...
            return new(accessor, cache, checkpoints,
                AddCheckpointConsumer(idleStream, idleId, cache), AddCheckpointConsumer(busyStream, busyId, cache));
        }

        private static StreamConsumerData AddCheckpointConsumer(
        ...
        private sealed record CheckpointScenario(
            PersistentStreamPullingAgent.ITestAccessor Accessor,
            IQueueCache Cache,
            List<StreamSequenceToken?> Checkpoints,
            StreamConsumerData Idle,
            StreamConsumerData Busy) : IAsyncDisposable
        {
            public Task Read(params (StreamConsumerData Consumer, long Sequence)[] messages)
                => Read(messages.Select(message => (IBatchContainer)new TestBatchContainer(
                    message.Consumer.StreamId.StreamId, new EventSequenceTokenV2(message.Sequence))).ToList());

            public async Task Read(IList<IBatchContainer> messages)
            {
                var receiver = Substitute.For<IQueueAdapterReceiver>();
                receiver.GetQueueMessagesAsync(Arg.Any<int>(), Arg.Any<CancellationToken>())
                    .Returns(Task.FromResult(messages));
                await Accessor.ReadFromQueue(QueueId.GetQueueId("queue", 0u, 0u), receiver, 1000);
            }

            public Task Remove(StreamConsumerData consumer)
            {
                var agent = (PersistentStreamPullingAgent)Accessor;
                return agent.RunOrQueueTask(() => agent.RemoveSubscriber(
                    consumer.SubscriptionId, consumer.StreamId, TestContext.Current.CancellationToken));
            }

            public async Task AssertCheckpoint(long expected)
            {
                Checkpoints.Clear();
                await Accessor.Shutdown();
                Assert.Equal(expected, Assert.Single(Checkpoints)?.SequenceNumber);
            }
        ...
        [InlineData("faulted")]
        [InlineData("canceled")]
        public async Task Shutdown_DrainsDetachedDeliveryBeforeCheckpointAndReceiverCleanup(string outcome)
        {
            var streamId = new QualifiedStreamId("provider", StreamId.Create("namespace", Guid.NewGuid()));
            var token = new EventSequenceTokenV2(1);
            var cache = new ShutdownQueueCache();
            var receiver = Substitute.For<IQueueAdapterReceiver>();
            receiver.GetQueueMessagesAsync(Arg.Any<int>(), Arg.Any<CancellationToken>())
                .Returns(Task.FromResult<IList<IBatchContainer>>([new TestBatchContainer(streamId.StreamId, token)]));
            receiver.Shutdown(Arg.Any<TimeSpan>(), Arg.Any<CancellationToken>()).Returns(_ => cache.StopReceiver());
            var (accessor, _, stream) = await CreateInitializedAgentWithStream(
                streamId, token, cache, new StreamPullingAgentOptions { BatchContainerBatchSize = 2 }, receiver: receiver);
            var release = new TaskCompletionSource<StreamHandshakeToken?>(TaskCreationOptions.RunContinuationsAsynchronously);
            var consumer = new RecordingConsumer { OnDelivery = _ => release.Task };
            ...
            data.Cursor = cache.GetCacheCursor(streamId.StreamId, null);

            try
            {
                Assert.True(await accessor.ReadFromQueue(QueueId.GetQueueId("queue", 0u, 0u), receiver, 1));
                await consumer.Delivered.Task.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
                Assert.Equal(1, cache.BatchProtections);

                var shutdown = accessor.Shutdown();
                await accessor.GetPubSubCache();
                Assert.False(shutdown.IsCompleted);
                Assert.False(cache.ReceiverStopped);
                Assert.Empty(cache.Progress);
                Assert.Equal(1, cache.OpenCursors);

                switch (outcome)
                {
                    case "acknowledged": release.SetResult(null); break;
                    case "delivery-token": release.SetResult(StreamHandshakeToken.CreateDeliveyToken(token)); break;
                    case "start-position":
                        release.SetResult(StreamHandshakeToken.CreateStartPositionToken(StreamSubscriptionStartPosition.EarliestAvailable));
                        break;
                    case "faulted": release.SetException(new InvalidOperationException("Delivery failed")); break;
                    case "canceled": release.SetCanceled(TestContext.Current.CancellationToken); break;
                }

                await shutdown.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
                Assert.True(cache.ReceiverStopped);
                Assert.Equal(0, cache.OpenCursors);
                Assert.Equal(0, cache.BatchProtections);
                Assert.Single(consumer.DeliveredTokens);
                Assert.Equal(outcome is "delivery-token" or "start-position" ? 3 : 2, cache.CursorAcquisitions);
                if (outcome == "start-position")
                {
                    Assert.Empty(cache.Progress);
                    Assert.Null(data.LastProcessedToken);
                }
                else
                {
                    Assert.Equal(outcome is "faulted" or "canceled" ? 0 : 1, Assert.Single(cache.Progress)?.SequenceNumber);
                }
                if (outcome is "faulted" or "canceled")
                {
        ...
        [InlineData("faulted")]
        [InlineData("canceled")]
        public async Task Shutdown_DrainsExplicitHandshakeWhileAcknowledgingAndSkippingLateWork(string outcome)
        {
            var streamId = new QualifiedStreamId("provider", StreamId.Create("namespace", Guid.NewGuid()));
            var token = new EventSequenceTokenV2(1);
            var cache = new ShutdownQueueCache();
            ...
            receiver.Shutdown(Arg.Any<TimeSpan>(), Arg.Any<CancellationToken>()).Returns(_ => cache.StopReceiver());
            var (accessor, pubSub, stream) = await CreateInitializedAgentWithStream(
                streamId, token, cache, new StreamPullingAgentOptions(), receiver: receiver);
            var agent = (PersistentStreamPullingAgent)accessor;
            ...
            Task Attach() => agent.RunOrQueueTask(() => agent.AddSubscriber(
                data.SubscriptionId, streamId, default, null, TestContext.Current.CancellationToken));

            try
            {
                await Attach().WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
                await started.Task.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
                Assert.Equal(1, data.PendingHandshakes);
                var shutdown = accessor.Shutdown();
                await accessor.GetPubSubCache();
                Assert.False(shutdown.IsCompleted);
                Assert.False(cache.ReceiverStopped);
                ...
                await Attach();
                await accessor.RunConsumerCursor(data);
                await accessor.RegisterStream(
                    new QualifiedStreamId("provider", StreamId.Create("late", Guid.NewGuid())), token, DateTime.UtcNow);
                Assert.Equal(1, handshakes);
                await pubSub.Received(1).RegisterProducer(Arg.Any<QualifiedStreamId>(), Arg.Any<GrainId>(), Arg.Any<CancellationToken>());
                ...
                switch (outcome)
                {
                    ...
                    case "start-position":
                        release.SetResult(StreamHandshakeToken.CreateStartPositionToken(StreamSubscriptionStartPosition.EarliestAvailable));
                        break;
                    case "faulted": release.SetException(new InvalidOperationException("Handshake failed")); break;
                    case "canceled": release.SetCanceled(TestContext.Current.CancellationToken); break;
                }

                await shutdown.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
                Assert.Equal(0, data.PendingHandshakes);
                Assert.Equal(outcome is "delivery-token" or "start-position" ? 2 : 1, cache.CursorAcquisitions);
                Assert.Equal(0, cache.OpenCursors);
                Assert.True(cache.ReceiverStopped);
                Assert.Empty(consumer.DeliveredTokens);
                Assert.Empty(consumer.Errors);
                if (outcome == "delivery-token")
                {
                    Assert.Equal(token, Assert.Single(cache.Progress));
                }
                else
                {
                    Assert.Empty(cache.Progress);
        ...
        [InlineData("faulted")]
        [InlineData("canceled")]
        public async Task Shutdown_DrainsRegistrationPinBeforeReceiverCleanup(string outcome)
        {
            var streamId = new QualifiedStreamId("provider", StreamId.Create("namespace", Guid.NewGuid()));
            var establishedStreamId = new QualifiedStreamId("provider", StreamId.Create("established", Guid.NewGuid()));
            var token = new EventSequenceTokenV2(1);
            var cache = new ShutdownQueueCache();
            cache.AddToCache([new TestBatchContainer(streamId.StreamId, token)]);
            var started = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
            ...
            pubSub.RegisterProducer(Arg.Any<QualifiedStreamId>(), Arg.Any<GrainId>(), Arg.Any<CancellationToken>())
                .Returns(call =>
                {
                    if (call.ArgAt<QualifiedStreamId>(0) == establishedStreamId)
                    {
            ...
            var agent = CreateAgent(pubSub, QueueId.GetQueueId("queue", 0u, 0u), receiver, adapterCache);
            var accessor = (PersistentStreamPullingAgent.ITestAccessor)agent;
            await InitializeAgent(agent);
            await accessor.RegisterStream(establishedStreamId, token, DateTime.UtcNow);
            var establishedStream = (await accessor.GetPubSubCache())[establishedStreamId];
            var establishedConsumer = establishedStream.AddConsumer(
                GuidId.GetGuidId(Guid.NewGuid()), establishedStreamId, new RecordingConsumer(), null, DateTime.UtcNow);
            establishedConsumer.IsRegistered = true;
            establishedConsumer.LastProcessedToken = new EventSequenceTokenV2(200);
            var registration = accessor.RegisterStream(streamId, token, DateTime.UtcNow);
            ...
            try
            {
                ...
                switch (outcome)
                {
                    case "completed": release.SetResult(new HashSet<PubSubSubscriptionState>()); break;
                    case "faulted": release.SetException(new InvalidOperationException("Registration failed")); break;
                    case "canceled": release.SetCanceled(TestContext.Current.CancellationToken); break;
                }
                await shutdown.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
            ...
            {
                release.TrySetResult(new HashSet<PubSubSubscriptionState>());
                await accessor.Shutdown().WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
            }
        }
        ...
        [TestArea("Streaming")]
        [Fact, TestCategory("BVT"), TestCategory("Streaming")]
        public async Task Initialize_WaitsForFullShutdownAndReopensAdmissionForTheNextRun()
        {
            var queueId = QueueId.GetQueueId("queue", 0u, 0u);
            var streamId = new QualifiedStreamId("provider", StreamId.Create("namespace", Guid.NewGuid()));
            var token = new EventSequenceTokenV2(1);
            ...
            var receiver = Substitute.For<IQueueAdapterReceiver>();
            var adapterCache = Substitute.For<IQueueAdapterCache>();
            adapterCache.CreateQueueCache(queueId).Returns(_ => new RecordingQueueCache());
            var agent = CreateAgent(pubSub, queueId, receiver, adapterCache);
            var accessor = (PersistentStreamPullingAgent.ITestAccessor)agent;
            await InitializeAgent(agent);
            var firstRelease = new TaskCompletionSource<StreamHandshakeToken?>(TaskCreationOptions.RunContinuationsAsynchronously);
            var secondRelease = new TaskCompletionSource<StreamHandshakeToken?>(TaskCreationOptions.RunContinuationsAsynchronously);
            using var cancellation = CancellationTokenSource.CreateLinkedTokenSource(TestContext.Current.CancellationToken);

            async Task<Task> Attach(TaskCompletionSource<StreamHandshakeToken?> release)
            {
                await accessor.RegisterStream(streamId, token, DateTime.UtcNow);
                ...
                var data = stream.AddConsumer(GuidId.GetGuidId(Guid.NewGuid()), streamId, consumer, null, DateTime.UtcNow);
                var attachment = accessor.AddSubscriber(data);
                await started.Task.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
                return attachment;
            }
            ...
            try
            {
                var firstAttachment = await Attach(firstRelease);
                var firstShutdown = accessor.Shutdown();
                var canceledInitialize = agent.RunOrQueueTask(() => agent.Initialize(cancellation.Token));
                await accessor.GetPubSubCache();
                Assert.False(firstShutdown.IsCompleted);
                Assert.False(canceledInitialize.IsCompleted);
                cancellation.Cancel();
                await Assert.ThrowsAnyAsync<OperationCanceledException>(() => canceledInitialize);
                await accessor.RegisterStream(
                    new QualifiedStreamId("provider", StreamId.Create("late", Guid.NewGuid())), token, DateTime.UtcNow);
                await pubSub.Received(1).RegisterProducer(Arg.Any<QualifiedStreamId>(), Arg.Any<GrainId>(), Arg.Any<CancellationToken>());

                var initialize = InitializeAgent(agent);
                await accessor.GetPubSubCache();
                Assert.False(initialize.IsCompleted);
                firstRelease.SetResult(StreamHandshakeToken.CreateDeliveyToken(token));
                await firstAttachment.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
                await cleanupStarted.Task.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
                Assert.False(firstShutdown.IsCompleted);
                Assert.False(initialize.IsCompleted);
                adapterCache.Received(1).CreateQueueCache(queueId);

                releaseCleanup.SetResult();
                await firstShutdown.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
                await initialize.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
                adapterCache.Received(2).CreateQueueCache(queueId);
                Assert.Empty(await accessor.GetPubSubCache());

                var secondAttachment = await Attach(secondRelease);
                var secondShutdown = accessor.Shutdown();
                await accessor.GetPubSubCache();
                Assert.False(secondShutdown.IsCompleted);
                await receiver.Received(1).Shutdown(Arg.Any<TimeSpan>(), Arg.Any<CancellationToken>());
                secondRelease.SetResult(StreamHandshakeToken.CreateDeliveyToken(token));
                await secondShutdown.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
                await secondAttachment.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
                await receiver.Received(2).Shutdown(Arg.Any<TimeSpan>(), Arg.Any<CancellationToken>());
            }
            finally
            {
                firstRelease.TrySetResult(null);
                secondRelease.TrySetResult(null);
        ...
        [TestArea("Streaming")]
        [Fact, TestCategory("BVT"), TestCategory("Streaming")]
        public async Task Shutdown_WaitsForPendingRegistrationsBeforeDeliveryProgress()
        {
            var registration = new TaskCompletionSource<ISet<PubSubSubscriptionState>>(TaskCreationOptions.RunContinuationsAsynchronously);
            var pubSub = Substitute.For<IStreamPubSub>();
            pubSub.RegisterProducer(default, default, TestContext.Current.CancellationToken)
        ...
        [TestArea("Streaming")]
        [Fact, TestCategory("BVT"), TestCategory("Streaming")]
        public async Task Handshake_RewindableDeliveryTokenReplaysAfterAcknowledgedRecord()
        {
            var queueId = QueueId.GetQueueId("queue", 0u, 0u);
            var streamId = StreamId.Create("namespace", Guid.NewGuid());
            var qualifiedStreamId = new QualifiedStreamId("provider", streamId);
            ...
            var testAccessor = (PersistentStreamPullingAgent.ITestAccessor)agent;
            await InitializeAgent(agent);
            await testAccessor.RegisterStream(qualifiedStreamId, new EventSequenceTokenV2(12), DateTime.UtcNow);
            var streamData = (await testAccessor.GetPubSubCache()).Single().Value;
            ...
            Assert.True(await testAccessor.DoHandshakeWithConsumer(
                consumerData,
                cacheToken: new EventSequenceTokenV2(12)));
            consumerData.IsRegistered = true;
            await testAccessor.RunConsumerCursor(consumerData);

            Assert.Equal([11L, 12L], consumer.DeliveredTokens.Select(static token => token.SequenceNumber));
            await testAccessor.Shutdown();
        }
        ...
        [TestArea("Streaming")]
        [Fact, TestCategory("BVT"), TestCategory("Streaming")]
        public async Task Handshake_InvalidRewindTokenSurfacesWithoutLiveFallback()
        {
            var queueId = QueueId.GetQueueId("queue", 0u, 0u);
            var streamId = new QualifiedStreamId("provider", StreamId.Create("namespace", Guid.NewGuid()));
            var queueCache = new RecordingQueueCache();
            ...
            var testAccessor = (PersistentStreamPullingAgent.ITestAccessor)agent;
            await InitializeAgent(agent);
            await testAccessor.RegisterStream(streamId, new EventSequenceTokenV2(12), DateTime.UtcNow);
            var streamData = (await testAccessor.GetPubSubCache()).Single().Value;
            var consumer = new StartingConsumer(
                StreamHandshakeToken.CreateStartToken(new EventSequenceTokenV2(2)));
            var consumerData = streamData.AddConsumer(
                GuidId.GetGuidId(Guid.NewGuid()),
                streamId,
                consumer,
                filterData: null,
                now: DateTime.UtcNow);
            queueCache.CursorFactory = static (_, _) =>
                throw new ArgumentOutOfRangeException("token");

            Assert.False(await testAccessor.DoHandshakeWithConsumer(
                consumerData,
                cacheToken: new EventSequenceTokenV2(12)));
        ...
        [TestArea("Streaming")]
        [Fact, TestCategory("BVT"), TestCategory("Streaming")]
        public async Task RunConsumerCursor_AwaitsHistoricalCursorAndDeliversRetainedBatch()
        {
            var queueId = QueueId.GetQueueId("queue", 0u, 0u);
            var streamId = StreamId.Create("namespace", Guid.NewGuid());
            var qualifiedStreamId = new QualifiedStreamId("provider", streamId);
            ...
            var consumerData = streamData.AddConsumer(
                ...
                filterData: null,
                now: DateTime.UtcNow);
            var retained = new TestBatchContainer(streamId, new EventSequenceTokenV2(2));
            var cursor = new AwaitableReplayCursor(retained);
            consumerData.Cursor = cursor;
            consumerData.IsRegistered = true;

            var delivery = testAccessor.RunConsumerCursor(consumerData);
            await cursor.MoveStarted.Task.WaitAsync(
                TimeSpan.FromSeconds(5),
                TestContext.Current.CancellationToken);
            ...
            Assert.Equal([2L], consumer.DeliveredTokens.Select(static token => token.SequenceNumber));
            Assert.Equal(2, consumerData.LastSafePartitionToken?.SequenceNumber);
            await testAccessor.Shutdown();
        }
        ...
        [TestArea("Streaming")]
        [Fact, TestCategory("BVT"), TestCategory("Streaming")]
        public async Task RunConsumerCursor_EndsAggregatedDeliveryAtReplayHandoff()
        {
            var queueId = QueueId.GetQueueId("queue", 0u, 0u);
            var streamId = StreamId.Create("namespace", Guid.NewGuid());
            var qualifiedStreamId = new QualifiedStreamId("provider", streamId);
            ...
            var agent = CreateAgent(
                ...
                receiver: null,
                queueAdapterCache,
                options: new StreamPullingAgentOptions { BatchContainerBatchSize = 2 },
                isRewindable: true);
            var testAccessor = (PersistentStreamPullingAgent.ITestAccessor)agent;
            await InitializeAgent(agent);
            await testAccessor.RegisterStream(qualifiedStreamId, new EventSequenceTokenV2(3), DateTime.UtcNow);
            var streamData = (await testAccessor.GetPubSubCache()).Single().Value;
            ...
            var consumerData = streamData.AddConsumer(
                ...
                filterData: null,
                now: DateTime.UtcNow);
            consumerData.Cursor = new BoundaryReplayCursor(
                new TestBatchContainer(streamId, new EventSequenceTokenV2(2)),
                new TestBatchContainer(streamId, new EventSequenceTokenV2(3)));
            consumerData.IsRegistered = true;
            ...
            Assert.Equal([2L, 3L], consumer.DeliveredTokens.Select(static token => token.SequenceNumber));
            Assert.Equal(3, consumer.DeliveryAttempts);
            await testAccessor.Shutdown();
        }
        ...
        [TestArea("Streaming")]
        [Fact, TestCategory("BVT"), TestCategory("Streaming")]
        public async Task RunConsumerCursor_HistoricalFailureSurfacesWithoutLiveFallback()
        {
            var queueId = QueueId.GetQueueId("queue", 0u, 0u);
            var streamId = StreamId.Create("namespace", Guid.NewGuid());
            var qualifiedStreamId = new QualifiedStreamId("provider", streamId);
            ...
            var consumerData = streamData.AddConsumer(
                ...
                filterData: null,
                now: DateTime.UtcNow);
            consumerData.Cursor = new AwaitableReplayCursor(
                new DataNotAvailableException("retained history expired"));
            consumerData.CursorStartToken = new EventSequenceTokenV2(2);
            consumerData.IsRegistered = true;

            await testAccessor.RunConsumerCursor(consumerData);

            Assert.Null(consumerData.Cursor);
            var error = Assert.IsType<DataNotAvailableException>(Assert.Single(consumer.Errors));
        ...
        [TestArea("Streaming")]
        [Fact, TestCategory("BVT"), TestCategory("Streaming")]
        public async Task RunConsumerCursor_TransientReplayFailureRestartsFromSafeToken()
        {
            var queueId = QueueId.GetQueueId("queue", 0u, 0u);
            var streamId = StreamId.Create("namespace", Guid.NewGuid());
            var qualifiedStreamId = new QualifiedStreamId("provider", streamId);
            ...
            await InitializeAgent(agent);
            await testAccessor.RegisterStream(qualifiedStreamId, new EventSequenceTokenV2(10), DateTime.UtcNow);
            queueCache.CursorFactory = (_, token) =>
            {
                Assert.Equal(2, token?.SequenceNumber);
                return retryCursor;
            };
            var streamData = (await testAccessor.GetPubSubCache()).Single().Value;
            var consumer = new StartingConsumer(startToken: null);
            var consumerData = streamData.AddConsumer(
                ...
                now: DateTime.UtcNow);
            consumerData.Cursor = new AwaitableReplayCursor(
                new TransientStreamReplayException(
                    "temporary retained-history failure",
            ...
            Assert.False(consumerData.IsReplayUnavailable);
            Assert.Equal([2L], consumer.DeliveredTokens.Select(static token => token.SequenceNumber));
            Assert.IsType<TransientStreamReplayException>(Assert.Single(consumer.Errors));
            Assert.Equal(2, queueCache.GetCacheCursorCallCount);
            await testAccessor.Shutdown();
        }
        ...
        [TestArea("Streaming")]
        [Fact, TestCategory("BVT"), TestCategory("Streaming")]
        public async Task RunConsumerCursor_UnsubscribeDuringReplayDoesNotRecreateCursor()
        {
            var queueId = QueueId.GetQueueId("queue", 0u, 0u);
            var streamId = StreamId.Create("namespace", Guid.NewGuid());
            var qualifiedStreamId = new QualifiedStreamId("provider", streamId);
            ...
            var consumerData = streamData.AddConsumer(
                ...
                filterData: null,
                now: DateTime.UtcNow);
            var cursor = new AwaitableReplayCursor(
                new TestBatchContainer(streamId, new EventSequenceTokenV2(2)));
            consumerData.Cursor = cursor;
            consumerData.CursorStartToken = new EventSequenceTokenV2(2);
            consumerData.IsRegistered = true;
            ...
            await cursor.MoveStarted.Task.WaitAsync(
                TimeSpan.FromSeconds(5),
                TestContext.Current.CancellationToken);
            agent.RemoveSubscriber_Impl(subscriptionId, qualifiedStreamId);
            await delivery;

            Assert.Empty(consumer.Errors);
            Assert.Empty(consumer.DeliveredTokens);
            Assert.Equal(1, queueCache.GetCacheCursorCallCount);
            await testAccessor.Shutdown();
        }
        ...
        public async Task DeliveryProgress_ReplayCursorDoesNotBlockLiveQueueCheckpoint()
        {
            ...
            var testAccessor = (PersistentStreamPullingAgent.ITestAccessor)agent;
            await InitializeAgent(agent);
            await testAccessor.RegisterStream(streamId, new EventSequenceTokenV2(10), timeProvider.GetUtcNow().UtcDateTime);
            var streamData = (await testAccessor.GetPubSubCache()).Single().Value;
            var consumerData = streamData.AddConsumer(
                GuidId.GetGuidId(Guid.NewGuid()),
                streamId,
                new StartingConsumer(startToken: null),
                filterData: null,
                now: timeProvider.GetUtcNow().UtcDateTime);
            consumerData.Cursor = new AwaitableReplayCursor(
        ...
        private sealed class FailOnceConsumer : IStreamConsumerExtension
        {
                ...
                CancellationToken cancellationToken)
            {
                ...
                {
                    throw new InvalidOperationException("retry historical delivery");
                }
            ...
            public Task<StreamHandshakeToken?> GetSequenceToken(GuidId subscriptionId, CancellationToken cancellationToken)
                => cancellationToken.IsCancellationRequested
                    ? Task.FromCanceled<StreamHandshakeToken?>(cancellationToken)
                    : Task.FromResult<StreamHandshakeToken?>(null);
        }

        private sealed class AwaitableReplayCursor : IAsyncQueueCacheCursor, IQueueCacheCursorProgress, IQueueCacheCursorReplayState
        {
            private readonly TaskCompletionSource _release =
                new(TaskCreationOptions.RunContinuationsAsynchronously);
            private readonly IBatchContainer? _batch;
            private readonly Exception? _exception;
            private int _moveCount;

            public AwaitableReplayCursor(IBatchContainer batch)
            {
                _batch = batch;
            }

            public AwaitableReplayCursor(Exception exception)
            {
            ...
            public bool HasPendingLiveHandoff => false;

            public void Release() => _release.TrySetResult();

            public bool MoveNext() => false;

            public async ValueTask<QueueCacheCursorMoveNextResult> MoveNextAsync(
                CancellationToken cancellationToken)
            {
                ...
                {
                    return QueueCacheCursorMoveNextResult.ItemAvailable;
                }
            ...
            public void RecordDeliverySuccess() => SafeSequenceToken = _batch?.SequenceToken;

            public void Dispose() => _release.TrySetCanceled();
        }

        private sealed class BoundaryReplayCursor(
            IBatchContainer historical,
            IBatchContainer live)
            : IAsyncQueueCacheCursor, IQueueCacheCursorProgress, IQueueCacheCursorReplayState
        {
            ...
            public StreamSequenceToken? SafeSequenceToken { get; private set; }
            public bool IsReplaying { get; private set; } = true;
            public bool HasPendingLiveHandoff { get; private set; }

            public bool MoveNext() => false;

            public ValueTask<QueueCacheCursorMoveNextResult> MoveNextAsync(
                CancellationToken cancellationToken)
            {
                cancellationToken.ThrowIfCancellationRequested();
                switch (_state++)
                {
                    case 0:
                        _current = historical;
                        return ValueTask.FromResult(QueueCacheCursorMoveNextResult.ItemAvailable);
                    case 1:
                        IsReplaying = false;
                        HasPendingLiveHandoff = true;
                        return ValueTask.FromResult(QueueCacheCursorMoveNextResult.Completed);
                    case 2:
                        HasPendingLiveHandoff = false;
                        _current = live;
                        return ValueTask.FromResult(QueueCacheCursorMoveNextResult.ItemAvailable);
                    default:
                        return ValueTask.FromResult(QueueCacheCursorMoveNextResult.Completed);
            ...
            public void RecordDeliverySuccess() => SafeSequenceToken = _current?.SequenceToken;
