...
namespace Orleans.Serialization.UnitTests
{
    ...
    public class ArcBufferWriterTests
    {
        ...
        [Fact]
        public void Peek_DoesNotReadPastPartialPinnedSlice()
        {
            using var source = new ArcBufferWriter();
            source.Write(Enumerable.Range(0, 32).Select(static value => (byte)value).ToArray());
            using var slice = source.PeekSlice(10);
            using var buffer = new ArcBufferWriter();
            buffer.AppendPinned(slice);
            Span<byte> destination = stackalloc byte[20];

            Assert.Equal(0, buffer.Peek(in destination).Length);
            Assert.Equal(10, buffer.Peek(destination));
            Assert.Equal(Enumerable.Range(0, 10).Select(static value => (byte)value), destination[..10].ToArray());
            Assert.Throws<ArgumentOutOfRangeException>(() => buffer.AdvanceReader(11));

            buffer.AdvanceReader(10);
...
||||||| parent of bb4be0b32f (feat(networking): introduce message-oriented networking transports)
