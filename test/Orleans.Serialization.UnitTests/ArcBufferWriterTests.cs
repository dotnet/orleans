...
public class ArcBufferWriterTests
{
    ...
    [Fact]
    public void AppendAndReset_TransfersSourceToEmptyDestinationAndKeepsDestinationWritable()
    {
        using var source = new ArcBufferWriter();
        source.Write([1, 2, 3]);
        using var destination = new ArcBufferWriter();

        destination.AppendAndReset(source);

        Assert.Equal([1, 2, 3], ToArray(destination));
        Assert.Equal(0, source.Length);

        destination.Write([4]);
        destination.WriteAt(0, [9]);
        destination.Truncate(3);
        source.Write([5, 6]);

        Assert.Equal([9, 2, 3], ToArray(destination));
        Assert.Equal([5, 6], ToArray(source));
    }

    [Fact]
    public void AppendAndReset_AppendsSourceToNonEmptyDestination()
    {
        using var destination = new ArcBufferWriter();
        ...
        Assert.Equal([1, 2, 3, 4, 5], ToArray(destination));
        Assert.Equal(0, source.Length);

        destination.Write([6]);
        source.Write([7]);

        Assert.Equal([1, 2, 3, 4, 5, 6], ToArray(destination));
        Assert.Equal([7], ToArray(source));
    }

    [Fact]
    public void AppendAndReset_AllowsPartiallyConsumedSourceWhenDestinationIsEmpty()
    {
        using var source = new ArcBufferWriter();
        source.Write([1, 2, 3, 4]);
        source.AdvanceReader(2);
        using var destination = new ArcBufferWriter();

        destination.AppendAndReset(source);

        Assert.Equal([3, 4], ToArray(destination));
        Assert.Equal(0, source.Length);
    }

    [Fact]
    public void AppendAndReset_RejectsPartiallyConsumedSourceWhenDestinationIsNotEmpty()
    {
        using var source = new ArcBufferWriter();
        source.Write([1, 2, 3]);
        source.AdvanceReader(1);
        using var destination = new ArcBufferWriter();
        destination.Write([0]);

        var exception = Assert.Throws<InvalidOperationException>(() => destination.AppendAndReset(source));

        Assert.Contains("partially consumed", exception.Message, StringComparison.Ordinal);
        Assert.Equal([0], ToArray(destination));
        Assert.Equal([2, 3], ToArray(source));
    }

    [Fact]
    public void AppendAndReset_RejectsSourceWithOutstandingSlices()
    {
        using var source = new ArcBufferWriter();
        source.Write([1, 2, 3]);
        using var slice = source.PeekSlice(source.Length);
        ...
        Assert.Contains("outstanding slices", exception.Message, StringComparison.Ordinal);
        Assert.Equal([1, 2, 3], ToArray(source));
        Assert.Equal(0, destination.Length);
    }

    [Fact]
    public void AppendAndReset_RejectsSelfAppend()
    {
        using var buffer = new ArcBufferWriter();
        buffer.Write([1]);

        var exception = Assert.Throws<InvalidOperationException>(() => buffer.AppendAndReset(buffer));

        Assert.Contains("itself", exception.Message, StringComparison.Ordinal);
        Assert.Equal([1], ToArray(buffer));
    }

    [Fact]
    public void AppendAndReset_ReleasesDestinationPreallocatedPagesBeforeAppending()
    {
        using var destination = new ArcBufferWriter();
        destination.Write([1, 2]);
        var buffers = new List<ArraySegment<byte>>(2);
        destination.ReplenishBuffers(buffers);
