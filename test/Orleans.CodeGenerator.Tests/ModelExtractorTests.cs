...
";
    ...
    public async Task ExtractProxyInterfaceModel_UsesOriginalMethodDefinitionForGeneratedMethodId()
    {
        ...
        var expectedMethodId = GeneratedCodeUtilities.CreateHashedMethodId(originalMethod, cancellationTokenType);

        Assert.Equal(expectedMethodId, method.GeneratedMethodId);
    }

    [Fact]
    public async Task ExtractProxyInterfaceModel_CancellationTokenDoesNotAffectGeneratedMethodId()
    {
        const string codeWithoutCancellation = """
            using Orleans;
            using Orleans.Runtime;
            using System.Threading.Tasks;
            ...
            public interface ITestGrain
            {
                ValueTask Echo(int value);
            }
            """;
        const string codeWithCancellation = """
            using Orleans;
            using Orleans.Runtime;
            ...
            public interface ITestGrain
            {
                ValueTask Echo(int value, CancellationToken cancellationToken);
            }
            """;

        var modelWithoutCancellation = ExtractProxyInterfaceModel(
            await CreateCompilation(codeWithoutCancellation),
            "TestProject.ITestGrain");
        var modelWithCancellation = ExtractProxyInterfaceModel(
            await CreateCompilation(codeWithCancellation),
            "TestProject.ITestGrain");

        var methodWithoutCancellation = Assert.Single(modelWithoutCancellation.Methods);
        var methodWithCancellation = Assert.Single(modelWithCancellation.Methods);
        Assert.Equal(
            methodWithoutCancellation.GeneratedMethodId,
            methodWithCancellation.GeneratedMethodId);
        Assert.Equal(
            methodWithoutCancellation.GeneratedMethodId,
            methodWithoutCancellation.LegacyGeneratedMethodId);
        Assert.NotEqual(
            methodWithCancellation.GeneratedMethodId,
            methodWithCancellation.LegacyGeneratedMethodId);
    }

    [Fact]
    public async Task ExtractProxyInterfaceModel_CancellationTokenDistinguishesOverloads()
    {
        const string code = """
            using Orleans;
            using Orleans.Runtime;
        ...
        var model = ExtractProxyInterfaceModel(
            await CreateCompilation(code),
            "TestProject.ITestGrain");

        Assert.Equal(2, model.Methods.Select(method => method.GeneratedMethodId).Distinct().Count());
        Assert.All(
            model.Methods,
            method => Assert.Equal(method.GeneratedMethodId, method.LegacyGeneratedMethodId));
    }

    [Fact]
    public async Task ExtractProxyInterfaceModel_NonTrailingCancellationTokenRetainsLegacyMethodId()
    {
        const string code = """
            using Orleans;
            using Orleans.Runtime;
            ...
            public interface ITestGrain
            {
                ValueTask Echo(int before, CancellationToken cancellationToken, Guid after);
            }
            """;
        ...
        Assert.Equal(method.LegacyGeneratedMethodId, method.GeneratedMethodId);
    }
