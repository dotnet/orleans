...
public interface ILogTestGrain : IGrainWithIntegerKey
{
    ...
    Task RaiseEventsWithUnsupportedSecond();

    Task Clear();

    Task ClearWithCancellation();
...
public interface ILogTestGrainWithAuxiliaryState : ILogTestGrain
{
    ...
    Task SetAuxiliaryValueAndAGlobal(int auxiliaryValue, int value);

    Task<int> SetAuxiliaryValueAndSynchronize(int value);
}
