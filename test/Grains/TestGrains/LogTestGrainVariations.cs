...
[Reentrant]
[LogConsistencyProvider(ProviderName = "JournaledState")]
public class LogTestGrainJournaledStateReentrantStorage : LogTestGrain
{
}
        ...
        ILogTestGrainWithAuxiliaryState
{
    ...
    public async Task SetAuxiliaryValueAndAGlobal(int auxiliaryValue, int value)
    {
        auxiliaryState.Value = auxiliaryValue;
        await SetAGlobal(value);
    }

    public async Task<int> SetAuxiliaryValueAndSynchronize(int value)
    {
        auxiliaryState.Value = value;
        await SynchronizeGlobalState();
        return auxiliaryState.Value;
    }
}
