...
public interface IActivationStartupTestGrain : IGrainWithStringKey
{
    Task<string> Invoke(string payload);
}

public sealed class ActivationStartupTestGrain :
    IActivationStartupTestGrain,
    IGrainBase,
    ILifecycleParticipant<IGrainLifecycle>
{
    private readonly ActivationStartupScenario _scenario;

    public ActivationStartupTestGrain(IGrainContext grainContext, ActivationStartupTestHooks hooks)
    {
    ...
    public Task<string> Invoke(string payload)
    {
        ...
        return Task.FromResult($"{payload}:{_scenario.RequestContextValue}");
    }
