...
namespace UnitTests.Directory
{
    ...
    public class CachedGrainLocatorTests
    {
        ...
        public async Task RegisterWhenNoOtherEntryExists()
        {
            ...
            grainDirectory.Register(expected, previousAddress: null).Returns(expected);

            var actual = await grainLocator.Register(expected, previousAddress: null);
            Assert.Equal(expected, actual);
            await grainDirectory.Received(1).Register(expected, previousAddress: null);
        ...
        public async Task RegisterWhenOtherEntryExists()
        {
            ...
            grainDirectory.Register(otherAddr, previousAddress: null).Returns(expectedAddr);

            var actual = await grainLocator.Register(otherAddr, previousAddress: null);
            Assert.Equal(expectedAddr, actual);
            await grainDirectory.Received(1).Register(otherAddr, previousAddress: null);
        ...
        public async Task RegisterWhenOtherEntryExistsAndPreviousAddressMatches()
        {
            ...
            grainDirectory.Register(replacement, previousAddress: null).Returns(existing);

            var actual = await grainLocator.Register(replacement, previousAddress: null);
            Assert.Equal(existing, actual);
            await grainDirectory.Received(1).Register(replacement, previousAddress: null);
            grainDirectory.ClearReceivedCalls();

            grainDirectory.Register(replacement, previousAddress: existing).Returns(replacement);
            actual = await grainLocator.Register(replacement, previousAddress: existing);
            await grainDirectory.Received(1).Register(replacement, previousAddress: existing);
            Assert.Equal(replacement, actual);

            // Now should be in cache
            Assert.True(grainLocator.TryLookupInCache(replacement.GrainId, out var result));
            Assert.NotNull(result);
        ...
        public async Task RegisterWhenOtherEntryExistsAndPreviousAddressDoesNotMatch()
        {
            ...
            grainDirectory.Register(replacement, previousAddress: nonMatching).Returns(existing);
            actual = await grainLocator.Register(replacement, previousAddress: nonMatching);
            await grainDirectory.Received(1).Register(replacement, previousAddress: nonMatching);
            Assert.Equal(existing, actual);
        ...
        public async Task RegisterWhenOtherEntryExistsButSiloIsDead()
        {
            ...
            grainDirectory.Register(expectedAddr, previousAddress: null).Returns(outdatedAddr, expectedAddr);

            var actual = await grainLocator.Register(expectedAddr, previousAddress: null);
            Assert.Equal(expectedAddr, actual);
            await grainDirectory.Received(2).Register(expectedAddr, previousAddress: null);
        ...
        public async Task CleanupWhenSiloIsDead()
        {
            ...
            grainDirectory.Register(expectedAddr, previousAddress: null).Returns(expectedAddr);
            grainDirectory.Register(outdatedAddr, previousAddress: null).Returns(outdatedAddr);

            await grainLocator.Register(expectedAddr, previousAddress: null);
            await grainLocator.Register(outdatedAddr, previousAddress: null);
        ...
        {            
            ...
            grainDirectory.Unregister(expectedAddr).Returns(async (t) => { await Task.Yield();  while (true) { } });
            ...
            // Unregister and check if cache was cleaned
            _ = grainLocator.Unregister(expectedAddr, UnregistrationCause.Force);
            Assert.False(grainLocator.TryLookupInCache(expectedAddr.GrainId, out _));
        }
        ...
        public async Task UnregisterRacesWithLookupSameId()
        {
            ...
            // Give up control then Run forever
            grainDirectory.Unregister(expectedAddr).Returns(async (t) =>
            {
                await Task.Yield();
                mre.WaitOne();
            });
            ...
            // Unregister and check if cache was cleaned
            Task t = grainLocator.Unregister(expectedAddr, UnregistrationCause.Force);
            Assert.False(grainLocator.TryLookupInCache(expectedAddr.GrainId, out _));

            // Add back to cache simulating a race from lookup
            await grainLocator.Register(expectedAddr, previousAddress: null);
            Assert.True(grainLocator.TryLookupInCache(expectedAddr.GrainId, out _));
        ...
        public async Task UnregisterRacesWithLookupDifferentId()
        {
            ...
            // Setup membership service
            mockMembershipService.UpdateSiloStatus(expectedSilo, SiloStatus.Active, "exp");
            mockMembershipService.UpdateSiloStatus(secondSilo, SiloStatus.Active, "exp");
            await lifecycle.OnStart();
            await WaitUntilClusterChangePropagated();
            ...
            grainDirectory.Register(expectedAddr, previousAddress: null).Returns(expectedAddr);
            grainDirectory.Register(secondAddr, previousAddress: null).Returns(secondAddr);
            ...
            // Add back to cache simulating a race from lookup
            await grainLocator.Register(secondAddr, previousAddress: null);
            Assert.True(grainLocator.TryLookupInCache(secondAddr.GrainId, out _));
