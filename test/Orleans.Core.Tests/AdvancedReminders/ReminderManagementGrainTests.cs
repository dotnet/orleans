...
public class ReminderManagementGrainTests
{
    ...
    [Fact]
    public async Task ListFilteredAsync_AppliesScheduleAndStatus()
    {
        ...
        var table = new InMemoryManagementReminderTable(
            ...
            new ReminderEntry
            {
                GrainId = GrainId.Create("test", "wrong-schedule"),
                ReminderName = "wrong-schedule",
                StartAt = now.AddMinutes(-20),
                Period = TimeSpan.Zero,
    ...
    public async Task MutationApis_WithReminderService_RescheduleReminderChain()
    {
        ...
        await jobManager.Received(2).ScheduleJobAsync(
