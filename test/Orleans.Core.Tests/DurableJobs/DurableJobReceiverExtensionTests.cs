...
public class DurableJobReceiverExtensionTests
{
    ...
    {
        // The already-completed TCS's continuation (which flips the internal task to a terminal
        // state) is scheduled asynchronously (TaskCreationOptions.RunContinuationsAsynchronously),
        // so it may not have run by the time we poll. Bound the wait by wall-clock time rather than
        // a fixed iteration count of Task.Yield(), so this remains reliable under CPU/thread-pool
        // contention (e.g. when the full test assembly runs alongside hundreds of other tests).
