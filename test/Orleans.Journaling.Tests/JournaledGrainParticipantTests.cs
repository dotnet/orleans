...
internal sealed class FirstJournaledParticipant : IJournaledGrainParticipant
{
    ...
    public FirstJournaledParticipant(IGrainContext grainContext, IServiceProvider serviceProvider, ParticipantRecorder recorder)
    {
...
internal sealed class SecondJournaledParticipant : IJournaledGrainParticipant
{
    ...
    public SecondJournaledParticipant(IGrainContext grainContext, IServiceProvider serviceProvider, ParticipantRecorder recorder)
    {
        _grainType = grainContext.GrainId.Type.ToString();
        _serviceProvider = serviceProvider;
        _recorder = recorder;
...
public sealed class ParticipantRecorder
{
    ...
    public void RecordFirstConstruction(string grainType) =>
        _counts.AddOrUpdate(grainType, static _ => new() { FirstConstructions = 1 }, static (_, value) => value with { FirstConstructions = value.FirstConstructions + 1 });

    public void RecordSecondConstruction(string grainType) =>
        _counts.AddOrUpdate(grainType, static _ => new() { SecondConstructions = 1 }, static (_, value) => value with { SecondConstructions = value.SecondConstructions + 1 });

    public void RecordFailureConstruction(string grainType) =>
        _counts.AddOrUpdate(grainType, static _ => new() { FailureConstructions = 1 }, static (_, value) => value with { FailureConstructions = value.FailureConstructions + 1 });

    public void RecordFirstInitialization(string grainType) =>
        _counts.AddOrUpdate(grainType, static _ => new() { FirstInitializations = 1 }, static (_, value) => value with { FirstInitializations = value.FirstInitializations + 1 });

    public void RecordSecondInitialization(string grainType) =>
        _counts.AddOrUpdate(grainType, static _ => new() { SecondInitializations = 1 }, static (_, value) => value with { SecondInitializations = value.SecondInitializations + 1 });
