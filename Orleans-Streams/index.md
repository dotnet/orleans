...
In a typical example below a chat user (implemented as Orleans Grain) joins a chat room, it gets a handle to a chat room name and subscribes to a stream of chat messages generated by all others users in this room. Notice that the chat user neither does not need to know about the chat room grain itself (there might not be such a grain in our system) nor about other user in that group that produce messages. Needless to say, to produce to the chat stream, user don't need to know who is currently subscribed to the stream. This demonstrates how chat users can be completely decoupled in time and space.
