using System;
using System.Threading;
using System.Threading.Tasks;
using Orleans.Placement;
using Orleans.Runtime;
using Orleans.Runtime.Placement;

namespace Orleans.Streams;

[GenerateSerializer]
internal sealed class PullingAgentPlacement : PlacementStrategy
{
    internal const string RetirementKey = "stream-pulling-agent-retirement";

    internal static TResult WithRetirementHint<TResult>(SiloAddress silo, Func<TResult> send)
    {
        var previous = RequestContext.Get(RetirementKey);
        RequestContext.Set(RetirementKey, true);
        try
        {
            return WithHint(silo, send);
        }
        finally
        {
            if (previous is null)
            {
                RequestContext.Remove(RetirementKey);
            }
            else
            {
                RequestContext.Set(RetirementKey, previous);
            }
        }
    }

    internal static TResult WithHint<TResult>(SiloAddress silo, Func<TResult> send)
    {
        var previous = RequestContext.Get(IPlacementDirector.PlacementHintKey);
        RequestContext.Set(IPlacementDirector.PlacementHintKey, silo);
        try
        {
            return send();
        }
        finally
        {
            if (previous is null)
            {
                RequestContext.Remove(IPlacementDirector.PlacementHintKey);
            }
            else
            {
                RequestContext.Set(IPlacementDirector.PlacementHintKey, previous);
            }
        }
    }
}

internal sealed class PullingAgentPlacementAttribute() : PlacementAttribute(new PullingAgentPlacement());

internal sealed class PullingAgentPlacementDirector(
    PullingAgentHostResolver hosts,
    IInternalGrainFactory grainFactory,
    IClusterMembershipService membership) : IPlacementDirector
{
    internal const string ProviderPropertyPrefix = "stream-pulling-agent-provider:";

    public async Task<SiloAddress> OnAddActivation(PlacementStrategy strategy, PlacementTarget target, IPlacementContext context)
    {
        var isCoordinator = target.GrainIdentity.Type == PullingCoordinatorGrain.GrainType;
        var (providerName, queueId) = isCoordinator
            ? (target.GrainIdentity.Key.ToString(), (QueueId?)null)
            : GetAgentIdentity(target.GrainIdentity);
        var compatible = context.GetCompatibleSilos(target);
        var requestContext = target.RequestContextData;
        if (!isCoordinator
            && requestContext is not null
            && requestContext.TryGetValue(PullingAgentPlacement.RetirementKey, out var retirement)
            && retirement is true
            && IPlacementDirector.GetPlacementHint(requestContext, compatible) is { } retirementHost
            && membership.CurrentSnapshot.GetSiloStatus(retirementHost) == SiloStatus.Active
            && await grainFactory.GetSystemTarget<IPullingAgentRuntime>(PullingAgentRuntime.TargetType, retirementHost)
                .CanRetire(providerName, queueId!.Value, CancellationToken.None))
        {
            return retirementHost;
        }

        if (IPlacementDirector.GetPlacementHint(target.RequestContextData, compatible) is { } hint
            && (await hosts.FilterEligibleSilos(providerName, queueId, target.GrainIdentity.Type, [hint], CancellationToken.None)).Length > 0)
        {
            return hint;
        }

        var eligible = await hosts.FilterEligibleSilos(providerName, queueId, target.GrainIdentity.Type, compatible, CancellationToken.None);
        if (eligible.Length == 0)
        {
            throw new OrleansException($"Stream provider '{providerName}' has no running, compatible host for {target.GrainIdentity}.");
        }

        return eligible[Random.Shared.Next(eligible.Length)];
    }

    private static (string ProviderName, QueueId? QueueId) GetAgentIdentity(GrainId grainId)
    {
        var (providerName, queueId) = PullingAgentId.Parse(grainId);
        return (providerName, queueId);
    }
}
