            ...
            try
            {
                ...
                while (!IsShutdown && consumerData.Cursor is not null)
                {
                    ...
                    catch (Exception exc)
                    {
                        ...
                        consumerData.SafeDisposeCursor(logger);
                        consumerData.Cursor = queueCache!.GetCacheCursor(consumerData.StreamId, OldestInStreamToken.Instance); // queueCache must be non-null here: consumerData.Cursor was only ever populated via queueCache.GetCacheCursor.
                    }
