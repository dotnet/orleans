...
internal sealed partial class S3JournalStorage : IJournalStorage
{
    ...
    private const int MaxConditionalRequestConflictRetries = 5;
    private const long MaxSinglePutObjectBytes = 5_000_000_000;
        ...
        try
        {
            for (var attempt = 0; ; attempt++)
            {
                ...
                catch (AmazonS3Exception exception) when (IsObjectAlreadyExists(exception))
                {
                    ...
                    IsConditionalRequestConflict(exception)
                    && attempt < MaxConditionalRequestConflictRetries)
                {
                    var current = await TryLoadWalStateAsync(expectedETag: null, cancellationToken).ConfigureAwait(false);
            ...
            for (var attempt = 0; attempt < 3; attempt++)
            {
                ...
                if (!ApplyCallerMetadataUpdate(metadata, setValues, removeValues))
                {
                    var partsCount = await GetWalPartsCountAsync(
                        walState.ETag,
                        walState.ProviderState.ContentLength,
                        properties.PartsCount,
                        cancellationToken).ConfigureAwait(false);
                    SetWal(
                        walState.ETag,
                        walState.ProviderState with { PartsCount = partsCount ?? walState.ProviderState.PartsCount },
                        walState.LastModified);
                    succeeded = true;
                    return CreateJournalMetadata(properties.ETag, metadata);
                }

                metadata[MetadataVersionMetadataKey] = CreateMetadataVersion();
                if (walState.Manifest.WalOffset > properties.ContentLength)
                {
                    throw new InvalidOperationException(
                        $"S3 journal payload offset {walState.Manifest.WalOffset:N0} exceeds WAL length {properties.ContentLength:N0}.");
                }

                var replacementLength = properties.ContentLength - walState.Manifest.WalOffset + CompactedWalMarkerBytes;
                if (replacementLength > MaxSinglePutObjectBytes)
                {
                    throw new InvalidOperationException(
                        $"S3 journal WAL metadata cannot be updated while the physical replacement object is larger than 5 GB. " +
                        $"Publish a checkpoint to compact the WAL before updating metadata. Replacement length: {replacementLength:N0} bytes.");
                }
                ...
                try
                {
                    ...
                    {
                        await SkipStreamAsync(walResult.ResponseStream, walState.Manifest.WalOffset, cancellationToken).ConfigureAwait(false);
                    }

                    await walResult.ResponseStream.CopyToAsync(payload, cancellationToken).ConfigureAwait(false);
                    metadata[CheckpointOffsetMetadataKey] = marker.Length.ToString(CultureInfo.InvariantCulture);
                    payload.Position = 0;
                    ...
                    SetWal(
                        response.ETag,
                        CreateWalProviderState(updatedManifest, payload.Length, partsCount: 1),
                        lastModified: null);
                    succeeded = true;
    ...
    public async ValueTask DeleteAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            for (var attempt = 0; ; attempt++)
            {
                ...
                {
                    walState = expectedETag is not null && attempt < _shared.Options.MaxMetadataOnlyConflictRetries
                        ? await RetryAfterMetadataOnlyConflictAsync(attempt, expectedProviderState, cancellationToken).ConfigureAwait(false)
                        : null;
                    if (walState is null)
                    {
                ...
                if (expectedETag is not null && !IsSameLogicalWal(walState.Value.ProviderState, expectedProviderState))
                {
                    throw CreateInconsistentWalStateException(
                        "S3 journal WAL was replaced while deleting the journal; recovery is required.",
                        expectedETag);
                }
    ...
    public async ValueTask ReplaceAsync(ReadOnlySequence<byte> value, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (!IsSameLogicalWal(walState.Value.ProviderState, expectedProviderState))
            {
                throw CreateInconsistentWalStateException(
                    "S3 journal WAL was replaced while publishing a checkpoint; recovery is required.",
                    expectedWalETag);
            }
            ...
            while (true)
            {
                ...
                var reclaimCheckpointOnFailure = false;
                try
                {
                    for (var attempt = 0; ; attempt++)
                    {
                        ...
                        catch (AmazonS3Exception exception) when (IsWalMutationConflict(exception))
                        {
                            ...
                            reclaimCheckpointOnFailure = exception.StatusCode is
    ...
    private async ValueTask AppendWithConditionalRewriteAsync(
        ...
        CancellationToken cancellationToken)
    {
        if (expectedProviderState.ContentLength > MaxSinglePutObjectBytes - value.Length)
        {
            throw new InvalidOperationException(
                $"S3-compatible journal append rewrites cannot produce an object larger than 5 GB. " +
        ...
        {
            throw CreateInconsistentWalStateException(
                "S3 journal WAL was replaced while appending; recovery is required.",
                expectedETag);
        }
    ...
    private async ValueTask EnsureWalAsync(CancellationToken cancellationToken)
    {
        ...
        while (!WalExists)
        {
            ...
            catch (AmazonS3Exception exception) when (IsObjectAlreadyExists(exception))
            {
                ...
                IsConditionalRequestConflict(exception)
                && conflictAttempt < MaxConditionalRequestConflictRetries)
            {
                conflictAttempt++;
                var current = await TryLoadWalStateAsync(expectedETag: null, cancellationToken).ConfigureAwait(false);
                if (current is not null)
        ...
        bool updateCache = true)
    {
        ...
        var partsCount = await GetWalPartsCountAsync(
            ...
            cancellationToken).ConfigureAwait(false);
        var walState = CreateWalState(walProperties, partsCount);
        if (updateCache)
        {
    ...
    private async ValueTask DelayForRetryAsync(int attempt, CancellationToken cancellationToken)
    {
        var delay = GetRetryDelay(
            _shared.Options.MetadataOnlyConflictInitialBackoff,
            _shared.Options.MetadataOnlyConflictMaxBackoff,
            attempt);
    ...
    internal static TimeSpan GetRetryDelay(TimeSpan initial, TimeSpan max, int attempt)
    {
        ...
        var multiplier = 1L << Math.Min(Math.Max(attempt, 0), 16);
        var cappedTicks = initial.Ticks > max.Ticks / multiplier
            ? max.Ticks
        ...
        CancellationToken cancellationToken)
    {
        for (var attempt = 0; ; attempt++)
        {
            ...
            catch (AmazonS3Exception exception) when (IsObjectAlreadyExists(exception))
            {
                ...
                && attempt < MaxConditionalRequestConflictRetries)
            {
                if (await ObjectExistsAsync(checkpointName, cancellationToken).ConfigureAwait(false))
                {
                    return false;
                }
    ...
    private Dictionary<string, string> CreateWalMetadata(WalManifest manifest)
    {
        ...
        if (manifest.WalOffset > 0)
        {
            metadata[CheckpointOffsetMetadataKey] = manifest.WalOffset.ToString(CultureInfo.InvariantCulture);
        }
    ...
    private static WalManifest CreateWalManifest(IDictionary<string, string>? metadata)
    {
        ...
        if (metadata is not null
            && metadata.TryGetValue(CheckpointOffsetMetadataKey, out var checkpointOffsetValue)
            && checkpointOffsetValue is { Length: > 0 }
            && (!long.TryParse(checkpointOffsetValue, NumberStyles.None, CultureInfo.InvariantCulture, out walOffset) || walOffset < 0))
        {
            throw new InvalidOperationException(
                $"S3 journal checkpoint offset metadata is invalid: '{checkpointOffsetValue}'.");
        }

        CheckpointReference? checkpoint = metadata is not null
            && metadata.TryGetValue(CheckpointMetadataKey, out var checkpointName)
            && checkpointName is { Length: > 0 }
                ? new CheckpointReference(checkpointName, walOffset)
                : null;
        return new WalManifest(fileMetadata, checkpoint, walOffset, generation, metadataVersion);
    }

    private static WalState CreateWalState(GetObjectMetadataResponse properties, int? partsCount = null)
    {
        var metadata = CopyMetadata(properties.Metadata);
        var manifest = CreateWalManifest(metadata);
        return new WalState(
            ...
            manifest,
            CreateWalProviderState(manifest, properties.ContentLength, partsCount ?? properties.PartsCount));
    }
    ...
    private async ValueTask<int?> GetWalPartsCountAsync(
        string expectedETag,
        long contentLength,
        int? reportedPartsCount,
        CancellationToken cancellationToken)
    {
        if (contentLength == 0)
        {
        ...
        {
            return MaxAppendPartsPerObject;
        }
    }
    ...
    private static async ValueTask SkipStreamAsync(Stream stream, long count, CancellationToken cancellationToken)
    {
        ...
        var buffer = new byte[(int)Math.Min(81920, count)];
        var remaining = count;
        while (remaining > 0)
        {
    ...
    private sealed record WalManifest(IJournalMetadata Metadata, CheckpointReference? Checkpoint, long WalOffset, string? Generation, string? MetadataVersion);
    ...
    internal sealed class S3JournalStorageShared
    {
        public S3JournalStorageShared(
            ...
            S3JournalStorageInstruments instruments,
