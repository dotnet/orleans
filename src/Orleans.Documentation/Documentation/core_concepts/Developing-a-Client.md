...
More information is in the Client Configuration guide.
...
Making calls to grain from a client is really no different from making such calls from within grain code.
The same `GetGrain<T>(key)` method, where `T` is the target grain interface, is used in both cases to obtain grain references.
...
A call to a grain method returns a `Task` or a`Task<T>` as required by the grain interface rules.
...
Observers is one such mechanism that enables exposing client side objects as grain-like targets to get invoked by grains.
