...
public static class SiloBuilderReminderConcurrencyExtensions
{
    ...
    private static void LogConfiguredTier(ILogger logger, string tier, ThrottleConfig config)
    {
        ...
        logger.LogInformation(
            ...
            tier,
            config.Concurrency is null ? "disabled" : $"maxConcurrent={config.Concurrency.MaxConcurrent}/{config.Concurrency.BlockMode}",
            config.Rate is null ? "disabled" : $"permitsPerSecond={config.Rate.PermitsPerSecond:0.##}/burstSize={config.Rate.BurstSize}/{config.Rate.BlockMode}",
            config.Overload is null ? "disabled" : $"{config.Overload.BlockMode}/pollInterval={config.Overload.PollInterval}",
            config.SlowStart is null ? "disabled" : $"initial={config.SlowStart.InitialCapacity}/interval={config.SlowStart.Interval}/{config.SlowStart.BlockMode}");
    }
}
