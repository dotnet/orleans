...
/// </summary>
internal sealed class InMemoryJobQueue : IAsyncEnumerable<IDurableJobRun>
{
    ...
    /// </remarks>
    public void RetryJobLater(IDurableJobRun jobContext, DateTimeOffset newDueTime)
    {
    ...
    /// <returns>
    /// An async enumerator that returns <see cref="IDurableJobRun"/> instances for jobs that are due.
    /// The enumerator checks for due jobs every second and terminates when the queue is marked complete and empty.
    /// </returns>
    public async IAsyncEnumerator<IDurableJobRun> GetAsyncEnumerator(CancellationToken cancellationToken = default)
    {
        ...
        while (true)
        {
            ...
            if (bucketToProcess is not null)
            {
                ...
                foreach (var (job, dequeueCount) in bucketToProcess.Jobs.ToList())
                {
                    ...
                    if (shouldYield)
                    {
                        yield return new DurableJobRun(job, Guid.NewGuid().ToString(), dequeueCount + 1);
                    }
                }
