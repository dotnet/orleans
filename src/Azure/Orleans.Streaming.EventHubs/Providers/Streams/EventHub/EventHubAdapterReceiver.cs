...
namespace Orleans.Streaming.EventHubs
{
    ...
    internal class EventHubAdapterReceiver : IQueueAdapterReceiver, IQueueCache
    {
        ...
        public int GetMaxAddCount() => this.flowController.GetMaxAddCount();
        ...
        public IQueueCacheCursor GetCacheCursor(StreamId streamId, StreamSequenceToken token) => new Cursor(this.cache, streamId, token);

        public bool IsUnderPressure() => this.GetMaxAddCount() <= 0;
        ...
        internal void ConfigureDataGeneratorForStream(StreamId streamId) => (this.receiver as EventHubPartitionGeneratorReceiver)?.ConfigureDataGeneratorForStream(streamId);
