...
namespace Orleans.Storage.Migration.AzureStorage
{
    ...
    public class MigrationAzureBlobGrainStorage : IGrainStorage, ILifecycleParticipant<ISiloLifecycle>
    {
        ...
        {
            var grainId = this.grainReferenceExtractor.GetGrainId(grainReference);
            ...
            try
            {
                ...
                var loadedState = this.ConvertFromStorageFormat(contents);
        ...
        private static string GetBlobName(string grainType, string grainId) => $"{grainType}-{grainId}.json";
        ...
        private async Task WriteStateAndCreateContainerIfNotExists(string grainType, string grainId, IGrainState grainState, BinaryData contents, string mimeType, BlobClient blob)
        {
            try
            {
                var conditions = string.IsNullOrEmpty(grainState.ETag)
        ...
        {
            lifecycle.Subscribe(OptionFormattingUtilities.Name<MigrationAzureBlobGrainStorage>(this.name), this.options.InitStage, Init);
        }
    ...
    public static class MigrationAzureBlobGrainStorageFactory
    {
        public static IGrainStorage Create(IServiceProvider services, string name)
        {
            ...
            return ActivatorUtilities.CreateInstance<MigrationAzureBlobGrainStorage>(services, name, optionsMonitor.Get(name));
        }
    }
}
