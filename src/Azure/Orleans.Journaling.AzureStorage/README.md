...
Both Azure providers implement `IJournalStorageCatalog.ListAsync`, returning identities incrementally in service traversal order, not guaranteed journal-id order. Set `ListOptions.Prefix` to enumerate an exact journal id and its descendants, and `MaxId` for an inclusive ordinal upper bound (default: unlimited). Both constraints apply and are snapshotted when enumeration starts. Consumers requiring due order must sort selected ids using `StringComparer.Ordinal`. The provider handles service continuations internally and yields identities from the current page before fetching the next page.

The Blob catalog scans the configured `ContainerName` and interprets append blobs named `<journalId>/wal` as journal identities. This traversal applies equally when a custom naming delegate or container factory produces the same entries. Each internal page requests up to 5000 blobs with a native name prefix, including checkpoints and other entries before filtering. For bounded, non-default prefixes, a separate one-item-preferring listing checks the exact `<Prefix>/wal` name before traversing strict descendants using the native `<Prefix>/` prefix. This preserves the exact prefix journal without letting its WAL name prevent a timestamp-range cutoff. Azure's flat List Blobs API returns names in lexical order, permitting termination after a safe ASCII raw WAL-name bound. The bound includes the maximum journal's WAL and conservatively accounts for shorter descendant ids whose `/wal` suffix sorts later, while fixed-width timestamp bounds can stop before fetching the future tail. Non-ASCII bounds are filtered without early termination to avoid relying on UTF-8 versus UTF-16 ordering.

Table enumeration supports custom partition mappings through the canonical journal id stored in each header. It requests up to 1000 header rows per service page; Table Storage determines the internal scan work required by that query. Prefixed queries combine canonical `JournalId` prefix/range filters with a reversible legacy `PartitionKey` prefix filter. A legacy upper bound is also safe when `MaxId` is a descendant whose remaining suffix consists only of unreserved ASCII characters, including fixed-width timestamps followed by `~`. Arbitrary URI escaping does not preserve ordinal order, so other legacy bounds are filtered after retrieval. Queries without a prefix retain the existing header scan to preserve legacy headers without a `JournalId` property. The legacy fallback can over-select canonical headers; both constraints are checked again before yielding. One enumerator advance can cross multiple empty or filtered pages.
