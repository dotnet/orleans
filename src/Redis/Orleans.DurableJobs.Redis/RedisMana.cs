...
{
    internal sealed class RedisJobShardManagerV2 : JobShardManager
    {
        ...
        private readonly ConcurrentDictionary<string, RedisJobShardVGh> _jobShardCache = new();
        ...
        public override async Task<List<IJobShard>> AssignJobShardsAsync(DateTimeOffset maxShardStartTime, CancellationToken cancellationToken)
        {
            ...
            foreach (var shardId in shardIds)
            {
                ...
                var shard = new RedisJobShardVGh(shardId, minDue, maxDue, _multiplexer!, _options.ShardOptions ?? new RedisJobShardOptions(), _loggerFactory.CreateLogger<RedisJobShard>());
                try
                {
        ...
        public override async Task<IJobShard> CreateShardAsync(DateTimeOffset minDueTime, DateTimeOffset maxDueTime, IDictionary<string, string> metadata, CancellationToken cancellationToken)
        {
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    var shard = new RedisJobShardVGh(shardId, minDueTime, maxDueTime, _multiplexer!, _options.ShardOptions ?? new RedisJobShardOptions(), _loggerFactory.CreateLogger<RedisJobShard>());
                    await shard.InitializeAsync(cancellationToken).ConfigureAwait(false);
                    _jobShardCache[shardId] = shard;
    ...
    internal sealed class RedisJobShardManagerOptions
    {
