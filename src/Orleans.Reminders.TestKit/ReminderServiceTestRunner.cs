...
public abstract class ReminderServiceTestRunner
{
    ...
    public virtual async Task ReminderService_RegisterLookupEnumerateAndUnregister()
    {
        ...
        var persisted = await ReminderTable.ReadRow(grainId, Name);
        var names = await grain.GetReminderNamesAsync();
        ...
        var afterRemoval = await ReminderTable.ReadRow(grainId, Name);
        var namesAfterRemoval = await grain.GetReminderNamesAsync();
    ...
    public virtual async Task ReminderService_UpdateReplacesScheduleAndETagWithoutDuplicate()
    {
        ...
        var original = await ReminderTable.ReadRow(grainId, Name);
