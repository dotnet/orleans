    ...
    OrleansManager setSelectorStrategy <VersionSelectorStrategy (string)> [<grain interface type code (int>])");
        ...
        private static void GrainReport(string[] args)
        {
            ...
            foreach (var silo in silos)
            {
                ...
                try
                {
                    var siloControl = client.GetSystemTarget<ISiloControl>(GrainConstants.SiloControlId, silo);
        ...
        private static void UnregisterGrain(string[] args)
        {
            ...
            var directory = client.GetSystemTarget<IRemoteGrainDirectory>(GrainConstants.DirectoryServiceId, silo);
