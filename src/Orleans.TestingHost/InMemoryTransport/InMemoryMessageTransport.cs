            ...
            while (!_connectionClosingCts.IsCancellationRequested)
            {
                ...
                while (TryDequeue(out request))
                {
                    ...
                    while (true)
                    {
                        ...
                        pipeCompleted = readResult.IsCompleted;
    ...
    private async Task ProcessWrites()
    {
        ...
        try
        {
            ...
            while (!_connectionClosingCts.IsCancellationRequested)
            {
                ...
                while (error is null && processingRequests.Count < SoftBatchMax && requests.TryDequeue(out var request))
                {
                    ...
                    foreach (var buffer in slice.MemorySegments)
                    {
                        ...
                        {
                            error = new ConnectionClosedException("The in-memory transport peer closed before all queued data was written.");
                            break;
                        }
                    }
                }
