...
{
    private readonly CronSchedulePattern _expression;

    private ReminderCronExpression(string expressionText, CronSchedulePattern expression)
    {
    ...
    public static ReminderCronExpression Parse(string expression)
    {
        ...
        var parsed = ReminderCronParser.Parse(expression);
    ...
    public static bool TryParse(string expression, out ReminderCronExpression? cronExpression)
    {
        ...
        catch (ReminderCronParseException)
        {
            cronExpression = null;
            return false;
        }
    ...
    /// <summary>
    /// Gets the next occurrence in UTC after the instant represented by <paramref name="from"/>.
    /// The input offset does not change the schedule's UTC calendar.
    /// </summary>
    ...
    /// <summary>
    /// Gets occurrences as UTC timestamps between two instants, which may have different offsets.
    /// </summary>
    public IEnumerable<DateTimeOffset> GetOccurrences(DateTimeOffset from, DateTimeOffset to, bool fromInclusive = true, bool toInclusive = false)
        => ToDateTimeOffsets(GetOccurrences(from.UtcDateTime, to.UtcDateTime, fromInclusive, toInclusive));

    internal static IEnumerable<DateTimeOffset> ToDateTimeOffsets(IEnumerable<DateTime> occurrences)
    {
        foreach (var occurrence in occurrences)
        {
            yield return new DateTimeOffset(occurrence);
        }
    }
