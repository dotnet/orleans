...
public sealed class ReminderEntry
{
    ...
    /// </summary>
    [Id(8)]
    public DurableJobPriority Priority { get; set; } = DurableJobPriority.Normal;
    ...
    internal IGrainReminder ToIGrainReminder() => new ReminderData(GrainId, ReminderName, ETag, ScheduleId, CronExpression, Priority, Action, CronTimeZoneId);
}
...
internal sealed class ReminderData : IGrainReminder
{
    ...
    public override string ToString() => $"<IOrleansReminder: GrainId={GrainId} ReminderName={ReminderName} RegistrationId={RegistrationId} ETag={ETag} CronExpression={CronExpression} CronTimeZone={CronTimeZone} Priority={Priority} Action={Action}>";
}
