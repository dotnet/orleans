...
internal sealed class RegisterReminderActivationConfiguratorProvider : IConfigureGrainContextProvider
{
    ...
    private RegisterReminderActivationConfigurator? ResolveConfigurator(GrainType grainType)
    {
        ...
        if (!typeof(global::Orleans.AdvancedReminders.IRemindable).IsAssignableFrom(grainClass))
        {
            logger.LogWarning(
                "Ignoring [RegisterReminder] declarations for grain type {GrainType} because it does not implement {Remindable}.",
                grainClass.FullName,
                typeof(global::Orleans.AdvancedReminders.IRemindable).FullName);
            return null;
        }
        ...
        ILogger logger) : ILifecycleObserver
    {
        ...
        {
            var reminderService = grainContext.ActivationServices.GetService<IAttributeReminderService>();
            if (reminderService is null)
            {
                logger.LogWarning(
                    ...
                    nameof(IAttributeReminderService));
                return;
            }
            ...
            foreach (var registration in registrations)
            {
                ...
                try
                {
                    ...
                    {
                        var schedule = ReminderSchedule.Cron(registration.Cron);
                        await reminderService.ReconcileReminder(
                            grainContext.GrainId,
                            registration.Name,
                            schedule,
                            registration.Priority,
                            registration.Action,
                            AttributeReminderRegistration.GetDeclarationId(schedule, registration.Priority, registration.Action));
                    }
                    else if (registration.Due is { } due && registration.Period is { } period)
                    {
                        var schedule = ReminderSchedule.Interval(due, period);
                        await reminderService.ReconcileReminder(
                            grainContext.GrainId,
                            registration.Name,
                            schedule,
