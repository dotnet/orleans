        ...
        MissedReminderAction action)
    {
        ...
        var declaration = schedule.Kind switch
        {
            ReminderScheduleKind.Interval => string.Create(
                ...
                $"v1|interval|{schedule.DueTime?.Ticks}|{schedule.DueAtUtc?.Ticks}|{schedule.Period?.Ticks}|{schedule.IsOneShot}|{(int)action}"),
            ReminderScheduleKind.Cron => string.Create(
                CultureInfo.InvariantCulture,
                $"v1|cron|{schedule.CronExpression?.Trim()}|{schedule.CronTimeZoneId?.Trim()}|{(int)action}"),
