...
Orleans Journaling persists durable state machine changes as ordered journal data which can be replayed to recover in-memory durable collections and values.
...
**Journal Entry**:
The logical item inside a **Journal Batch**, consisting of a state machine id and one durable operation payload.
_Avoid_: Record

**Control Journal Entry**:
A **Journal Entry** with a **Runtime State Machine Id** whose payload carries journaling infrastructure metadata instead of an application **Durable Operation**.
...
_Avoid_: State Machine Storage, storage codec
...
_Avoid_: Journal Id, State Machine Storage Id
...
**Durable Operation**:
A state-machine-specific change encoded inside a **Journal Entry**, such as list add, dictionary set, or queue dequeue.
...
A durable state machine which appears in persisted journal data but is not currently registered by user code.
...
The runtime component which binds durable state machines to one **Journal Storage**, replays **Journal Entries**, and flushes pending **Durable Operations**.
_Avoid_: Grain context, storage provider

**State Machine Manager Handle**:
An owned lifecycle wrapper for a **State Machine Manager** created outside grain activation.
_Avoid_: Synthetic grain context

**Transactional Journaled Grain**:
A durable grain mode whose normal requests are method-bracketed transactions over all durable state machines in the grain journal.
...
**Abort-by-Recovery**:
A transaction abort strategy which resets durable state machines and replays stored **Journal Entries** while omitting the aborted transaction region.
...
- A **Journal Entry** belongs to exactly one durable state machine.
- A **Journal Entry** with a **Runtime State Machine Id** belongs to Orleans journaling infrastructure, not user code.
- A **Control Journal Entry** is still framed and ordered like any other **Journal Entry**; its meaning is owned by the manager for that **Runtime State Machine Id**.
...
- A **Compaction Request** is raised by **Journal Storage** and honored by the **State Machine Manager**.
...
- A grain has one active **Journal Format** key. All runtime and user durable state machines managed by that grain's manager must use that same key.
...
- Durable state machine services are unkeyed within a grain and resolve codecs using the grain's active **Journal Format** key.
- A **State Machine Manager** owns recovery and write ordering for one **Journal Storage**.
- A **State Machine Manager Handle** owns shutdown and disposal for a **State Machine Manager** created by services such as DurableJobs.
...
- A **State Machine Manager** may flush **Journal Entries** from multiple callers in one **Journal Batch** when each caller waits for the flush containing its changes.
...
- A **Retired State Machine** can temporarily retain copied **Journal Entries** so its data is preserved across recovery and compaction.
...
- Transaction abort in journaling means **Abort-by-Recovery**, not per-state-machine undo or disposal of a pending **Journal Entry** writer.
...
- "storage identity" in new API discussions means **Journal Storage Id**, not **Journal Id** or **State Machine Storage Id**.
...
- DurableJobs creates **State Machine Managers** through **State Machine Manager Handles**, not synthetic grain contexts.
