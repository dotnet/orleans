...
public sealed class VolatileStateMachineStorageProvider : IStateMachineStorageProvider
{
    ...
    public VolatileStateMachineStorageProvider() : this(StateMachineLogFormatKeys.OrleansBinary)
    {
    }

    /// <summary>
    /// Initializes a new instance of the <see cref="VolatileStateMachineStorageProvider"/> class.
    /// </summary>
    /// <param name="logFormatKey">The default state machine log format key.</param>
    public VolatileStateMachineStorageProvider(string logFormatKey) : this(logFormatKey, logFormatKeySelector: null)
    {
    }
    ...
    public VolatileStateMachineStorageProvider(string logFormatKey, Func<GrainType, string>? logFormatKeySelector)
    {
    ...
    public IStateMachineStorage Create(IGrainContext grainContext)
    {
        ...
        return _storage.GetOrAdd(grainContext.GrainId, _ => new VolatileStateMachineStorage(logFormatKey));
    }
...
public sealed class VolatileStateMachineStorage : IStateMachineStorage
{
    ...
    public VolatileStateMachineStorage() : this(StateMachineLogFormatKeys.OrleansBinary)
    {
    }
