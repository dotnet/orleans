...
/// <summary>
/// An <see cref="ILogValueCodec{T}"/> implementation that wraps an Orleans <see cref="IFieldCodec{T}"/>
...
/// <remarks>
/// This adapter bridges the new <see cref="ILogValueCodec{T}"/> abstraction to the existing Orleans
/// serialization infrastructure, preserving backward compatibility with data written using the
/// legacy Orleans binary format.
/// </remarks>
internal sealed class OrleansLogValueCodec<T>(IFieldCodec<T> fieldCodec, SerializerSessionPool sessionPool) : ILogValueCodec<T>
{
    /// <inheritdoc/>
