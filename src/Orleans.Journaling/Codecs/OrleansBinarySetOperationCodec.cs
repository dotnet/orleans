    ...
    ILogValueCodec<T> codec) : IDurableSetOperationCodec<T>
{
    ...
    public void WriteRemove(T item, LogStreamWriter writer) =>
        DurableOperationCodecWriter.Write(writer, output => WriteRemovePayload(item, output));
    ...
    private void ApplySnapshot(ReadOnlySequence<byte> remaining, IDurableSetOperationHandler<T> consumer)
    {
