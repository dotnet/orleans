...
internal sealed class BlockedBloomFilter : IAnchoredGrainsFilter
{
    ...
    private static readonly double[] _coefficients = {
    ...
    /// <param name="capacity">The capcity to store.</param>
    ...
    {
        ulong hash = XxHash3.HashToUInt64(id.Key.AsSpan(), id.GetUniformHashCode());
        int index = GetBlockIndex(hash, _blocks); // important to get index before rotating the hash
        ...
        int mask1 = ComputeMask1(hash);
        int mask2 = ComputeMask2(hash);
    ...
    public bool Contains(GrainId id)
    {
        ...
        int block1 = _filter[index];
        int block2 = _filter[index + 1];
