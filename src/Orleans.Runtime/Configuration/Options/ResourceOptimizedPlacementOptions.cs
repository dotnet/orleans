...
public sealed class ResourceOptimizedPlacementOptions
{
    ...
    /// <item>When this value is 1, then the policy will always favor the local silo, regardless of its relative utilization score. This policy essentially becomes equivalent to <see cref="PreferLocalPlacement"/>.</item>
    /// </list>
    /// </summary>
    /// <remarks><i>
    ...
    (IOptions<ResourceOptimizedPlacementOptions> options) : IConfigurationValidator
{
    ...
    public void ValidateConfiguration()
    {
        ...
        if (_options.CpuUsageWeight +
            ...
            _options.PhysicalMemoryWeight == 0f)
        {
            throw new OrleansConfigurationException(
                $"The sum of all the weights in {nameof(ResourceOptimizedPlacementOptions)} must be a value greater than 0");
        }
