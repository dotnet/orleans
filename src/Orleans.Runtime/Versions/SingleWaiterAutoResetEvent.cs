    ...
    public struct ManualResetValueTaskSourceCore<TResult>
    {
        ...
        public void OnCompleted(Action<object> continuation, object state, short token, ValueTaskSourceOnCompletedFlags flags)
        {
            ...
            if (oldContinuation != null)
            {
                ...
                {
                    case null or ThreadPoolSynchronizationContext:
                        ...
                        break;
                    case WorkItemGroup wig:
                        wig.Enqueue(continuation, state);
                        break;
        ...
        private void SignalCompletion()
        {
            ...
            {
                if (_capturedContext is not null)
                {
                    if (_capturedContext is OrleansSynchronizationContext osc)
                    {
                        if (osc is WorkItemGroup wig)
                        {
                            wig.Enqueue(_continuation, _continuationState);
                            return;
                        }
                        else if (osc is ThreadPoolSynchronizationContext)
                        {
                    ...
                    InvokeOnUnknownCapturedContext();
                }
                else
                {
        ...
        private void InvokeContinuationWithContext()
        {
            ...
            if (_capturedContext is not null)
            {
                try
                {
                    ...
                    if (_capturedContext is WorkItemGroup wig)
                    {
                        wig.Enqueue(_continuation, _continuationState);
                        return;
                    }

                    if (_capturedContext is ThreadPoolSynchronizationContext)
                    {
                        ThreadPool.QueueUserWorkItem(_continuation, _continuationState, preferLocal: true);
                        return;
                    }
            ...
            {
                InvokeWithoutCapturedContext(currentContext);
                return;
            }
        }

        private readonly void InvokeWithoutCapturedContext(ExecutionContext currentContext)
        {
            if (RunContinuationsAsynchronously)
            {
                try
        ...
        private void InvokeOnUnknownCapturedContext()
        {
            switch (_capturedContext)
            {
                case WorkItemGroup wig:
