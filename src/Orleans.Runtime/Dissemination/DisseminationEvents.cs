...
internal static class DisseminationEvents
{
    ...
    public const string SendGateEventName = "Dissemination.SendGate";
    public const string NamespacePendingLimitReason = "namespace_pending_limit";

    public static void EmitSendGate(SiloAddress localSilo, SiloAddress peer, string kind, string stage)
    {
        if (Listener.IsEnabled(SendGateEventName))
        {
            Listener.Write(SendGateEventName, new DisseminationSendGateEvent
            {
                LocalSilo = localSilo,
                Peer = peer,
                Kind = kind,
...
internal sealed class DisseminationSendGateEvent
{
    public required SiloAddress LocalSilo { get; init; }
