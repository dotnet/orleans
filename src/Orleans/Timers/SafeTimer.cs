...
namespace Orleans.Runtime
{
    ...
    internal class SafeTimer : IDisposable
    {
        ...
        public SafeTimer(ILoggerFactory loggerFactory, TimerCallback callback, object state)
        {
            callbackFunc = callback;
            safeTimerBase = new SafeTimerBase(loggerFactory, callbackFunc, state);
        }

        public SafeTimer(ILoggerFactory loggerFactory, TimerCallback callback, object state, TimeSpan dueTime, TimeSpan period)
        {
            callbackFunc = callback;
            safeTimerBase = new SafeTimerBase(loggerFactory, callbackFunc, state, dueTime, period);
        }
