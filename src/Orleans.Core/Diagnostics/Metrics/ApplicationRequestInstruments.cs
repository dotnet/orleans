...
internal static class ApplicationRequestInstruments
{
    ...
    private static readonly long[] AppRequestsLatencyHistogramBuckets = new double[] { 0.1, 1, 2, 4, 6, 8, 10, 50, 100, 200, 400, 800, 1_000, 1_500, 2_000, 5_000, 10_000, 15_000 }
        .Select(x => (long)(x * TimeSpan.TicksPerMillisecond))
        .ToArray();
    private static readonly HistogramAggregator AppRequestsLatencyHistogramAggregator = new(AppRequestsLatencyHistogramBuckets, Array.Empty<KeyValuePair<string, object>>(), value => new ("duration", $"{value / (double)TimeSpan.TicksPerMillisecond}ms"));
