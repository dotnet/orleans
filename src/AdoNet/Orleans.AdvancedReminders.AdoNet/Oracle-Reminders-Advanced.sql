...
/
CREATE INDEX IX_ADV_REM_NEXTDUE ON ORLEANSADVANCEDREMINDERSTABLE(SERVICEID, NEXTDUEUTC);
/
...
CREATE OR REPLACE FUNCTION UpsertAdvancedReminderRow(PARAM_SERVICEID IN NVARCHAR2, PARAM_GRAINHASH IN INT, PARAM_GRAINID IN VARCHAR2, PARAM_REMINDERNAME IN NVARCHAR2,
                                                ...
                                                PARAM_ACTION IN NUMBER, PARAM_VERSION IN NUMBER)
RETURN NUMBER IS
    ...
    IF PARAM_VERSION = -1 THEN
      BEGIN
        INSERT INTO ORLEANSADVANCEDREMINDERSTABLE
          (ServiceId, GrainId, ReminderName, StartTime, Period, CronExpression, CronTimeZoneId, NextDueUtc, LastFireUtc, ScheduleId, JobId, JobShardId, Action, GrainHash, Version)
        VALUES
          (PARAM_SERVICEID, PARAM_GRAINID, PARAM_REMINDERNAME, PARAM_STARTTIME, PARAM_PERIOD, PARAM_CRONEXPRESSION, PARAM_CRONTIMEZONEID, PARAM_NEXTDUEUTC, PARAM_LASTFIREUTC, PARAM_SCHEDULEID, PARAM_JOBID, PARAM_JOBSHARDID, PARAM_ACTION, PARAM_GRAINHASH, 0);
        currentVersion := 0;
      EXCEPTION
...
(
    ...
    SELECT Version
    FROM (
      SELECT UpsertAdvancedReminderRow(:ServiceId, :GrainHash, :GrainId, :ReminderName, :StartTime, :Period, :CronExpression, :CronTimeZoneId, :NextDueUtc, :LastFireUtc, :ScheduleId, :JobId, :JobShardId, :Action, :Version) AS Version
      FROM DUAL
    )
...
(
    ...
    SELECT GrainId, ReminderName, StartTime, Period, CronExpression, CronTimeZoneId, NextDueUtc, LastFireUtc, ScheduleId, JobId, JobShardId, Action, Version
