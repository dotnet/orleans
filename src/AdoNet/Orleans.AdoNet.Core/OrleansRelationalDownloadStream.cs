...
internal class OrleansRelationalDownloadStream(DbDataReader reader, int ordinal) : Stream
{
    /// <summary>
    ...
    /// </summary>
    private readonly long _totalBytes = reader.GetBytes(ordinal, 0, null, 0, 0);
    ...
    public override int Read(byte[] buffer, int offset, int count)
    {
        ...
        try
        {
            ...
            if (length > 0)
            {
                bytesRead = reader.GetBytes(ordinal, _position, buffer, offset, length);
                _position += bytesRead;
            }
