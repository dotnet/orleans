...
internal static class RelationalStorageExtensions
{
        ...
        if (parameters.Any())
        {
            ...
            if (onlyOnceColumns != null && onlyOnceColumns.Any())
            {
                ...
                for (var i = 0; i < onlyOnceProperties.Length; ++i)
                {
                    ...
                    if (useSqlParams)
                    {
                        var parameterName = string.Format("@{0}", (nameMap?.TryGetValue(onlyOnceProperties[i].Name, out var parameter) ?? false) ? parameter : onlyOnceProperties[i].Name);
    ...
    {
        return storage.SupportsStreamNatively()
            ? reader.GetStream(ordinal)
            : new OrleansRelationalDownloadStream(reader, ordinal);
    }
}