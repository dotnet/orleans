...
namespace Orleans.Runtime.MembershipService
{
    public class AdoNetClusteringTable: IMembershipTable
    {
        ...
        {
            if (logger.IsEnabled(LogLevel.Trace)) logger.Trace(string.Format("IMembershipTable.CleanupDefunctSiloEntries called with beforeDate {0} and clusterId {1}.", beforeDate, this.clusterId));
            try
            {
            ...
            {
                if (logger.IsEnabled(LogLevel.Debug)) logger.Debug("AdoNetClusteringTable.CleanupDefunctSiloEntries failed: {0}", ex);
                throw;
            }
        }
    }
