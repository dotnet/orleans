...
#endif
{
    ...
    internal static class RelationalStorageExtensions
    {
        ...
        public static Task<IEnumerable<TResult>> ReadAsync<TResult>(
            ...
            CancellationToken cancellationToken)
        {
            return storage.ReadAsync(query, parameterProvider, (record, i, cancellationToken) => Task.FromResult(selector(record)), cancellationToken: cancellationToken);
        }
        ...
        public static Task<IEnumerable<TResult>> ReadAsync<TResult>(this IRelationalStorage storage, string query, object parameters, CancellationToken cancellationToken = default)
        {
            ...
            }, (selector, resultSetCount, token) => Task.FromResult(selector.ReflectionSelector<TResult>()),
            cancellationToken: cancellationToken);
        }
        ...
        /// <summary>
        /// Uses <see cref="IRelationalStorage"/> with <see cref="DbExtensions.ReflectionSelector{TResult}(IDataRecord)"/>.
        /// </summary>
        /// <param name="storage">The storage to use.</param>
