...
namespace Orleans.Storage
{
    ...
    public class AdoNetGrainStorage: IGrainStorage, ILifecycleParticipant<ISiloLifecycle>
    {
        ...
        public async Task ClearStateAsync<T>(string grainType, GrainId grainId, IGrainState<T> grainState)
        {
            ...
            if(logger.IsEnabled(LogLevel.Trace))
            {
                logger.LogTrace(
                    ...
                    "Clearing grain state: ServiceId={ServiceId} ProviderName={Name} GrainId={GrainId} ETag={ETag}.",
                    serviceId,
                    name,
                    grainId,
                    grainState.ETag);
            ...
            try
            {
                var grainIdHash = HashPicker.PickHasher(serviceId, this.name, grainId, grainState).Hash(grainId.Key.Value.ToArray());
                var grainTypeHash = HashPicker.PickHasher(serviceId, this.name, grainId, grainState).Hash(grainId.Type.Value.Value.ToArray());
                var clearRecord = (await Storage.ReadAsync(CurrentOperationalQueries.ClearState, command =>
                {
                    ...
                    command.AddParameter("GrainIdKey", grainId.Key.ToString());
                    command.AddParameter("GrainTypeHash", grainTypeHash);
                    command.AddParameter("GrainTypeString", grainId.Type.ToString());
            ...
            catch(Exception ex)
            {
                logger.LogError(
                    ...
                    ex,
                    "Error clearing grain state: ServiceId={ServiceId} ProviderName={Name} GrainId={GrainId} ETag={ETag}.",
                    serviceId,
                    name,
                    grainId,
                    grainState.ETag);
            ...
            if(logger.IsEnabled(LogLevel.Trace))
            {
                logger.LogTrace(
                    ...
                    "Cleared grain state: ServiceId={ServiceId} ProviderName={Name} GrainId={GrainId} ETag={ETag}.",
                    serviceId,
                    name,
                    grainId,
                    grainState.ETag);
        ...
        public async Task ReadStateAsync<T>(string grainType, GrainId grainId, IGrainState<T> grainState)
        {
            ...
            if (logger.IsEnabled(LogLevel.Trace))
            {
                logger.LogTrace(
                    ...
                    "Reading grain state: ServiceId={ServiceId} ProviderName={Name} GrainId={GrainId} ETag={ETag}.",
                    serviceId,
                    name,
                    grainId,
                    grainState.ETag);
            ...
            try
            {
                ...
                var readRecords = (await Storage.ReadAsync(
                    ...
                    command =>
                    {
                        ...
                        command.AddParameter("GrainIdKey", grainId.Key.ToString());
                        command.AddParameter("GrainTypeHash", grainTypeHash);
                        command.AddParameter("GrainTypeString", grainId.Type.Value.ToString());
                        command.AddParameter("ServiceId", serviceId);
                    },
                ...
                if(state == null)
                {
                    logger.LogInformation(
                        ...
                        "Null grain state read (default will be instantiated): ServiceId={ServiceId} ProviderName={Name} GrainId={GrainId} ETag={ETag}.",
                        serviceId,
                        name,
                        grainId,
                        grainState.ETag);
                ...
                if (logger.IsEnabled(LogLevel.Trace))
                {
                    logger.LogTrace(
                        ...
                        "Read grain state: ServiceId={ServiceId} ProviderName={Name} GrainId={GrainId} ETag={ETag}.",
                        serviceId,
                        name,
                        grainId,
                        grainState.ETag);
            ...
            catch(Exception ex)
            {
                logger.LogError(
                    ...
                    ex,
                    "Error reading grain state: ServiceId={ServiceId} ProviderName={Name} GrainId={GrainId} ETag={ETag}.",
                    serviceId,
                    name,
                    grainId,
                    grainState.ETag);
        ...
        public async Task WriteStateAsync<T>(string grainType, GrainId grainId, IGrainState<T> grainState)
        {
            ...
            if (logger.IsEnabled(LogLevel.Trace))
            {
                logger.LogTrace(
                    ...
                    "Writing grain state: ServiceId={ServiceId} ProviderName={Name} GrainId={GrainId} ETag={ETag}.",
                    serviceId,
                    name,
                    grainId,
                    grainState.ETag);
            ...
            catch(Exception ex)
            {
                logger.LogError(
                    ...
                    ex,
                    "Error writing grain state: ServiceId={ServiceId} ProviderName={Name} GrainId={GrainId} ETag={ETag}.",
                    serviceId,
                    name,
                    grainId,
                    grainState.ETag);
            ...
            if (logger.IsEnabled(LogLevel.Trace))
            {
                logger.LogTrace(
                    ...
                    "Wrote grain state: ServiceId={ServiceId} ProviderName={Name} GrainId={GrainId} ETag={ETag}.",
                    serviceId,
                    name,
                    grainId,
                    grainState.ETag);
        ...
        private static InconsistentStateException CheckVersionInconsistency(string operation, string serviceId, string providerName, string storageVersion, string grainVersion, string grainId)
        {
            ...
            if(storageVersion == grainVersion || storageVersion == string.Empty)
            {
                ...
                return new InconsistentStateException($"Version conflict ({operation}): ServiceId={serviceId} ProviderName={providerName} GrainId={grainId} ETag={grainVersion}.");
            }
