...
5.  [AWS DynamoDB](https://aws.amazon.com/dynamodb/) - In this implementation we use the cluster Deployment ID as the Partition Key the Silo Identity (`ip-port-generation`) as the RangeKey making the record unity. The optmistic concurrency is made by the `ETag` attribute by making conditional writes on DynamoDB. The implementation logic is pretty similar to Azure Table Storage one since both services are very similar.
