...
3) **Portability and flexibility** - as part of Orleans's philosophy, we do not want to have a strong dependence on any particular technology, but rather a flexible design where different components can be switched with different implementations. This is indeed the purpuse that `MembershipTable` abstraction serves.
