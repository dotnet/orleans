...
An exception or event identifies where Orleans noticed a problem. Preserve the exception chain, structured <xref:Microsoft.Extensions.Logging.EventId>, category, trace ID, silo identity, cluster membership view, and the first occurrence, then correlate that evidence to determine the root cause.
...
| Reminder or timer runs late, twice, or misses an expected run | [Reminder and timer timing](#reminder-and-timer-timing) |
| Available memory falls, GC pauses rise, or a container is OOM-killed | [Memory pressure](#memory-pressure) |
| Shutdown exceeds the termination grace period | [Shutdown exceeds the termination budget](#shutdown-exceeds-the-termination-budget) |
...
**Likely causes:** client and silo cluster identities differ; the gateway provider returns an empty active-silo set; advertised gateway addresses resolve outside client-routable paths; DNS, firewall, NAT, or network policy blocks traffic; or TLS names, trust, or client-certificate requirements differ.
...
**Remedy:** correct identity/provider configuration or publish addresses which the client can route to. Replace an invalid certificate or trust configuration. Surface deterministic configuration failures immediately and bound startup retries.

**Prevent:** validate exact advertised endpoints from each deployment network and report readiness after the client has a usable gateway. Follow [Client connection failures](troubleshooting.md#client-cant-connect).
...
**Likely causes:** a silo stopped or lost network reachability; membership convergence is in progress; a stale activation address was used during churn; the destination is shutting down; or overload caused a rejection.
...
**Likely causes:** gateway load shedding is enabled and CPU or memory crossed its configured threshold; grain schedulers are saturated; a grain is hot; the thread pool or CPU is starved; or a storage/stream dependency runs below the offered rate.
...
**Remedy:** remove the bottleneck or reduce load. Increase a timeout when measured valid latency exceeds the budget. Treat the timed-out operation as having an unknown outcome and reconcile application state.
...
**Likely causes:** synchronous blocking, CPU-heavy work, an unbounded loop, external I/O with an unbounded wait or latency beyond the request budget, or an inter-grain call cycle. Reentrancy can progress a cycle, while synchronous blocking continues to occupy the scheduler.
...
**Confirm:** preserve the stored and expected version values, grain type/key, provider operation, and membership timeline. Determine whether more than one activation or an external process wrote the record. Log identifiers and versions while keeping grain state and secrets redacted.

**Remedy:** stop unsafe writers and reconcile the record according to application semantics. Bound retries to transient conflicts and preserve newer records. If an <xref:Orleans.Storage.InconsistentStateException> escapes the activation which raised it, Orleans deactivates that activation so a later call can reload state.

**Prevent:** keep one authoritative writer per state record, use provider concurrency tokens, route updates through that writer, and test recovery from ambiguous writes and activation churn.
...
**Confirm:** correlate provider request units, partition throttles, latency, and retry volume with Orleans storage operations and activation count. Compare affected grain keys through an approved low-cardinality partition classification and keep raw grain keys out of metrics.
...
**Signals:** <xref:Orleans.Serialization.CodecNotFoundException> reports `Could not find a codec for type ...` or `Could not find a copier for type ...`; another serialization exception reports an unsupported wire type or unknown/required type or field; a `KeyNotFoundException` reports `Could not find a base type serializer for type ...` or `Could not find a value serializer for type ...`; or activation/storage reads begin failing after deployment.

**Likely causes:** mixed versions use incompatible contracts or serializer identities; a serializable type or member identity changed; member IDs were reused or renumbered; a renamed type lacks a stable alias; required serializer registration differs; or persisted/queued data contains an old representation.
...
**Remedy:** roll back to a compatible version or deploy a reader which understands both representations, then migrate data deliberately. Preserve unreadable state and queues until recovery and retention requirements are established.
...
**Confirm:** compare membership views and timestamps from all silos with platform events, provider latency, pings, and advertised endpoints. For suspected split brain, determine whether isolated silos remained alive and could reach the membership store. Require corroborating membership, provider, network, and platform evidence before diagnosing a partition.
...
**Signals:** one grain type/key has high queue or latency while other silos have spare capacity; a small set of silos has disproportionate CPU or activations; or throughput remains flat as silos are added.
...
**Remedy:** partition the domain workload across keys, use hierarchical aggregation or batching, use stateless workers for suitable stateless work, or relax an unnecessarily restrictive placement rule. An ordinary stateful activation retains single-turn concurrency when moved; partitioning the key adds parallelism.

**Prevent:** load test skewed and celebrity-key traffic, define per-key limits, monitor placement/activation distribution through approved low-cardinality classifications, and keep raw grain keys private. See [How do I avoid hot grains?](../../resources/frequently-asked-questions.md#how-do-i-avoid-hot-grains).
...
**Remedy:** use reminders for work which must survive activation loss, restore provider health, reduce scheduler pressure, and make callbacks idempotent. Reconcile missed business work from durable application state and use callback count as a supporting signal.

**Prevent:** choose timers for activation-scoped scheduling, make reminder work idempotent, monitor provider health and scheduling delay, and use timers and reminders for best-effort scheduled callbacks. Reconcile time-critical work from durable state. <xref:Orleans.Hosting.ReminderOptions.MinimumReminderPeriod> defaults to one minute; registration rejects periods below the configured value. Configuring a lower value emits a production-suitability warning.

## Memory pressure

**Signals:** low `orleans-runtime-available-memory` in MB of GC-available memory, growing process working set or GC heap, long GC pauses, activation churn, or container OOM kills.
...
**Remedy:** reduce load, bound the responsible data structure or payload, and scale out when work partitions. Identify retained growth before raising a limit so the change addresses the source of memory pressure.
...
## Shutdown exceeds the termination budget
...
**Likely causes:** the platform budget is shorter than host shutdown; new work continues arriving; a lifecycle participant continues past cancellation; grain/dependency calls block; or stream/provider cleanup exceeds the deadline.

**Confirm:** compare application and platform deadlines, inspect final lifecycle logs and traces, and capture stacks before forced termination. Identify the lifecycle stage or hosted service which remained active.
...
**Prevent:** test shutdown under load and dependency failure, preserve rollout surge capacity, and monitor forced terminations. Follow [Shutdown exceeds its budget](troubleshooting.md#shutdown-doesnt-complete).
