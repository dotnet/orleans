...
Immutable membership metadata uses a companion prefix which older Orleans binaries don't enumerate:

- `<KvRootFolder>/orleans-metadata/<ClusterId>` when `KvRootFolder` is configured.
- `orleans-metadata/<ClusterId>` when `KvRootFolder` isn't configured.
...
The companion metadata prefix contains one `<silo-address>` key per silo. The first available metadata value for a silo instance remains authoritative. This separate prefix allows older silos to replace legacy membership registrations without deleting metadata.
...
- Give silo identities permissions for both the membership and companion metadata prefixes. Client identities need only the membership prefix for gateway discovery.
