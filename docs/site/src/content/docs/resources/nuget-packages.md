...
Use [classic reminders](../grains/timers-and-reminders.md#reminders) for recurring interval callbacks and [Advanced Reminders](../grains/advanced-reminders.md) for one-shot, interval, or cron callbacks with operational controls. Durable jobs schedule application-defined one-time work. Durable Messaging composes Durable Jobs with Journaling for recoverable, at-least-once grain messages. Grain timers are activation-scoped and use the core runtime rather than a provider package.
