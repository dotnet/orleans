...
builder.UseOrleans(siloBuilder =>
{
    ...
    internal static void ConfigureMissedLogging(string[] args)
    {
        // <configure_advanced_reminder_missed_logging>
HostApplicationBuilder builder = Host.CreateApplicationBuilder(args);
...
{
    internal static void QueryOffsetTimestamps()
    {
        // <query_advanced_reminder_cron_offsets>
        ReminderCronBuilder schedule = ReminderCronBuilder
            .DailyAt(2, 30)
            .InTimeZone("America/New_York");

        var from = new DateTimeOffset(2025, 3, 9, 1, 45, 0, TimeSpan.FromHours(-5));
        DateTimeOffset? next = schedule.GetNextOccurrence(from);
        // 02:30 is skipped that morning: next is 2025-03-09 07:00:00 +00:00.

        IEnumerable<DateTimeOffset> upcoming = schedule.GetOccurrences(
            from,
            from.AddDays(3),
            fromInclusive: true,
            toInclusive: false);
    ...
    Orleans.AdvancedReminders.IRemindable
{
    ...
    // <register_advanced_reminder_one_shot_delay_and_utc>
    public Task ScheduleExpirationAfterDelayAsync()
    {
        return this.RegisterOrUpdateAdvancedReminder(
            "expire",
    ...
    public Task ScheduleExpirationAtUtcAsync()
    {
        var dueAtUtc = new DateTime(2030, 4, 15, 16, 30, 0, DateTimeKind.Utc);
        return this.RegisterOrUpdateAdvancedReminder(
            "expire",
            dueAtUtc,
            MissedReminderAction.FireImmediately);
    ...
    string? continuationToken)
{
    ...
    // <query_advanced_reminder_missed>
internal static Task<ReminderManagementPage> ListCurrentlyMissedAsync(
    IGrainFactory grainFactory,
    string? continuationToken)
{
    return grainFactory.GetReminderManagementGrain().ListFilteredAsync(
        new ReminderQueryFilter
        {
