...
When <xref:Orleans.Configuration.DynamoDBStorageOptions.ServiceId> is empty, the keys are built from <xref:Orleans.Configuration.ClusterOptions.ServiceId>, as in the other grain storage providers. Earlier versions left it empty, so the keys start with an underscore, and a provider that finds such state in its table fails to start until <xref:Orleans.Configuration.DynamoDBStorageOptions.UseClusterServiceId> is set: `false` keeps the empty <xref:Orleans.Configuration.DynamoDBStorageOptions.ServiceId>, with a warning, and `true` uses <xref:Orleans.Configuration.ClusterOptions.ServiceId>. The check relies on the key format the provider records in the table since <xref:Orleans.Configuration.DynamoDBStorageOptions.UseClusterServiceId> was introduced, or on the table being empty when this version first starts: when upgrading from a version without that option, stop the silos that run that version first, or set <xref:Orleans.Configuration.DynamoDBStorageOptions.UseClusterServiceId> explicitly, since they keep writing keys with an empty <xref:Orleans.Configuration.DynamoDBStorageOptions.ServiceId> that this version would not find. State written before then stays under its old key unless <xref:Orleans.Configuration.DynamoDBStorageOptions.MigrateLegacyKeys> is also set: a grain with no state under the new key then reads it from the old one, and its next write moves it, in one transaction. Enable the migration once every silo runs a version that supports it, with the same options on every silo: a silo still writing the old keys, from an older version or with <xref:Orleans.Configuration.DynamoDBStorageOptions.UseClusterServiceId> set to `false`, is not protected from a migrating silo moving or deleting state it recreates, since an item recreated after <xref:Orleans.Configuration.DynamoDBStorageOptions.DeleteStateOnClear> starts again at ETag 0. Set <xref:Orleans.Configuration.DynamoDBStorageOptions.MigrateLegacyKeys> to `false` when every grain has been written, to spare the second read for grains that have no state. The provider records its choice in the table and keeps it while these options are left unset.
