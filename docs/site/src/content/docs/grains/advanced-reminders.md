...
1. The service creates one durable job for that next due time. The job carries the target grain ID, reminder name, and schedule ID; its job ID and shard ID are written back to the reminder definition.
...
1. For an interval or cron reminder, the service calculates one next occurrence, persists it, and creates one replacement job. For a one-shot reminder, there is no next occurrence, so the definition is removed.

The reminder table remains the source of truth. Execution is partitioned into owned time shards, and recovery and administration traverse reminder storage in hash ranges or pages instead of maintaining a permanent in-memory copy of every reminder. Durable Jobs caps each shard at 10,000 live jobs by default. When that cap is reached, the shard is closed to additions and drains normally while scheduling rolls over to a new shard for the same time bucket. Durable Jobs also dequeues a due bucket in batches of at most 1,024 and bounds journal mutation batches by operation count and estimated encoded size. This keeps the active working set tied to bounded shards while far-future intent stays durable.

For million-registration deployments, capacity-plan both stores. One million reminder definitions produce at most one million next-occurrence jobs, but they must be distributed across time buckets and shard journals. Adjust `DurableJobsOptions.ShardDuration`, `ShardStripeCount`, `MaxJobsPerShard`, `MaxPendingOperationsPerShard`, `MaxConcurrentJobsPerSilo`, `MaxShardBatchOperationCount`, and `MaxShardBatchSizeBytes` from measured due-time concentration and provider limits. The defaults prevent a single shard from retaining an unbounded million-job snapshot or ingress queue; they don't make a million callbacks due at the same instant free. Keep metadata small and load-test callback CPU, downstream I/O, storage throughput, recovery scans, and rolling restart behavior.
...
Durable Jobs preserves mutation order within one shard without putting unrelated shards behind one process-wide lock. Scheduling maps a job to a key composed of its time bucket and stripe. Concurrent callers which need the same missing key share one shard-creation operation, while different bucket or stripe keys can initialize concurrently. Canceling one caller's wait doesn't cancel that shared creation for the remaining callers; silo shutdown does.
...
During one continuation sequence, the management grain retains only the current hash bucket and reuses it across pages, then releases it when the sequence completes. This avoids rereading and deserializing the same provider range for every page while keeping memory independent of the total table size. A pathologically skewed bucket—for example, hundreds of thousands of reminders on one grain identity—must still be materialized as one provider range. Distribute operational reminder ownership across grain identities and load-test the largest expected per-hash bucket.
