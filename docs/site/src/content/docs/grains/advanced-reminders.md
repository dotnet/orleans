...
The recovery service divides reminder storage into 4,096 hash ranges and advances through 256 ranges per one-minute reconciliation cycle instead of scanning the whole table during startup. Each provider returns continuation pages of at most 256 rows. Entries are repaired in batches of 32, and durable-job membership is loaded once per referenced shard within each range. Far-future entries are deferred until they enter `ShardLoadLookaheadPeriod`. The caller observes registration failures, Durable Jobs owns persisted delivery retries, and the singleton reconciliation cursor repairs persisted rows with missing handles. Persisted job handles remain authoritative while Durable Jobs executes or retries an occurrence. A forced repair rotates the occurrence token before replacing the job, so an older job becomes a harmless no-op if it later runs.
