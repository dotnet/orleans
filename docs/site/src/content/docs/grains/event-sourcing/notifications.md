...
# `JournaledGrain` state notifications

Override `OnStateChanged` to react after the confirmed version can have increased:
...
Override `OnTentativeStateChanged` to react when the tentative view changes:
...
`RaiseEvent` triggers this callback. It can also run when confirmation or synchronization changes the tentative suffix.
