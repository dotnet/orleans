...
description: Build an Orleans 10 URL shortener with ASP.NET Core and .NET 10.
ms.date: 08/02/2026
ms.topic: quickstart
ms.devlang: csharp
---
...
This quickstart is the canonical beginner path for Orleans 10. You build a URL shortener in one ASP.NET Core process, define a grain, persist its state in memory for local development, and call it from Minimal API endpoints.
...
dotnet package add Microsoft.Orleans.Server --version 10.*
```
