...
`ResourceOwnershipConsumer` drives the typed gates for a finite set of service resources. Its caller feeds installed provider views into `InstallViewAsync`. The consumer validates the provider's service and authority namespace and rejects views older than the provider's current observation at installation, admission, and asynchronous completion boundaries. Provider advancement therefore closes old-view admission even before the caller installs the next local view. The consumer compares local owned sets, synchronously registers transitions, and then exposes the new local assignment. A service implements `IResourceOwnershipProtocol` for checkpoint/replay, peer transport, and recipient-enforced external fencing.

A continuous move requests the predecessor's retained state after its release has drained. Advancing the local view retires retained entries for superseded handoff targets; an active release holds its own state reference until it finishes. That release completes its recipient-fenced checkpoint across later placement views while validating authority availability. Missing continuity, including skipped A -> B -> A placement, selects recovery. The destination installs state and acquires an external fence before opening admission. Requests and snapshot replies are tied to a receiver identity and view, and the consumer rechecks those identities after waits. A delayed response therefore cannot populate a replacement receiver.

When a pending acquisition is superseded before its receiver becomes ready, the following release completes after the acquisition aborts. Handoff requires a ready predecessor, so the successor recovers durable state. A failed predecessor acquisition retains its original exception and blocks subsequent transitions.
