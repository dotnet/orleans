...
After receiver and registration drains succeed, retirement unregisters each indexed publisher and persists removal of that index entry. The manager logs each queue failure and continues visiting the remaining queues. After the sweep, it reports the original exception for one failed queue or an aggregate of multiple failures. The remaining durable entries support another stop attempt. Ambiguous index writes trigger a state reread before subsequent mutation, so a retry works from the storage provider's committed state.
