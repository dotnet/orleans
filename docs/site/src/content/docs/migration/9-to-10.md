...
| Call filters | No new Orleans 10 filter registration model | Continue registering incoming and outgoing filters on <xref:Orleans.Hosting.ISiloBuilder> or <xref:Orleans.Hosting.IClientBuilder>. |
...
For the clustering metadata migration, apply the schema migration before rolling Orleans binaries. The migration retains the legacy membership queries and database routine signatures, so older silos continue to write during the rollout. Metadata-aware silos select the versioned query bundle when they initialize. Restart a metadata-aware silo which initialized before the migration so that it selects the versioned contract.

Other membership providers store silo metadata inline in their existing membership records, documents, or key/value values. During a mixed-version rolling upgrade, older replace-style provider writes can temporarily remove the inline metadata field. Each active metadata-aware silo restores its own metadata on its next heartbeat.
