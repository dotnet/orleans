...
Register the [ADO.NET](https://learn.microsoft.com/dotnet/framework/data/adonet/ado-net-overview) partitioned stream provider with <xref:Orleans.Hosting.SiloBuilderAdoNetStreamExtensions.AddAdoNetStreams*>. Install the matching database driver and apply the SQL Server, PostgreSQL, or MySQL streaming schema shipped in the package source. Each queue maps to an immutable, ordered stream partition with a durable, ownership-fenced checkpoint. Message-ID allocation and insertion commit together, producing gap-free IDs within each partition. The stream partition pipeline resumes strictly after the earliest safe scan/delivery checkpoint, redelivering uncheckpointed records after recovery. Initial acquisition can start immediately before the earliest retained message. Ownership acquisition propagates cancellation, and a receiver retains its queue reservation until an in-flight acquisition command settles.

<xref:Orleans.Configuration.AdoNetStreamOptions.RetentionPeriod*> retains checkpointed records for one day by default before cleanup. <xref:Orleans.Configuration.AdoNetStreamOptions.MaximumRetentionPeriod*> makes older records eligible for hard deletion, including unprocessed records. Retention and cleanup intervals must be at least one second; for valid intervals, fractional seconds round upward to the next whole second. Cleanup inspects at most <xref:Orleans.Configuration.AdoNetStreamOptions.CleanupBatchSize*> records in message-ID order and deletes the eligible prefix ending before the first ineligible record. A younger leading record therefore retains later records even when database timestamps move backward.

The receiver checks each read batch for consecutive IDs beginning immediately after its admitted offset. When a cleanup response fails, the next read reconciles durable retained bounds with that offset, using the next message ID to detect fully deleted history. An unread gap permanently faults that receiver's source with <xref:Orleans.Streams.DataNotAvailableException>; subsequent reads report the same failure. Size retention for the expected processing lag and recovery window, and use the reported partition and missing message IDs to investigate data loss. <xref:Orleans.Configuration.AdoNetStreamOptions.FaultOnDeliveryFailure*> can fault one failing subscription while preserving the multicast partition record.
