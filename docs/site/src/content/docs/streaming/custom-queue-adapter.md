...
Use <xref:Orleans.Streams.IQueueCache.TryGetCacheCursor*> and <xref:Orleans.Streams.IQueueCache.TryGetCacheCursorAtPosition*> for cursor acquisition, and <xref:Orleans.Streams.IQueueCacheCursor.MoveNextWithResult*> for live-cache movement. Their typed results distinguish successful acquisition or delivery, an empty live tail, unavailable cache positions, and unsupported start positions. Retained-history cursors continue through the asynchronous replay path until they attach to the live cache.
